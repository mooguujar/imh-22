...
var RBChatDialogHelper = (function () {
    ...
    */
    UIModule7.prototype.showGSDialog = function (title, fn_closeCallback, okButtonText
        ...
        var html =
            "<div id=\'dialog-" + dialogId + "\' class=\'groupSend-dialog\' style=\'position: fixed; z-index: 9999;\'>" +
            "    <div class=\'mask show\'>" +
            ...
            "                       <a id=\'dialog-header-expandbtn-" + dialogId + "\' class=\'close light-box-close\'><i class=\'icon-arrow-bottom\'></i></a>" +
            "                    </div>" +
            "                </header>" +
        ...
        // 关闭按钮点击事件处理
        $("#dialog-header-expandbtn-" + dialogId).click((e) => {
            const $icon = $(e.currentTarget).find("i");
            if ($icon.hasClass("icon-arrow-bottom")) {
                $(`#dialog-body-${dialogId}`).hide()
                $icon.removeClass("icon-arrow-bottom").addClass("icon-arrow-top");
                $(`#dialog-${dialogId} header`).css('border-radius', '10px')
            } else {
                $(`#dialog-body-${dialogId}`).show()
                $(`#dialog-${dialogId} header`).css('border-radius', '10px 10px 0 0')
                $icon.removeClass("icon-arrow-top").addClass("icon-arrow-bottom");
            }
        });
        // 关闭按钮点击事件处理
    ...
    */
    ...
    UIModule7.prototype.showDialog = function (title, cancelButtonText, okButtonText
        , bodyHTML, dialogId, fn_cancelCallback, fn_submitCallback, isShowFooter, lightboxStyleExtra, bodyStyleExtra
        , notShowCancelButton, notShowOkButton, options = {}, notShowCloseButton) {

        const { files, sendUsers } = options
        ...
        var html =
            ...
            "            <div class=\'lightbox\' " + (lightboxStyleExtra ? "style=\'" + lightboxStyleExtra + "\'" : "") + ">" +
            "                <header>" +
            "                    <h4 id=\'dialog-header-title-" + dialogId + "\'>" + (title ? title : "提示") + "</h4>" + (notShowCloseButton ? '' : 
            "                    <a id=\'dialog-header-closebtn-" + dialogId + "\' class=\'close light-box-close\'><i class=\'icon-close\'></i></a>") +
            "                </header>" +
            "                <div id=\'dialog-body-" + dialogId + "\' class=\'body\' " + (bodyStyleExtra ? "style=\'" + bodyStyleExtra + "\'" : "") + ">" +
            (bodyHTML ? bodyHTML : "") +
        ...
        // 发送图片/发送视频时ctrl+enter为换行
        if (title == '发送图片' || title == '发送视频') {
            const _el = $(`#dialog-${dialogId} textarea`)
            _el.on('keydown', function(event) {
            ...
            $(`#dialog-${dialogId}`).on('drop', function (e) {
                e.preventDefault()
                ...
                let fileExt = files[0].fileMd5.substr(files[0].fileMd5.lastIndexOf(".") + 1);
                if (images_ext.indexOf(fileExt.toLowerCase()) != -1) {
                    originFileType = 'image'
                } else if (video_ext.indexOf(fileExt.toLowerCase()) != -1) {
                    originFileType = 'video'
                }
                ...
                for (let i = 0; i < _files.length; i++) {
                    ...
                    if (
                        (originFileType == 'image' && images_ext.indexOf(fileExt.toLowerCase()) == -1) ||
                        (originFileType == 'video' && video_ext.indexOf(fileExt.toLowerCase()) == -1)
                    ) {
                        RBChatDialogHelper.showAlertDialog_INFO('提示', '请选择同类型的文件!');
                        return
                    }  

                    if (originFileType == 'image' && file.size > 10 * 1024 * 1024) {
                        //大小限制判断
                ...
                RBChatChattingContentPaneUI.circle_update_file(uploadFiles.length, resultList, uploadFiles, function () {
                    // 上传成功
                    if (resultList.length > 0) {
                        RBChatDialogHelper.showMulImageSendDialog(resultList, originFileType == 'image', false, sendUsers)
                        // 取消按钮
                        $("#dialog-footer-cancelbtn-" + dialogId).click()
                    } else {
                        alert('上传图片失败')
                    }
        ...
        var func = function (e) {
            // 发送图片/发送视频时ctrl+enter为换行
            if (e.ctrlKey && e.key === 'Enter' && (title == '发送图片' || title == '发送视频')) return
            if (e.keyCode == 13) {
                fn_submitCallback ? fn_submitCallback() : fn_closeDialogDefalt()
                e.preventDefault();
            }
     ...
     * @returns 
     */
    UIModule7.prototype.showMulImageSendDialog = function (files, is_img = true, isMoive=false, sendUsers) {
       
        var that = this;
        // 群发弹框关掉
        if(window.groupChatId){
            that.closeDialog(window.groupChatId);
            window.groupChatId = null;
        }
        let isGroupSend = window.userList && window.userList.length > 0;
        var dialogId = this.nextDialogId();
        var file_html = RBChatUtils.draw_mul_pic(files, !is_img,isMoive)
        let inputContent = isGroupSend ? '(包含小程序请用|||分隔,例如app:xxx|||内容)' : ''
        // 要显示于对话框中的html内容
        var bodyHTML =
            "<div class=\'mul-img-send\' style='margin-left:10px;height:600px'>" +
            "  <div class='row-1'>" + file_html + "</div>" +
            //(isGroupSend ? "":"  <div class='row-3'> <input name='hb-send' id='hb-send' type='checkbox' value='' checked/><span>合并发送</span></div>" )+
            " <div class='row-2'><textarea id=\'dialog-info-" + dialogId + "\' rows='10' style='padding:3px;width:350px'  placeholder=\'说点什么 ..." + inputContent + "\' /> </div>" +
            "</div>";
        var isHebing = true;
        var onSure = function () {
            const text = $('#dialog-info-' + dialogId).val();
            // 选中
            if (isGroupSend || isHebing) {
                const msgBody = {
                    text,
                    files,
                    type: is_img ? '0' : '1',
                ...
                if(!isGroupSend){
                    ...
                    if (result && result.length) {
                        result.forEach((str, index) => {
                            const t = str.toLowerCase().substr(0, 4)
                            if (t == 'app:') {
                                hasApp = true
                    ...
                    if ((result.length > 1 || hasApp)&& !isMinApp) {
                        alert('没有找到相关得小程序')
                        return
                    }
                    
                    const _arr = [].concat(window.userList)
                    if (!window._groupSendMap) {
                        window._groupSendMap = {}
                    }

                    let total = _arr.length
                    if (apps.length) {
                        total = _arr.length * 2
                    }
                    ...
                    RBChatGroupSend.groupSendTaskOp(task, 'add')

                    that.closeDialog(dialogId);
                    setTimeout(() => {
                        apps.forEach(app => {
                            const arr = RBChatUtils.chunkArray(_arr, 150)
                            arr.forEach((item, index) => {
                                setTimeout(() => {
                                    const infos = item.map(item2 => {
                                        const fp = MBProtocalFactory.genFingerPrint()
                                        window._groupSendMap[fp] = {
                                            uid: item2[0],
                                            content: app,
                                            msgType: 9,
                                            nk: item2[1],
                                    ...
                                    const p = GChatDataHelper.addSystemInfo_groupSendForLocalUser(infos, app, 9, id)
                                    IMSDK.sendData(p);
                                }, 0 * 1000)
                            })
                        })
                    }, firstSend == 'app' ? 0 : 1000)
                    ...
                    setTimeout(() => {
                        ...
                        arr.forEach((item, index) => {
                            setTimeout(() => {
                                const infos = item.map(item2 => {
                                    ...
                                    window._groupSendMap[fp] = {
                                        uid: item2[0],
                                        content: JSON.stringify(_msgBody),
                                        msgType: 13,
                                        nk: item2[1],
                                        time: Date.now()
                                    }
                                ...
                                const p = GChatDataHelper.addSystemInfo_groupSendForLocalUser(infos, JSON.stringify(_msgBody), 13, id)
                                IMSDK.sendData(p);
                        ...
                        // RBChatChattingContentPaneUI.circleSendGroupMsg(_arr.length, JSON.stringify(_msgBody), 13, [].concat(_arr), function () {
                        //     window.userList  = null;
                        //     that.closeDialog(dialogId);
                        // }, apps.length ? true : false)
                    }, firstSend == 'content' ? 0 : 1000)
                }
                // 未选中
            } else {
                if (!RBChatMainUI.getCurrentSelectedAlarmDataId() && !sendUsers) {
                    that.closeDialog(dialogId);
                    return;
                }

                let sendUser = sendUsers
                if (!sendUsers) {
                    sendUser = { currentSelectedAlarmType: RBChatMainUI.getCurrentSelectedAlarmType(), currentSelectedAlarmDataId: RBChatMainUI.getCurrentSelectedAlarmDataId() };
                }
                // 发送图片
                if (is_img) {
                    for (file of files) {
        ...
        // 先把对话框显示出来
        this.showDialog(is_img ?'发送图片':'发送视频'//" "
            , "取消"
            , '发送'
            , bodyHTML
            , dialogId
    ...
    UIModule7.prototype.retryGroupSend = function () {
        return
        setTimeout(() => {
            const _keys = Object.keys(window._groupSendMap || {})
            if (_keys.length) {
                const msgTypeMapping = {}
                _keys.forEach(fp => {
                    const { uid, content, msgType, time, retryTimes = 0 } = window._groupSendMap[fp]
                    // 存在第二次的群发消息，不走重发逻辑
                    if (time + 30 * 1000 < Date.now()) return
                    if (retryTimes >= 2) {
                        delete window._groupSendMap[fp]
                        return
                    }
                    // window._groupSendMap[fp].retry = true
                    const _infoKey = `${msgType}_${content}`
                    const fp2 = MBProtocalFactory.genFingerPrint()
                    window._groupSendMap[fp2] = { ...window._groupSendMap[fp], time: Date.now(), retryTimes: retryTimes + 1 }
                    delete window._groupSendMap[fp]
                    const msg = { uid, fp: fp2 }
                    if (!msgTypeMapping[_infoKey]) {
                        msgTypeMapping[_infoKey] = [
                            msg
                        ]
                    } else {
                        msgTypeMapping[_infoKey].push(msg)
                    }
                })
                const _keys2 = Object.keys(msgTypeMapping)
                if (_keys2.length) {
                    _keys2.forEach((key, index) => {
                        const [msgType, content] = key.split(/_(.+)/);  // 注意这个正则
                        setTimeout(() => {
                            const _arr = msgTypeMapping[key]
                            const arr = RBChatUtils.chunkArray(_arr, 150)
                            arr.forEach((params, index2) => {
                                setTimeout(() => {
                                    const p = GChatDataHelper.addSystemInfo_groupSendForLocalUser(params, content, msgType)
                                    IMSDK.sendData(p);
                                    if (arr.length - 1 == index2) {
                                        this.retryGroupSend()
                                    }
                                }, index2 * 1000)
                            })
    ...
    UIModule7.prototype.showImageSendDialog = function (dialogId, type, obj, fn_submitCallback) {
        ...
        if (type == 0) {
            ...
            var blob = obj instanceof File ? obj : obj.getAsFile(), reader = new FileReader();
     ...
     */
    UIModule7.prototype.showUserInfo = async function (ree) {
        ...
        var countryType = ree.countryType;
        ...
        let _isFriendFromServer = ree.isFriend == 1
        if (isFriend != _isFriendFromServer) {
            await RosterProvider.refreshRosterAsync()
            isFriend = RosterProvider.isUserInRoster(uid);
        }
        ...
        if (RBChatUtils.isMobile()) {
            bodyHTML +=
                ...
                "			<a id=\'im-panel-main-rightdetail-content-user-avatar_a-root" + dialogId + "\' target='_blank' href=\'" + heade_photo + "\'>" +
                "               <img id=\'im-panel-main-rightdetail-content-user-avatar_" + dialogId + "\' src=\'" + heade_photo + "\' onerror='javascript:$(this).remove()'>" +
                "           </a>" +
                ...
                "		</div>" +
                "		<div class=\'info\'>" +
                "			<h4 title=\'" + nicknameWithRemark + "\'>" + nicknameWithRemark +"("+(countryType=='CH'?'中国用户':'越南用户')+")"+ "</h4>" +
                ...
                "		<dd><span class=\'label\'>最近上线：</span><span class=\'content\'>" + (isMe ? '当前在线': latest_login_time) + "</span></dd>" +
            ...
            bodyHTML +=
                ...
                (hasRemark ?
                    "   <dd><span class=\'label\'>昵称：</span><span class=\'content\'>" + nickname + "</span></dd>" : "") +
                "		<dd><span class=\'label\'>ID号：</span><span class=\'content\'>" + uid + "</span></dd>" +
                "		<dd><span class=\'label\'>注册来源：</span><span class=\'content\'>"+sourceTxt+"</span></dd>" +
                "		<dd><span class=\'label\'>设备类型：</span><span class=\'content\'>"+deviceType+"</span></dd>" +
                "		<dd><span class=\'label\'>用户类型：</span><span class=\'content\'>"+(ree.userType - 0 == 1?'游客':'注册用户' )+"</span></dd>" +
                "		<dd><span class=\'label\'>注册时间：</span><span class=\'content\'>" + register_time + "</span></dd>" +
                "		<dd><span class=\'label\'>最近上线：</span><span class=\'content\'>" +  (isMe ? '当前在线': latest_login_time) + "</span></dd>" +
                "		<dd><span class=\'label\'>登陆IP：</span><span class=\'content\'>" + ree.latest_login_ip + "</span></dd>" +
                "		<dd><span class=\'label\'>注册IP：</span><span class=\'content\'>" + ree.registerIp + "</span></dd>" +
                "		<dd><span class=\'label\'>登陆地址：</span><span class=\'content label-loginAddress-2\'></span></dd>" +
                "		<dd><span class=\'label\'>代理群管id：</span><span class=\'content\'>" + agentUid + "</span></dd>" +
                "		<dd><span class=\'label\'>代理群管ued：</span><span class=\'content\'>" + agentUed + "</span></dd>" +
                "		<dd><span class=\'label\'>上次存款时间：</span><span class=\'content\'>" + (ree.uedLastRechargeDate || '') + "</span></dd>" +
                "		<dd><span class=\'label\'>UED首存时间：</span><span class=\'content\'>" + (ree.uedFirstRechargeDate || '') + "</span></dd>" +
                // "		<dd><span class=\'label\'>最近 IP：</span><span class=\'content\'>" + latest_login_ip + "</span></dd>" +
                "	</dl>" +
                (isMe ? "" : "	<dl>" +
                    "	   <dt>营销策略</dt>" +
        ...
        setTimeout(() => {
            RBChatUtils.getCityByIP(ree.latest_login_ip).then(res => {
                document.querySelector('.chat-user-info .label-loginAddress-2').innerHTML = res
            })
        })
        ...
        // 否则显示的是别人的信息
            ...
            if (isFriend) {
                ...
                var fn_submitCallback = function () {
                    ...
                    window._chatId = null
                    if (RBChatSmallWindowUI.checkIsWindow(srcUid)) {
                        window._chatId = srcUid
                        RBChatSmallWindowUI.resetWindow(srcUid)
                    }
        ...
        if (isMe) {
            ...
            function toBase64(imgUrl) {
                ...
                const imageUrl = imgUrl +'?nocache=' + formattedDate;
                image.src = imageUrl
                // image.onload为异步加载
                image.onload = () => {
    ...
    UIModule7.prototype.showMinAppInfo = function (ree) {
        ...
        if (ree && ree.length > 0) {
            ...
            const modifyUrl = (url) => {
                // 创建一个 URL 对象来方便解析 URL
                let urlObj = new URL(url);
                if (urlObj.searchParams.has('userId')) {
                    urlObj.searchParams.set('userId', LocalUserInfo.getUid());
                }
            ...
            ree.forEach(item => {
                contentHTML = contentHTML + "<div class='my-min-app-item' onclick=\"javascript:minappJump('" + modifyUrl(item.appletUrl) + "','" + item.appletId + "')\"> <img src='" + item.appletImage + "'/> <span>" + item.appletName + "</span></div>"
            })
    ...
    UIModule7.prototype.showFriendRemarkEdit = function (friendInfo) {
        ...
        var bodyHTML =
            ...
            "			<div class=\'right\'>" +
            "				<input id=\'dialog-modifyfriendremark-form-remark-" + dialogId + "\' maxlength=\'32\' class=\'text ember-text-field\' type=\'text\' value=\'" + oldRemark + "\' placeholder=\'备注最多允许32个汉字\'>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
                ...
                , function (returnValue) {
                        if (returnValue) {
                            ...
                            if ('1' == returnValue) {
                                ...
                                // 更新右侧好友信息查看界面中的显示
                                if (RBChatMainUI.getCurrentSelectedAlarmDataId() == friendUid) {
                                ...
                                // 更新首页消息列表中的显示
                                RBChatAlarmsUI.updateItemTitle(AlarmMessageType.reviceMessage, friendUid, remark);
    
                                // 更新好友列表中的显示
                                RBChatRosterUI.updateFriendNicknameWithRemark(friendUid, remark);
    ...
    UIModule7.prototype.showFriendRemarkEdit2 = function (friendInfo) {
        ...
        var bodyHTML =
            ...
            "			<div class=\'right\'>" +
            "				<input id=\'dialog-modifyfriendremark-form-remark-" + dialogId + "\' maxlength=\'50\' class=\'text ember-text-field\' type=\'text\' value=\'" + oldRemark + "\' placeholder=\'备注最多允许50个汉字\'>" +
            "			</div>" +
            "		</li>" +
            "	</ul>" +
            "</form>";
        ...
        var fn_submitCallback = function () {
            ...
            RBChatRestHelper.submitMerberRemarkModifiyToServer(remark, friendUid
                ...
                , function (returnValue) {
                     if (returnValue) {
                        if (RBChatMainUI.getCurrentSelectedAlarmDataId() == friendUid) {
                            RBChatSmallWindowUI.getWindowDom('.im-panel-main-rightdetail-content-lb-remark4friend-2').text(remark)
                        }
                        // 先关闭当前修改对话框
                        that.closeDialog(dialogId);
                        RBChatRightDetailUI.updateMemberRemarkButtonVisibility(true)
                        return;
                    } else {
    ...
    // 显示设置好友分组
    UIModule7.prototype.showSetFriendsGroup = function(){
        var that = this;
        var dialogId = that.nextDialogId();
        var  condition_div  = '<div class="friend-goup-conditions"><div id="friend-goups-div"></div><div id="friend-goups-add">新增分组</div></div>';
        var search_div = '<div class="friend-group-input"><input type="text" id="friend-group-input" placeholder="请输入用户的昵称进行搜索"/></div>'
        var  list_div = '<div class="friend-goup-list" id="friend-goup-list"></div>';
        var bodyHTML= '<div class="friend-goup-root">'+condition_div+search_div+list_div+'</div>';
        // 默认选择未分组
        var group_select=  window.friends_group_list.filter(item=> item.groupId=='0')[0];
        // 初始分组
        var init_group = function(list){
            //分组创建
            $('#friend-goups-div').empty();
            var html = "<span >未分组用户列表("+list.length+"个)</span>";
            $('#friend-goups-div').html(html)
            init_group_user(group_select,list);
        }   

         // 初始分组用户
         var init_group_user = function(groupInfo,list){
            //分组创建
            $('#friend-goup-list').empty();
            var html = "<div class='fgs-user-item'><input type='checkbox' value='0' id='user-select-all'/><div style='flex:1'>用户id</div><div style='flex:2'>昵称</div><div style='flex:1'>分组</div></div>";
            list.forEach(item => {
                var nickname=(item.nickNameWithRemark || item.nickname);
                html+= "<div class='fgs-user-item'>"+("<input type='checkbox' class='user-list-checkbox' value='"+item.user_uid+"'/>")+"<div style='flex:1'>"+item.user_uid+"</div><div style='flex:2'>"+nickname+"</div><div style='flex:1'>"+groupInfo.groupName+"</div></div>";
            })
            $('#friend-goup-list').html(html)
            $('#user-select-all').change(function() {
                var that = this;
                $('.user-list-checkbox').each(function(index, i){
                    if (index > 100) return
        ...
        var sureFunc = function(){
            ...
            if(users.length > 0){
                ...
                RBChatDialogHelper.showGroupSelectForm(group_select.groupId,function(group_id){
                    RBChatRestHelper.cmd_1008_2_70(group_id,users.join(','),function(){
                        RBChatRosterUI.deal_fen_local_to_last(function () {
                            RBChatRosterUI.reloadFromCache();
                            // 刷新会话
                            RBChatAlarmsUI.reload(null);
                            //好友分组
                            group_select=  window.friends_group_list.filter(item=> item.groupId=='0')[0];
                            //初始分组
                            init_group(group_select.list || [])
        ...
        //初始分组
        init_group(group_select.list || []);
        $('#friend-group-input').on('input', function() {
            var input = $(this).val();
            if(!input || input.length == 0){
                init_group(group_select.list || [])
            }else{
                var list = group_select.list || [];
                var  select_list = list.filter(item=> {
                    var name = item.nickNameWithRemark || item.nickname;
                    return name.indexOf(input) !=-1;
                })
                init_group(select_list)
            }
        });
        //新增分组
        $('#friend-goups-add').click(function(){
            // 创建分组成功，重新回掉
            RBChatDialogHelper.showGroupCreateForm(function(){
                 //初始分组
                 init_group(group_select.list || [])
            });
        })
     ...
     * 发送合集列表ui
     */
     UIModule7.prototype.getFastSendItemUI = function (data = {}, index, type) {
        const { id, files = [], content = '', selected, title = '' } = data
        let _index = index
        if (_index == undefined) {
            _index = $('.fastSend-item').length + 1
        }
        const fileUrl = files.map(item => RBChatUtils.getImageDownloadURL(item.fileMd5))?.join(',') || ''
        const html = `
            <li class="fastSend-item fastSend-item-${id}" data-id="${id}">
                <h3>
                    <input class="fastSend-item-checkbox" type="checkbox" ${selected == 1 ? 'checked' : ''} style="${type == 'send' ? '' : 'display: none;'}">
                    <span class="fastSend-item-index">第${_index}条</span>
                    <input class="title-input" placeholder="标题" value="${title}" />(标题,不作为内容发送)
                </h3>
                <div class="fastSend-file-box">
                    <input class="url-input" disabled placeholder="请选择文件上传" value="${fileUrl}" />
                    <span class="fastSend-uploadBtn" onclick="javascript: fastSendFileSelect(this)">选择文件</span>
                    <span class="fastSend-delete" onclick="javascript: fastSendDelete(this)">删除</span>
                    <input type="file" name="upload-image-mul-input" multiple="" onchange="javascript: fastSendFileUpload(event)" class="file-input" style="display: none;">
                </div>
                <textarea class="fastSend-item-textarea" placeholder="请输入发送内容" rows="5" style='width:100%'>${content}</textarea>
            </li>
        `
     ...
     * 发送合集配置(list)
     */
     UIModule7.prototype.showFastSendForm = function (type, successFn) {
        ...
        const _cache = JSON.parse(localStorage.getItem('fast-send-data') || '[]')
        let html = ''
        _cache.forEach((item, index) => {
            html += this.getFastSendItemUI(item, index + 1, type)
        })
        const _emptyEl = '<div class="fast-send-add">暂无数据<span class="fast-send-add-btn">新增一条</span></div>'
        if (!html) {
            html = _emptyEl
        }
        ...
        // 要显示于对话框中的form表单html内容
        var bodyHTML =`
            <div class="fast-send-container">
                <span class="fast-send-add-btn-2">新增一条</span>
                <ul class='fast-send-list 22sda'>
                    ${html}
                </ul>
            </div>
            <div class="fast-send-loading">文件上传中...</div>
        `;

        // 选择文件
        window.fastSendFileSelect = function (el) {
            const fileInput =  $(el).parent().find('.file-input'); // 获取相邻的 input
            if (fileInput) {
                fileInput.click(); // 触发选择文件框
            }
        }

        // 文件上传
        window.fastSendFileUpload = function (e) {
            e.preventDefault()
            ...
            if (fileList.length) {
                RBChatDialogHelper.showAlertDialog_INFO('提示', '请选择图片或者视频!');
                return
            }

            const successCb = (files) => {
                const input = $(e.target).parent().find('.url-input')
                const fileUrl = files.map(item => RBChatUtils.getImageDownloadURL(item.fileMd5)).join(',')
                if (input) {
                    input.val(fileUrl)
                    input.attr('data-files', JSON.stringify(files))
                }
                if (type == 'send') saveCache()
            }
            if (imageList.length) { // 批量上传图片
                ...
                var resultList = []
                RBChatChattingContentPaneUI.circle_update_file(imageList.length, resultList, imageList, function () {
                    e.target.value = ''
                    // 上传成功
                    if (resultList.length > 0) {
                        successCb(resultList)
                    } else {
                        alert('上传图片失败')
                    }
                }, 'fastSend')
                ...
                var resultList = []
                RBChatChattingContentPaneUI.circle_update_file(videoList.length, resultList, videoList, function () {
                    e.target.value = ''
                    // 上传成功
                    if (resultList.length > 0) {
                        successCb(resultList)
        ...
        // 删除当前配置
        window.fastSendDelete = (e) => {
            const boxEl =  $(e).closest('.fastSend-item')
            boxEl.remove() // 删除当前项

            const _els = $('.fastSend-item')
            if (_els.length) { // 更新索引
                for (let i = 0; i < _els.length; i++) {
                    const _el = _els[i]
                    $(_el).find('.fastSend-item-index').text(`第${i + 1}条`)
                }
            } else { // 显示无内容ui
                $('.fast-send-add-btn-2').hide()
                $('.fast-send-list').append(_emptyEl)
                initEmptyAddEvent()
            }
            if (type == 'send') saveCache()
        }
        ...
        const saveCache = () => {
            const _els = $('.fastSend-item')
            const list = []
            const _old = JSON.parse(localStorage.getItem('fast-send-data') || '[]')
            for (let i = 0; i < _els.length; i++) {
                const _el = _els[i]
                const fileEl = $(_el).find('.url-input')
                const content = $(_el).find('.fastSend-item-textarea').val()
                const selected = $(_el).find('.fastSend-item-checkbox').is(':checked')
                const title = $(_el).find('.title-input').val()
                const id = $(_el).attr('data-id')
                let files = fileEl.attr('data-files') ? JSON.parse(fileEl.attr('data-files')) : []
                if (!files?.length && fileEl.val()) { // 没有修改时
                    files = _old.find(item => item.id == id)?.files || []
                }
                list.push({ content, files, id, title, selected: selected ? 1 : 0 })
            }

            localStorage.setItem('fast-send-data', JSON.stringify(list))
        }

        // 点击保存按钮要执行的回调函数
        var fn_submitCallback = () => {
            const _els = $('.fastSend-item')
            const list = []
            const _old = JSON.parse(localStorage.getItem('fast-send-data') || '[]')
            ...
            for (let i = 0; i < _els.length; i++) {
                ...
                const id = $(_el).attr('data-id')
                if (!content.trim()) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '请填写内容!');
                    return
                }
                let files = fileEl.attr('data-files') ? JSON.parse(fileEl.attr('data-files')) : []
                if (!files?.length && fileEl.val()) { // 没有修改时
            ...
            localStorage.setItem('fast-send-data', JSON.stringify(list))
            if (type == 'send' && !isSelected) {
                RBChatDialogHelper.showAlertDialog_INFO('提示', '请至少勾选一项发送!');
                return
            }
        ...
        // 初始化内容输入框事件
        const initTextareaEvent = (id) => {
            const el = $(`.fastSend-item-${id} .fastSend-item-textarea`)
            if (!el.length) return
            el.keydown((e) => {
                if (e.keyCode == 13) {
            ...
            el.blur((e) => {
                ...
                if (_val?.toLowerCase()?.substr(0, 4) == 'app:') {
                    const minAppFlag = _val.split(':')[1];
                    const obj = window.app_applets.find(item => item.appletId == minAppFlag || item.appletName == minAppFlag);
                    if (!obj) {
                        RBChatDialogHelper.showAlertDialog_INFO('提示', '该小程序不存在!');
                    }
                }
                if (type == 'send') saveCache()
            })
        ...
        // 初始化title输入框事件
        const initTitleInputEvent = (id) => {
            const el = $(`.fastSend-item-${id} .title-input`)
            if (!el.length) return
        ...
        // 初始化checkbox事件
        const initCheckboxEvent = (id) => {
            if (type != 'send') return
            const el = $(`.fastSend-item-${id} .fastSend-item-checkbox`)
            if (!el.length) return
            el.change((e) => {
                if (type == 'send') saveCache()
            })
        ...
        // 初始化新增事件
        const initAddEvent = () => {
            const el = $(`.fast-send-add-btn-2`)
            if (!el.length) return
            el.click((e) => {
                if ($('.fastSend-item').length >= 20) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '最多同时添加20条!');
                    return
                }
                const id = Date.now()
                const _html = this.getFastSendItemUI({ id }, null, type)
                $('.fast-send-list').append(_html)
                setTimeout(() => {
                    initTextareaEvent(id)
                    initCheckboxEvent(id)
                    initTitleInputEvent(id)
                    $(`#dialog-body-${dialogId}`).scrollTop($(`#dialog-body-${dialogId}`)[0].scrollHeight);
                })
            })
        }

        // 初始化无数据新增事件
        const initEmptyAddEvent = () => {
            $('.fast-send-add-btn').click(() => {
                const id = Date.now()
                const _html = this.getFastSendItemUI({ id }, null, type)
                $('.fast-send-add').remove()
                $('.fast-send-list').append(_html)
                $('.fast-send-add-btn-2').show()
                setTimeout(() => {
                    initTextareaEvent(id)
                    initCheckboxEvent(id)
                    initTitleInputEvent(id)
        ...
        setTimeout(() => {
            ...
            if (!_cache.length) {
                ...
                _cache.forEach(item => {
                    initTextareaEvent(item.id)
                    initCheckboxEvent(item.id)
                    initTitleInputEvent(item.id)
                })
                $('.fast-send-add-btn-2').show()
            }
        })
     ...
     * 批量添加好友
     */
     UIModule7.prototype.showBatchAddFriendForm = function () {
        ...
        var bodyHTML =
            ...
            "		<li>" +
            "			<div>" +
            "				<textarea id=\'dialog-queryuser-" + dialogId + "\' row='5' style='width:100%'/>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>请输入用户的ID号,按逗号隔开</p>" +
            "			</div>" +
            "		</li>" +
            "	</ul>" +
            "</form>";
        ...
        // 循环添加好友
        var circle_add_friend = function(ids,success_ids,fail_ids,callBack){
            if(ids.length > 0){
                var userId=ids[0]
                RBChatRestHelper.submitAddFriendRequestToServer(userId, '新朋友你好'
                    // 数据读取成功后的回调
                    , function (returnValue) {
                        success_ids.push(userId);
                        ids.splice(0, 1);
                        circle_add_friend(ids,success_ids,fail_ids,callBack);
                    }
                    // 数据读取失败后的回调
                    , function (errorThrownStr) {
                        fail_ids.push(userId);
                        ids.splice(0, 1);
                        circle_add_friend(ids,success_ids,fail_ids,callBack);
                    }
                );
        ...
        // 点击保存按钮要执行的回调函数
        var fn_submitCallback = function () {
                var userIds= $('#dialog-queryuser-'+dialogId).val();
                if(userIds){
                    var ids =userIds.split(',');
                    const success_ids =[];
                    const fail_ids =[];
                    circle_add_friend(ids,success_ids,fail_ids,function(){
                        alert('成功添加好友:'+success_ids.length+'个'+(fail_ids.length > 0 ? ',添加失败id:'+fail_ids.join(','):''));
                    });
                }else{
                    alert('好有信息不能为空')
                }
   ...
   */
    UIModule7.prototype.showSendMsgForm2 = function (uids, isBatch = false, title = '') {
        ...
        var bodyHTML =
            ...
            "		<li>" +
            "			<div>" +
            "				<textarea id=\'dialog-queryuser-form-uidormail-" + dialogId + "\' maxlength=\'500\' class=\'text ember-text-field\' type=\'text\' placeholder=\'要发送的消息内容(包含小程序请用|||分隔,例如app:xxx|||内容)\' style=\'padding: 5px;\' />" +
            "			</div>" +
            "		</li>" +
                    "<li id=\"upload-mul-image\">"+
                        "<i class=\"icon-chat2\" style='cursor:pointer;' onclick=\"send_pic();\" title=\"发送图片\"></i>"+
                        "<input type=\"file\" name=\"upload-image-mul-input\" multiple id=\"upload-image-mul-input\" style=\"display: none;\">"+
                    "</li>"+
            "	</ul>" +
            "</form>";
        
        window.send_pic = function(e){
            window.userList = uids;
            $('#upload-image-mul-input').click();
        }
        ...
        var fn_submitCallback = function () {
            ...
            window.userList = uids
            window._testTime = Date.now()
            if (!window._groupSendMap) {
                window._groupSendMap = {}
            }

            let total = uids.length
            if (apps.length && content) {
                total = uids.length * 2
            }
            ...
            setTimeout(() => {
                apps.forEach(app => {
                    const arr = RBChatUtils.chunkArray(uids, 150)
                    arr.forEach((item, index) => {
                        setTimeout(() => {
                            const infos = item.map(item2 => {
                                const fp = MBProtocalFactory.genFingerPrint()
                ...
                if (content) {
                    ...
                    arr.forEach((item, index) => {
                        setTimeout(() => {
                            ...
                            const p = GChatDataHelper.addSystemInfo_groupSendForLocalUser(infos, content, MsgType.TYPE_TEXT, id)
                            IMSDK.sendData(p);

                            if (!apps.length && index === arr.length - 1) {
                                console.log('All done');
                                that.retryGroupSend()
                            }
                        }, 0 * 1000)
                    })
                    // RBChatChattingContentPaneUI.circleSendGroupMsg(uids.length, content, MsgType.TYPE_TEXT, [].concat(uids), function () {
                    //     window.userList  = null;
                    // }, apps.length ? true : false)
                }
            }, firstSend == 'content' ? 0 : 1000)
    ...
    * 选择分组对话框。
    */
     UIModule7.prototype.showGroupSelectForm = function (unselectId,callBack, desc) {
        var that = this;
        // 对话框id，为了防止全局取id对象发生冲突，建议必须使用
        var dialogId = that.nextDialogId();

         var html = "<span>分组列表：</span><select id='friend-q-select'>";
         window.friends_group_list.forEach(item => {
                    if(item.groupId !=unselectId && item.groupId!='0')
                        html = html + "<option value='" + item.groupId+ "' "+">" + item.groupName+ "</option>"
        })
         html+="</select>"
         if (desc) {
            html += `<p class=\'hint\'><i class=\'icon-info\' ></i>${desc}</p>`
         }
        ...
        // 点击保存按钮要执行的回调函数
        var fn_submitCallback = function () {
            var groupId = $('#friend-q-select').val();
            that.closeDialog(dialogId);
            callBack(groupId);
        };

        // 显示对话框
        that.showDialog("移至分组"
            , "取消"
            , "确定"
            , bodyHTML
            , dialogId
    ...
    * 创建分组对话框。
    */
    UIModule7.prototype.showGroupCreateForm = function (callBack) {
        ...
        var getSelectUI = function (list, tag, id = '') {
            var html = "<select id='" + id + "' style='width: 100%;height: 34px;padding: 0 5px;'>"
            list.forEach(item => {
        ...
        var bodyHTML =
            ...
            "		<li>" +
            "			<div>" +
            "				<input id=\'dialog-queryuser-form-uidormail-fenlei\' maxlength=\'20' class=\'text ember-text-field\' type=\'text\' placeholder=\'分类名称\' style=\'padding: 0px 0 0px 5px;\'>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
        ...
        var fn_submitCallback = function () {
            ...
            // 取出界面上form表单字段内容
            var uidOrMail = $.trim($("#dialog-queryuser-form-uidormail-" + dialogId).val());
            var uidOrMailFenlei = $.trim($("#dialog-queryuser-form-uidormail-fenlei").val());
            ...
            RBChatRestHelper.submitCreateFenzu(uidOrMail, uidOrMailFenlei
                // 数据读取成功后的回调
                , function (returnValue) {
                    if (returnValue) {
                        // 添加分组
                        RBChatRosterUI.deal_fen_local_to_last(function () {
                            RBChatRosterUI.createFenzu(returnValue, uidOrMail, uidOrMailFenlei, 0, 0, true);
                            RBChatRosterUI.initGroupFenzuDragEvent()
                            that.closeDialog(dialogId);
                            if(callBack){
                                callBack();
                            }
    ...
    * 修改分组对话框。
    */
    UIModule7.prototype.showGroupModifyNameForm = function (groupId, groupName, groupType) {
        ...
        var bodyHTML =
            ...
            "		<li>" +
            "			<div>" +
            "				<input id=\'dialog-queryuser-form-uidormail-fenlei-2\' maxlength=\'20' class=\'text ember-text-field\' type=\'text\' placeholder=\'分类名称\' value='" + groupType + "' style=\'padding: 0px 0 0px 5px;\'>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
        ...
        var fn_submitCallback = function () {
            ...
            // 取出界面上form表单字段内容
            var uidOrMail = $.trim($("#dialog-queryuser-form-uidormail-" + dialogId).val());
            var uidOrMailFenlei = $.trim($("#dialog-queryuser-form-uidormail-fenlei-2").val());
            ...
            RBChatRestHelper.submitModifyFenzu(groupId, uidOrMail, uidOrMailFenlei
                // 数据读取成功后的回调
                , function (returnValue) {
                    if (returnValue) {
                        $(`#rstore-group-${groupId}`).attr('groupType', uidOrMailFenlei)
                        // 添加分组
     ...
     * 设置常用提示语
     */
    UIModule7.prototype.showVioceSettingDialog = function (ree) {
        ...
        const vioceMapping = [
            { id: 1, name: '提示音1' , path: 'audio_msg.mp3' },
            { id: 2, name: '提示音2' , path: 'audio_msg_1.mp3' },
            { id: 3, name: '提示音3' , path: 'audio_msg_2.mp3' },
            { id: 4, name: '提示音4' , path: 'audio_msg_4.mp3' },
            { id: 5, name: '提示音5' , path: 'audio_msg_5.mp3' },
            { id: 6, name: '提示音6' , path: 'audio_msg_6.mp3' },
            { id: 7, name: '提示音7' , path: 'audio_msg_7.mp3' },
        ]

        let itemHTML = "<span>选择提示音</span><select id='vocie-select'>"
        vioceMapping.forEach(item => {
            itemHTML += "<option value='" + item.path + "'>" + item.name + "</option>"
        })
        itemHTML += "</select>"
        ...
        // 要显示于对话框中的form表单html内容
        var bodyHTML =
            "<div id='vocie-select-box'>" 
            + itemHTML
            + 
            "<button>播放提示音</button></div>";

        setTimeout(() => {
            const uid = IMSDK.getLoginInfo().loginUserId 
            const _voice = localStorage.getItem(`${uid}_voice`)
            if (_voice) {
                document.getElementById("vocie-select").value = _voice
            }
            $('#vocie-select-box button').click(e => {
                const selectElement = document.getElementById("vocie-select");
                const selectedValue = selectElement.value
                RBChatUtils.playAudio(AUDIO_BASE_URL + selectedValue)
            })
        })
        ...
        var fn_submitCallback = function () {
            ...
            const selectedValue = selectElement.value
            const uid = IMSDK.getLoginInfo().loginUserId 
            localStorage.setItem(`${uid}_voice`, selectedValue)
            that.closeDialog(dialogId);
        };
    ...
    UIModule7.prototype.showWelcomDialog = function (ree) {
        ...
        var bodyHTML =
            ...
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置添加好友欢迎语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-09-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'您还没有设置直播用户点击私聊客服欢迎语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置直播用户点击私聊客服欢迎语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-08-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'您还没有设置未开发用户上线欢迎语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置未开发用户上线欢迎语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            ...
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置会员上线欢迎语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            ...
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置离线自动回复语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-04-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'设置一键打招呼[7天未存款]提示语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置一键打招呼[7天未存款]提示语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-05-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'设置一键打招呼[15天未存款]提示语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置一键打招呼[15天未存款]提示语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-06-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'设置一键打招呼[30天未存款]提示语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置一键打招呼[30天未存款]提示语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "		<li>" +
            "			<div>" +
            "				<i  style=\'position: absolute;left: 10px;top: 10px;font-size: 15px;color: #9d9d9d;\' ></i>" +
            "				<textarea id=\'dialog-welcom-07-" + dialogId + "\' maxlength=\'1000\' rows=\'5\' placeholder=\'设置一键打招呼[60天未存款]提示语\'></textarea>" +
            "				<p class=\'hint\'><i class=\'icon-info\' ></i>设置一键打招呼[60天未存款]提示语,最多1000字(发送小程序案例'app:XXX|||内容')</p>" +
            "			</div>" +
            "		</li>" +
            "	</ul>" +
            "</form>";

        const textAreaEvent = (dom) => {
            dom.blur(e => {
                const _val = e.target.value
                const arr = _val.split('|||')
                if (arr.length > 2) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '格式不规范,最多同时配置一条小程序跟一条文本消息!');
                    return
                } else if (arr.length == 1) { // 普通消息
                    return
                }
                let hasApp = false
                ...
                if (!hasApp) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '小程序不存在, 请重新配置!');
                }
            })
        }
        ...
        setTimeout(() => {
            if (ree.length > 0) {
                ...
                $('#dialog-welcom-04-' + dialogId).val(ree.length > 3 ? (ree[3] || '') : '');
                $('#dialog-welcom-05-' + dialogId).val(ree.length > 4 ? (ree[4] || '') : '');
                $('#dialog-welcom-06-' + dialogId).val(ree.length > 5 ? (ree[5] || '') : '');
                $('#dialog-welcom-07-' + dialogId).val(ree.length > 6 ? (ree[6] || '') : '');
                $('#dialog-welcom-08-' + dialogId).val(ree.length > 7 ? (ree[7] || '') : '');
                $('#dialog-welcom-09-' + dialogId).val(ree.length > 8 ? (ree[8] || '') : '');

                textAreaEvent($('#dialog-welcom-01-' + dialogId))
                textAreaEvent($('#dialog-welcom-02-' + dialogId))
                textAreaEvent($('#dialog-welcom-03-' + dialogId))
                textAreaEvent($('#dialog-welcom-04-' + dialogId))
                textAreaEvent($('#dialog-welcom-05-' + dialogId))
                textAreaEvent($('#dialog-welcom-06-' + dialogId))
                textAreaEvent($('#dialog-welcom-07-' + dialogId))
                textAreaEvent($('#dialog-welcom-08-' + dialogId))
                textAreaEvent($('#dialog-welcom-09-' + dialogId))
            }
        }, 100)
        ...
        var fn_submitCallback = function () {
            ...
            let content3 = $('#dialog-welcom-04-' + dialogId).val() || '';
            let content4 = $('#dialog-welcom-05-' + dialogId).val() || '';
            let content5 = $('#dialog-welcom-06-' + dialogId).val() || '';
            let content6 = $('#dialog-welcom-07-' + dialogId).val() || '';
            let content7 = $('#dialog-welcom-08-' + dialogId).val() || '';
            let content8 = $('#dialog-welcom-09-' + dialogId).val() || '';

            RBChatRestHelper.setWleockFromServer({ content0, content1, content2,content3,content4,content5,content6,content7,content8}
                // 数据读取成功后的回调
                , function (returnValue) {
    ...
    UIModule7.prototype.showTaskDetail = function (key) {
        ...
        var req_data = function () {
            $.ajax({
                ...
                url: RBChatConfig.FILE_HTTPS_URL + "/" + key,
                type: "GET",
    ...
    UIModule7.prototype.showWeiHuDialog = function () {
        ...
        var initEvent = function () {
            ...
            window.sendMuchMsg = function () {
                const list = search_user_list.filter(item => item.selected).slice(0, 3000)
                    if (list && list.length > 0) {
                        if(list.length > 3000){
                            alert('群发最大用户为3000!')
                            return;
                        }
                        that.showSendMsgForm2(list.map(item => [item.user_uid, item.nickname]), list.length > 1, list.length > 1 ? '向' + list.length + '个好友群发消息' : '')
                    } else {
                        alert('请选择需要发送得用户!')
                    }
            }
            ...
            // 群发消息(分组)
            window.sendMuchMsgByFenzu = function () {
                var dialogId = that.nextDialogId();
                var  list_div = '<div class="friend-goup-list friend-goup-list2" id="friend-goup-list2"></div>';
                var bodyHTML= '<div class="friend-goup-root">'+list_div+'</div>';
                // 默认选择未分组
                var group_select=  window.friends_group_list.filter(item=> item.groupId=='0')[0];
                // 初始分组
                var init_group = function(list){
                ...
                var init_group_user = function(groupInfo,list){
                    //分组创建
                    $('#friend-goup-list2').empty();
                    var html = "<div class='fgs-user-item'><input type='checkbox' value='0' id='user-select-all'/><div style='flex:1'>分组名称</div></div>";
                    window._groupFenzu.filter(item => item.v).forEach(item => {
                        html+= "<div class='fgs-user-item fgs-user-item2'>"+("<input type='checkbox' class='user-list-checkbox' value='"+item.v+"'/>")+"<div style='flex:1'>"+item.l+"</div></div>";
                    })
                    $('#friend-goup-list2').html(html)
                    $('#user-select-all').change(function() {
                        var that = this;
                        $('.user-list-checkbox').each(function(i){
                            $(this).prop('checked', that.checked);
                        });
                    });
                }  
                ...
                var sureFunc = function(){
                    var users =[]
                    $('.fgs-user-item2 input[type="checkbox"]:checked').each(function(i){
                        users.push($(this).val());
                    });
                    if(users.length > 0){
                        let arr = []
                        window._search_user_list.forEach(item => {
                            if (users.includes(item.groupId + '') || (users.includes('-1') && !item.groupId)) {
                                arr.push(item)
                            }
                        })
                        if (!arr.length) {
                        ...
                        if (arr.length > 3000) {
                            alert('所选分组群发最大用户为3000!')
                            return
                        }
                        that.closeDialog(dialogId)
                        that.showSendMsgForm2(arr.map(item => [item.user_uid, item.nickname]), arr.length > 1, arr.length > 1 ? '向' + arr.length + '个好友群发消息' : '')
                    }else{
                        alert('请选择分组')
                    }
                }
                // 先把对话框显示出来
                that.showDialog("群发消息(分组)"
                    , "取消"
                    , "确定"
                    , bodyHTML
                    , dialogId
            ...
            // 移动分组
            window.changeFenzu = function () {
                const users = search_user_list.filter(item => item.selected).map(item => item.user_uid)
                if(users.length){
                    if(users.length > 1000){
                        alert('选择用户数最大为1000')
                        return;
                    }
                    RBChatDialogHelper.showGroupSelectForm('',function(group_id){
                        RBChatRestHelper.cmd_1008_2_70(group_id,users.join(','),function(){
                            RBChatRosterUI.deal_fen_local_to_last(function () {
                                window.queryKh()
                                alert('操作成功')
        ...
        var getItemUI = function (item) {
            ...
            const show_t = nickName && nickName.length > 0 ? nickName.substr(0, 1).toUpperCase():'';
            let l_html = ''
            let m_html = ''
            let n_html = ''
            let s_html = ''
            let f_html = ''
            const groupName = window.friends_group_list.find(item2 => item2.groupId == (item.groupId || '0')).groupName
            ...
            if(item.isUEDFunc){
                ...
                f_html = ' <span<font color=black>' + groupName + '(所在分组)</font></span>'
            }
            ...
            itemsHTML += "<div class=\"group_dom\"  srcuid='" + userId + "'  \>" +
                ...
                "</div>"
            //ued及其他信息
            itemsHTML += "<div class='kh-user-ued'> " + l_html + m_html + n_html + s_html + f_html + "</div>"
            // 功能区域
        ...
        var reqData = function (params) {
            RBChatRestHelper.kehuToServer(params, function (returnValue) {
                ...
                if (!Object.keys(params).length) {
                    window._search_user_list = search_user_list
                }
                init_ui();
                initEvent();
        ...
        RBChatRestHelper.queryFenzu(
            ...
            function (returnValue) {
                ...
                var bodyHTML = "<div class='kehu-wh-root' id='kehu-wh-root'>" +
                    ...
                    "  <div class='row'><div class='item item2'><a class='kehu-wh-root-bt' href='javascript:void(0)' onclick='javascript:choiceAllSelect(1)'>全选</a> <a class='kehu-wh-root-bt' href='javascript:void(0)'  onclick='javascript:choiceAllSelect(0)'>取消</a> <a class='kehu-wh-root-bt k-l' href='javascript:void(0)'  onclick='javascript:sendMuchSMSMsg()'>群发短信</a> <a class='kehu-wh-root-bt k-l' href='javascript:void(0)'  onclick='javascript:sendMuchMsg()'>群发消息</a><a class='kehu-wh-root-bt k-l' href='javascript:void(0)'  onclick='javascript:sendMuchMsgByFenzu()'>群发消息(分组)</a><a class='kehu-wh-root-bt k-l' href='javascript:void(0)'  onclick='javascript:changeFenzu()'>移动分组</a><a class='kehu-wh-root-bt k-w' href='javascript:void(0)' onclick='javascript:groupAddFriend()'>群加好友</a> </div><div class='item'></div><div class='item item2 t-right'><button onclick='javascript:queryMonthNoLogin()' style='margin-right:10px'>查询一月未登录UED会员</button><button onclick='javascript:queryKh()'>查询</button></div> </div>" +
   ...
   */
                ...
                if (step == 0) {
                    ...
                    if (userList && userList.length > 0) {
                        ...
                        var send_area = "<div class='kchat-im-panel-main-chat-textarea' id='kchat-im-panel-main-chat-textarea-2' style='position: unset !important;border:1px solid #e2e4e8;z-index:999'>" +
                            ...
                            '</li>' +
                            '<li id="upload-file">' +
                            '<i class="icon-chat2" onclick="$(\'#upload-file-input-2\').click();" title="发送文件33"></i>' +
    ...
    */
        ...
        var showForward_html = function (list) {
            ...
            var fn_submitCallback = function () {
                ...
                if (!selectlist.length) {
                    sending = false;
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '请选择要转发的会话');
                }
                const arr = RBChatUtils.chunkArray(selectlist, 150)
                if (!window._groupSendMap) {
                    window._groupSendMap = {}
                }
                arr.forEach((item, index) => {
                    setTimeout(() => {
                        ...
                        const infos = friendArr.map(item2 => {
                            const fp = MBProtocalFactory.genFingerPrint()
                            window._groupSendMap[fp] = {
                                uid: item2[0],
                                content: msgContent,
                        ...
                        groupArr.forEach(item2 => {
                            const fp = MBProtocalFactory.genFingerPrint()
                            RBChatChattingContentPaneUI.doSendMessageImpl(msgContent, msgType, function () {
                            }, fp, {
                                currentSelectedAlarmType: item2[2],
                                currentSelectedAlarmDataId: item2[0],
                            }, false, true)
                        })
                        const p = GChatDataHelper.addSystemInfo_groupSendForLocalUser(infos, msgContent, msgType)
                        IMSDK.sendData(p);

                        if (index === arr.length - 1) {
                            console.log('All done');
                            RBChatDialogHelper.showAlertDialog_INFO('提示', '转发完成');
                            sending = false;
                            that.closeDialog(dialogId);
                        }
                    }, 0 * 1000)
        ...
        var fn_submitCallback = function () {
            ...
            RBChatRestHelper.submitAddFriendRequestToServer(friendUserUid, saySomthingToHim
                ...
                , function (returnValue) {
                    ...
                    if (data.retCode != 0) {
                        RBChatDialogHelper.showAlertDialog_INFO('提示', data.data);
                    }
                    // 再显示一个发送成功提示
     ...
     * 显示一键拉群的对话框。
     */
    UIModule7.prototype.showInviteGroupMemberDialog2 = function (uid, nickName) {
        var dialog = RBChatGroupMemberDialogFactory(
            GroupMemberDialogUsed.USED_FOR_INVIE_GROUP, uid, nickName);
        dialog.loadAndShow();
    };
     ...
     * 显示消息撤回的对话框。
     */
    UIModule7.prototype.showReCallDialog = function (gid) {
        var dialog = RBChatGroupMemberDialogFactory(
            GroupMemberDialogUsed.RE_CALL, gid);
        dialog.loadAndShow();
    };
        ...
        if (localUserInfoRee && ge) {
            ...
            if (isGroupOwner || ge.manage_mark - 0 == 1) {
                ...
                bodyHTML_content =
                    "<dl>" +
                    "	<dd>" +
                    "		<span style=\'max-width: 340px;min-height: 120px;\' class=\'content\'>" + RBChatChattingContentPaneUI.replaceEmojiPlaceholderToHTML(g_notice); + "</span>" +
                    "	</dd>" +
                    "</dl>";
            }
        ...
        $destSendBtnObj.uploadifive({
            ...
            'onUploadComplete': function (file, data) {
                ...
                if (dialogId) {
                    ...
                    if (l.length > 0) {
                        // l.eq(0).attr('src', 'https://oss.nongzhiw.cn/head/'+localUserInfo.userAvatarFileName)
                        l.eq(0).attr('src', RBChatConfig.FILE_HTTPS_URL + '/head/'+localUserInfo.userAvatarFileName)
                    } else {
                        // $('#im-panel-main-rightdetail-content-user-avatar_a-root' + dialogId).append(" <img id=\'im-panel-main-rightdetail-content-user-avatar_" + dialogId + "\' src=\'" + 'https://oss.nongzhiw.cn/head/'+localUserInfo.userAvatarFileName + "\' onerror='javascript:$(this).remove()'>")
                        $('#im-panel-main-rightdetail-content-user-avatar_a-root' + dialogId).append(" <img id=\'im-panel-main-rightdetail-content-user-avatar_" + dialogId + "\' src=\'" + RBChatConfig.FILE_HTTPS_URL + '/head/'+localUserInfo.userAvatarFileName + "\' onerror='javascript:$(this).remove()'>")
                    }
                } else {
                    if ($avatarObj)
                        // $avatarObj.attr('src', 'https://oss.nongzhiw.cn/head/'+localUserInfo.userAvatarFileName);
                        $avatarObj.attr('src', RBChatConfig.FILE_HTTPS_URL + '/head/'+localUserInfo.userAvatarFileName);
                }
            },
    ...
    UIModule7.prototype.showAdminOnlineNoticeAlert = function (noticeId) {
        const dialogId = this.nextDialogId();
        const fn_submitCallback = () => {
            RBChatRestHelper.updateNotice(noticeId)
            this.closeDialog(dialogId);
        }
        AudioPromptHelper.newMessagePromt()
        // 显示对话框
        this.showDialog("强制响应"
            , "取消"
            , "确定"
            , '<p style="color: red;padding-bottom: 10px;">这是强制响应，请立即点击确认响应, 以防超时</p>'
            , dialogId
            , null
            , fn_submitCallback
            , true
