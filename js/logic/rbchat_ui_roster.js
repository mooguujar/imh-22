...
var RBChatRosterUI = (function () {
     ...
     * 显示分组ui
     */
    UIModule3.prototype.showGroupUI= function(groupId, isShow, type){
            var that =this;
            // 收起
            if(!isShow){
            ...
            // 展开
                ...
                window.friends_group_list.forEach(item=>{
                    if(item.groupId- groupId != 0) {
                        if (type == 'check' && this.getCheckedFriends(item.groupId).length) return
     ...
     */
     UIModule3.prototype.countGroupUI =function(groupId='0'){
       ...
       if(obj){
            ...
            var on_line_list = obj.list.filter(item=> item.isOnline == 1 || item.onlineWeb == 1);
     ...
     * 创建分组
     */
    UIModule3.prototype.createFenzu = function(groupId,groupName,groupType,lineCount=0,allCount=0,isFrist=false, isFirstGroup = false){
        const status = isFirstGroup ? '1': '0';
        const  group_html = "<div status='"+status+"' class='rstore-friend-group' groupType='"+groupType+"' groupId='"+groupId+"' id='rstore-group-"+groupId+"'><div class='rstore-friend-group-item1'><img id='rstore-group-img-"+groupId+"' src='images/gourp-open.png'/> <span  id='rstore-group-name-"+groupId+"'>"+groupName+"</span></div> <span id='rstore-group-count-"+groupId+"'>"+lineCount+"/"+allCount+"</span></div>";
        var that = this;
        if(isFrist){
            ...
            if( window.friends_group_list &&  window.friends_group_list.length > 0){
                window.friends_group_list.forEach(item=>{
                    item.list.forEach(item_=>{
                        ...
                        if(item_.isOnline == 1 || item_.onlineWeb == 1){
                            online_count++;
                        }
                    })
                })
     ...
     *
     */
    UIModule3.prototype.getCheckedFriends = function(groupid) {
        const _attr = groupid ? `[data-fenzu-groupid="${groupid}"]` : ''
        const checkboxes = document.querySelectorAll(`.fenzu-checkbox${_attr}`);
        const checkedValues = [];

        checkboxes.forEach(cb => {
            if (cb.checked) {
                const obj = {
                    uid: cb.getAttribute('uid'),
                    nickname: cb.getAttribute('nickname'),
                    groupname: cb.getAttribute('groupname'),
                }
    ...
    UIModule3.prototype.add = function(ree, toFirst, groupId='0', isNewAddFriend=false, isAddFriendList=false){
        ...
        // 准备好好友的html
        const _nickname = RBChatUtils.getNickNameWithRemark(ree)
                ...
                RBChatRestHelper.queryFenzu(
                    ...
                    function (returnValue) {
                        ...
                        if(server_group_list.length > 0){
                            ...
                            const list = []
                            server_group_list.forEach(item => {
                                if (item[2]) {
                                    const obj = list.find(item2 => item[2] == item2.title && item2.type != 'default')
                                    const _obj = {
                                        title: item[1],
                                        id: item[0]
                                    }
                                    if (obj) {
                                        ...
                                        list.push({
                                            title: item[2],
                                            id: `parent_${Math.floor(Math.random() * 100000)}`,
                                            list: [_obj]
                                        })
                                    }
                                } else {
                            ...
                            for(var i = 0; i < list.length;i++){
                                ...
                                let itemHtml = ''
                                if (itemList?.length) {
                                    itemHtml = "<div class='im-panel-msg-popupmenu-move-group-list'>"
                                    itemList.forEach(item => {
                                        elIds.push(item.id)
                                        itemHtml += "<div class='im-panel-msg-popupmenu-move-group-list-item' id='im-panel-msg-popupmenu-move-group-"+item.id+"'>" + item.title + "</div>"
                                    })
                                    itemHtml += '</div>'
                                ...
                                itemHtml =  itemHtml ? item.title + itemHtml : item.title
                                const className = itemList?.length ? ' popParent' : ''
                                html = html + 
                                    "<li class='im-panel-msg-popupmenu-move-group-item" + className + "' id='im-panel-msg-popupmenu-move-group-"+item.id+"' data-title=" + item.title + ">移至: "
                                        + itemHtml +
                                    "</li>"
                            }
                            ...
                            y = y + server_group_list.length * 35 > windowHeight ? 
                                Math.max(0, windowHeight - server_group_list.length * 35) : y
                            // 在鼠标点击的位置显示菜单
                            newPopupObj.css("top", y + 'px');
                            newPopupObj.css("left", x + "px");
                            newPopupObj.show();
                            ...
                            try {
                                const _parents = document.querySelectorAll('.im-panel-msg-popupmenu-move-group-item.popParent')
                                _parents.forEach(el => {
                            ...
                            for(var i = 0; i < elIds.length;i++){
                                const item = elIds[i];
                                $("#im-panel-msg-popupmenu-move-group-"+item).click(function(){
                                    const checkedValues = that.getCheckedFriends()
                                    if (checkedValues.length) {
                                        const dialogId = RBChatDialogHelper.nextDialogId();
                                        const fn_submitCallback = () => {
                                            const _uids = checkedValues.map(item => item.uid).join(',')
                                            RBChatRestHelper.cmd_1008_2_70(item, _uids,function(rv){
                                                if(rv){
                                                    that.deal_fen_local_to_last(function(){
                                                        const _lastGroupId = document.querySelector('.rstore-friend-group[status="1"]').getAttribute('groupid');
                                                        newPopupObj.remove();
                                                        that.countGroupUI(item);
                                                        that.countGroupUI(_lastGroupId);
                                                        that.showGroupUI(groupId,true)
                                                    });
                                                }else{
                                                    alert('操作失败,请重试')
                                                }
                                        ...
                                        let itemHtml = "<div class='fgs-user-item'><div style='flex:1'>用户id</div><div style='flex:2'>昵称</div><div style='flex:2'>分组</div></div>";
                                        checkedValues.forEach(item => {
                                            const { uid, nickname, groupname } = item
                                            itemHtml+= "<div class='fgs-user-item'><div style='flex:1'>"+uid+"</div><div style='flex:2'>"+nickname+"</div><div style='flex:2'>"+groupname+"</div></div>";
                                        })
                                        const _groupTitle = $("#im-panel-msg-popupmenu-move-group-"+item).attr('data-title') || $("#im-panel-msg-popupmenu-move-group-"+item).text()
                                        const _html = `
                                            <p>您确定将以下好友都移动至 '${_groupTitle}' 分组吗?
                                            </p>
                                            <div style="padding-left: 10px;">
                                                ${itemHtml}
                                            </div>
                                        `
                                        RBChatDialogHelper.showDialog("批量分组移动"
                                            , "取消"
                                            , "确定"
                                            , _html
                                            , dialogId
                                        ...
                                        RBChatRestHelper.submitMoveFenzu(item,uid,function(rv){
                                            if(rv){
                                                that.deal_fen_local_to_last(function(){
                                                    const _lastGroupId = $(`#roster_li_uid_${uid}`).attr('bgroup')
                                                    newPopupObj.remove();
                                                    that.countGroupUI(item);
                                                    that.countGroupUI(_lastGroupId);
                                                    that.showGroupUI(groupId,true)
                            ...
                            parentIds.forEach(id => {
                                $(`#im-panel-msg-popupmenu-move-group-${id}`).on('mouseenter', e => {
                                    const $parent = $(e.target);
                                    const $child = $parent.find('.im-panel-msg-popupmenu-move-group-list');

                                    const offset = $parent.offset();             // 获取父元素在页面中的坐标
                                    const width = $parent.outerWidth();          // 获取父元素宽度
                                    const top = offset.top;
                                    const left = offset.left + width;            // 子菜单左边 = 父元素右边
                                ...
                                $(`#im-panel-msg-popupmenu-move-group-${id}`).on('mouseleave', e => {
                                    const $parent = $(e.target);
                                    $parent.find('.im-panel-msg-popupmenu-move-group-list').hide();
                                })
                            })
     ...
     * @param nickname
     */
    UIModule3.prototype.inviteGroupDialog = function(uid, nickname){
        //// 利用对话框返回的值 （true 或者 false）
        //if (confirm("确定要删除好友\""+nickname+"\"吗？")) {
        //    this.delete(uid);
        //}
    ...
    UIModule3.prototype.initGroupFenzuDragObj = {
    ...
    UIModule3.prototype.initGroupFenzuDragEvent = function() {
        const list = document.getElementById('kchat-im-panel-userlist-roster');
        let draggedItem = null;

         // 找到所有旧的 .rstore-friend-group，清除事件（通过 clone）
        // const oldItems = list.querySelectorAll('.rstore-friend-group');
        // oldItems.forEach(oldItem => {
        //     const newItem = oldItem.cloneNode(true); // 克隆（不会保留事件）
        //     oldItem.parentNode.replaceChild(newItem, oldItem); // 替换旧节点
        // });

        // 给每个分组 div 添加拖拽属性和事件
        document.querySelectorAll('.rstore-friend-group').forEach(item => {
            item.setAttribute('draggable', 'true');

            item.addEventListener('dragstart', (e) => {
                const _groupId = $(item).attr('groupid')
                const _status = $(item).attr('status')
                if (_status == 1) {
                    this.showGroupUI(_groupId, false)
                }
                draggedItem = item;
                e.dataTransfer.effectAllowed = 'move';
            ...
            item.addEventListener('dragend', () => {
                ...
                draggedItem = null;
                document.querySelectorAll('.rstore-friend-group').forEach(el => {
                    el.classList.remove('drag-over-top', 'drag-over-bottom');
                });
                const _newListUI = document.querySelectorAll('.rstore-friend-group')
                const _cache = []
                for (let i = 0; i < _newListUI.length; i++) {
                    _cache.push(_newListUI[i].getAttribute('groupid'))
                }

                localStorage.setItem('local_fenzuSort', JSON.stringify(_cache))
            });
            ...
            item.addEventListener('drop', (e) => {
                e.preventDefault();
                const dragging = list.querySelector('.dragging');
                if (!dragging || dragging === e.target) return;
                let nextSibling = e.target.nextSibling
                if (!nextSibling) {
                    const _firstEl = document.querySelectorAll('.rstore-friend-group')[0]
                    const _lastEl = document.querySelectorAll('.rstore-friend-group')[document.querySelectorAll('.rstore-friend-group').length - 1]
                    
                    if (_firstEl == e.target) {
                        nextSibling = document.querySelectorAll('.rstore-friend-group')[1]
                    } else if (_lastEl == e.target) {
                        list.appendChild(dragging)
                        return
                    }
                }
                if (!e.target.classList.contains('drag-over-top') && nextSibling) {
                    list.insertBefore(dragging, nextSibling);
                } else {
                    list.insertBefore(dragging, e.target);
