...
var RBChatChattingContentPaneUI = (function () {
    ...
    var UIModule5 = function (argument) {
        ...
        // 聊天内容显示区外层父div
        this.$chatBox = (chatId) => {
            return window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap', null, chatId) : $('#im-panel-main-chatcontentpane-wrap');
        }
        // 聊天内容显示区
        this.$chatBoxContent = (type) => {
            return window.RBChatSmallWindowUI && type != 'main' ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane') : $('#im-panel-main-chatcontentpane');
        }
        // 聊天内容输入框
        this.$inputMessage = () => {
            if (window._chatType == 'chat') return $('.im-panel-inputcontent');
            return window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('.im-panel-inputcontent') : $('.im-panel-inputcontent');
        }
        // 文本消息的发送按钮
        this.$sendMessageButton = () => {
            return window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-sendbutton') : $('#im-panel-sendbutton');
        }
        // 
        this.$chatBoxAllContent = (type) => {
            return window.RBChatSmallWindowUI && type != 'main' ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-toplevel') : $('#im-panel-main-chatcontentpane-toplevel');
        }

        // 消息队列，用于重新发送
        this.sendMsgRetryQueue = []
        // 消息重新发送定时器
        this.sendMsgRetryTimer = null
        // 图片预加载队列
        this.preloadImgQueue = []
        // 图片预加载正在下载数
        this.preloadingImgNums = 0
    ...
    UIModule5.prototype.initEventProcess = function () {
        ...
        this.$inputMessage().on('input propertychange', function (e) {
            const temp = $(this).val();
            const _windowEl = $(e.target).closest('.small-window-box');
            if (_windowEl.length) { // 保存草稿
                const _id = $(_windowEl).attr('id').split('_')[1]
                RBChatSmallWindowUI.updateWindowAttr(_id, { draft: temp })
            }
            // 群组增加@引用功能
        ...
        this.$inputMessage().on('paste', function (e) {
            ...
            if (_windowEl.length) {
                sendUser = {
                    ...
                    currentSelectedAlarmDataId: _windowEl.attr('id')?.split('_')[1]
                }
            }
            //是文本，判断是否是消息内图片复制
            if (clipdata.types && clipdata.types.length > 0 && clipdata.types[0].indexOf('text') >= 0) {
                ...
                if (text.indexOf('fingerPrintOfProtocal') >= 0 && text.indexOf('fingerPrintOfParent') >= 0) {
                    ...
                    RBChatDialogHelper.showImageSendDialog(dialogId, 0, img_obj, function () {
                        that.doSendImageMessage4IM(img_obj.text, sendUser);
                    })
                    return false;
                }
            }
            if (clipdata.items && clipdata.items.length > 0) {
                ...
                if (item.kind == 'file' && item.type.match(/^image\//i)) {
                    ...
                    RBChatDialogHelper.showImageSendDialog(dialogId, 1, item, function (base64Str) {
                        that.uploadBase64Str(base64Str, true, sendUser)
                    })
                    return true;
                }
            }
        ...
        const _els = this.$chatBoxAllContent()
        // 阻止浏览器默认行为，允许拖放
        _els.on('dragover', (event) => {
          event.preventDefault(); // 阻止浏览器默认行为
        //   _els.classList.add('dragover'); // 拖拽时更改样式
        });
    
        _els.on('drop', function (e) {
            e.preventDefault()
            const files = event.dataTransfer.files;
            if (!files.length) return

            const images_ext = ['jpg', 'png', 'jpeg']
            const video_ext = ['mp4']
            let imageList = [], videoList = [], fileList = []
            let fileTypes = 0
            for (let i = 0; i < files.length; i++) {
                const file = files[i]
                let fileExt = file.name.substr(file.name.lastIndexOf(".") + 1);
                if (images_ext.indexOf(fileExt.toLowerCase()) != -1) {
                    if (!imageList.length) fileTypes ++
                    imageList.push(file)
                } else if (video_ext.indexOf(fileExt.toLowerCase()) != -1) {
                    if (!videoList.length) fileTypes ++
                    videoList.push(file)
                }  else {
                    if (!fileList.length) fileTypes ++
                    fileList.push(file)
                }
            }
            if (fileTypes > 1) {
                RBChatDialogHelper.showAlertDialog_INFO('提示', '请选择同类型的文件!');
                return
            }
            let sendUser = {
                currentSelectedAlarmDataId: 'main'
            }
            const _windowEl = $(e.target).closest('.small-window-box')
            if (_windowEl.length) {
                sendUser = {
            ...
            if (imageList.length) { // 批量上传图片
                for (let i = 0; i < imageList.length; i++) {
                    const file = imageList[0]
                    let fileExt = file.name.substr(file.name.lastIndexOf(".") + 1);
                    fileExt = fileExt.toLowerCase()
                    if (!(fileExt == 'jpg' || fileExt == 'png' || fileExt == 'jpeg')) {
                        return alert('图片仅支持jpg、png、jpeg格式')
                    }

                    //大小限制判断
                    if (file.size > 10 * 1024 * 1024) {
                        return alert('图片大小限制为' + RBChatConfig.UPLOAD_AVATAR_IMAGE_DATA_MAX_LENGTH)
                    }
                }
                var resultList = []
                that.circle_update_file(imageList.length, resultList, imageList, function () {
                    // 上传成功
                    if (resultList.length > 0) {
                        RBChatDialogHelper.showMulImageSendDialog(resultList, true, false, sendUser)
                    } else {
                        alert('上传图片失败')
                    }
                }, sendUser?.currentSelectedAlarmDataId)
            } else if(videoList.length) { // 批量上传视频
                var resultList = []
                that.circle_update_file(videoList.length, resultList, videoList, function () {
                    // 上传成功
                    if (resultList.length > 0) {
                        RBChatDialogHelper.showMulImageSendDialog(resultList, false, false, sendUser)
                    } else {
                        alert('上传视频失败')
                    }
                }, sendUser?.currentSelectedAlarmDataId)
                ...
                if (fileList.length > 1) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '最多同时上传一个文件!');
                    return
                }
                const file = fileList[0]
                if (file.size > 500 * 1024 * 1024) {
                    return alert('文件文件大小限制为' + RBChatConfig.SEND_FILE_DATA_MAX_LENGTH)
                }
                var resultList = []
                
                that.upload_(file, false, false, sendUser)
            }
        })
        ...
        this.$inputMessage().keydown(function (event) {
            ...
            if (event.which === 13) {
                ...
                // 仅按Enter时发送
                    ...
                    if (!RBChatSmallWindowUI.getWindowAttr(event.target)) {
        ...
        // 鼠标点击事件
        this.$sendMessageButton().click(function (e) {
            if ($(e.target).closest('.small-window-box').length) {
                window._chatType = 'window'
            } else {
                window._chatType = 'chat'
            }
            console.log($(e.target).closest('.small-window-box').length, 213123123)
            that.doSendTextMessage4IM();
        });

        // 聊天框-滚动监听
        const _el = window.RBChatSmallWindowUI ? 
        RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap') :
        $('#im-panel-main-chatcontentpane-wrap')
        _el.scroll(function () {
            const s = window.scrollInfo;
            if (s) {
                console.log($(this).scrollTop(), s.top,123123123)
                ...
                const _bottomEl = RBChatSmallWindowUI.getWindowDom('.im-panel-main-toBottom-content', null, s.beyongDataId)
                if ($(this).scrollTop() <= 300) {
                    const top = s.top;
                    if (top >= 0) {
                        //往上拉，代表有数据
                        // var temp_list = list.slice(top - RBChatConfig.LOAD_CHAT_RECORDER_COUNTS, top)
                        window.scrollInfo.top = -1
                        // 获取到 
                        const t = $('#im-panel-main-chatcontentpane').children().eq(0);
                        const { beyongDataId, isGroupChatting } = s
                        function getContainerScrollDistanceFromBottom(container) {
                            // 获取容器的总高度（包括不可见部分）
                        ...
                        function setScrollDistanceFromBottom(containers, distance) {
                            const container = containers[0]
                            const scrollHeight = container.scrollHeight;  // 容器的总高度
                            const clientHeight = container.clientHeight;  // 容器的可视区域高度
                            // 计算目标滚动位置，滚动条离底部设置为指定距离
                            const targetScrollTop = scrollHeight - clientHeight - distance;
                            // 设置容器的滚动位置
                            container.scrollTop = targetScrollTop;
                        }
                        const lastScrollTop = getContainerScrollDistanceFromBottom(RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap')[0])
                        if (!list?.length) {
                            window.scrollInfo.top = 0
                            return
                        }
                        const _minData = list.reduce((min, item) => {
                            return item.date < min.date ? item : min;
                        });
                        const _minDate = _minData.date
                        RBChatRestHelper.queryChattingHistoryFromServer(
                            ...
                            , function (returnValue) {
                                ...
                                const chatHistoryDatas = that.dealMsgFromServer(chattingHistoryList, s)
                                RBChatChattingContentPaneUI.insertChatItems(chatHistoryDatas, beyongDataId, true, false);
                                setTimeout(() => {
                                    setScrollDistanceFromBottom(RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap'), lastScrollTop)
                                    window.scrollInfo.top = chatHistoryDatas.length < 30 ? -1 : 0
                                })
                            }
                            // 数据加载失败后的回调
                        ...
                        // RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap').animate({ scrollTop: t.offset().top }, 100);
                        ...
                        // 获取到 
                        const t = $('#im-panel-main-chatcontentpane').children().eq(0);
                        RBChatChattingContentPaneUI.insertChatItems(temp_list.reverse(), beyongDataId, true, false);
                        RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap').animate({ scrollTop: t.offset().top }, 100);
                    }
                } else if (RBChatAlarmsUI.isScrollBottom(s.beyongDataId)) {
                    if (_bottomEl.length) _bottomEl.css('display', 'none')
                    window._unReadNum = 0
                    RBChatAlarmsUI.updateUnReadUI(s.beyongDataId)
                } else {
                    if (_bottomEl.length) _bottomEl.css('display', 'flex')
                }
            }
        })
    };
    ...
    UIModule5.prototype.initShotcutWords = function () {
        ...
        var onlineMessageInputObj = this.$inputMessage();//RBChatSmallWindowUI.getWindowDom('.im-panel-inputcontent');
    ...
    // 图片预加载
    UIModule5.prototype.preloadImg = function() {
        if (this.preloadingImgNums > 5 || !this.preloadImgQueue.length) return
        this.preloadingImgNums++
        const url = this.preloadImgQueue.shift()
        ...
        img.onload = function() {
            cb()
            console.log('ttt 预加载 url 图片已缓存:', url);
        };
        img.onerror = function() {
            cb()
            console.log('ttt 预加载 url 图片加载失败:', url);
        };
    }
     ...
     * @param msg 消息体
     */
    UIModule5.prototype.addPreloadImgQueue = function(msg) {
        const _selectecAlarmDataId = RBChatMainUI.getCurrentSelectedAlarmDataId();
        let _urls = []
        try {
            const _msg = JSON.parse(msg.dataContent)
            // 当前会话不加入队列
            if (_selectecAlarmDataId == _msg.t || _selectecAlarmDataId == msg.from) return
            const _m = JSON.parse(_msg.m)
            if (_msg.ty == 13 && _m.files.length) {
                _urls = _m.files.filter(item => item.fileLength < 5 * 1024 * 1024).map(item => RBChatUtils.getImageDownloadURL(item.fileMd5, false))
            }
        } catch (err) {}
        if (!_urls.length) return
        this.preloadImgQueue = this.preloadImgQueue.concat(_urls)
        this.preloadImg()
    }

    // 上传文件
    UIModule5.prototype.uploadBase64Str = function (base64Str, isUsedForImageMsg, sendUsers) {
        ...
        RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').css('display', 'block');
        RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').text(logTag + '上传中 0%');
        ...
        RBChatUtils.uploadFile(file, function (data) {
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').text(logTag + "上传中 " + (data.percent * 100).toFixed(0) + "%");
        }, function (errorMsg) {
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').css('display', 'none');
            ...
            if (isUsedForImageMsg) { // 图片消息的发送
                // test单图消息
                that.calculate_image_size(file,function(dr){
                    const msgBody = {
                        text:'',
                        files:[{'fileLength':0,'fileMd5':objFromServer.fileMd5,'fileName':objFromServer.fileMd5, ...dr}],
                        type: '0',
                       
                    }
                    var fingerPrint = MBProtocalFactory.genFingerPrint();
                    // 向群内所有人发送此次公告内容
                    that.doSendMessageImpl(JSON.stringify(msgBody), 13, function () {

                    }, fingerPrint, sendUser);
                })
    ...
    UIModule5.prototype.upload_ = function (file, isUsedForImageMsg, isVideo = false, sendUsers, retryTimes = 0) {
        if (retryTimes >= 3) {
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').css('display', 'none');
            alert('上传失败')
            return
        ...
        RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, sendUsers?.currentSelectedAlarmDataId).css('display', 'block');
        RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, sendUsers?.currentSelectedAlarmDataId).text(logTag + '上传中 0%');
        // 当前选择得用户
        var sendUser = { currentSelectedAlarmType: RBChatMainUI.getCurrentSelectedAlarmType(), currentSelectedAlarmDataId: RBChatMainUI.getCurrentSelectedAlarmDataId() };
        if (sendUsers?.currentSelectedAlarmType) {
            sendUser = sendUsers
        }
        $("input[type='file'").val(null)
        /**
         ...
         */
        RBChatUtils.uploadFile(file, function (data) {
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, sendUsers?.currentSelectedAlarmDataId).text(logTag + "上传中 " + (data.percent * 100).toFixed(0) + "%");
        }, function (errorMsg) {
            that.upload_(file, isUsedForImageMsg, isVideo, sendUsers, ++retryTimes)
            // RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, sendUsers?.currentSelectedAlarmDataId).css('display', 'none');
            //alert(logTag+'上传出错，'+errorMsg);
            // RBChatDialogHelper.showAlertDialog_WARN('上传出错', logTag + '上传出错，' + errorMsg);
            // 关闭文件上传提示UI的显示（之所有用延迟关闭，是因为uploadifive控件的设计时，onError调用后，会接着调用onUpload函数，
            // 这样的话，就会导致错误提示显示完成后，UI上确还显示着文件正在上传的信息，这就不合适了！）
    ...
    UIModule5.prototype.initFileUplodifive5 = function (usedFor) {
        ...
        var $imageMsgSendBtnObj = RBChatSmallWindowUI.getWindowDom('#upload-image-input');
        var $fileMsgSendBtnObj = RBChatSmallWindowUI.getWindowDom('#upload-file-input');
        ...
        //实现拖文件上传
        dragEvent(RBChatSmallWindowUI.getWindowDom('#kchat-im-panel-main-chat-textarea'))
        $destSendBtnObj.unbind("change");
        $destSendBtnObj.on('change', function (e) {
            const _windowEl = $(e.target).closest('.small-window-box')
            let sendUser = {
                currentSelectedAlarmDataId: 'main'
            }
            if (_windowEl.length) {
                sendUser.currentSelectedAlarmDataId = _windowEl.attr('id')?.split('_')[1]
                sendUser.currentSelectedAlarmType = _windowEl.attr('msgType')
            }
            // 文件判断
            ...
            // 上传文件
            that.upload_(file, isUsedForImageMsg, false, sendUser)
        });
        RBChatSmallWindowUI.getWindowDom('#upload-video-input').unbind("change");
        //上传视频
        RBChatSmallWindowUI.getWindowDom('#upload-video-input').on('change', function () {
            // 文件判断
            ...
            const file = files[0];
            // 上传文件
            that.upload_(file, isUsedForImageMsg, true)
        });

        RBChatSmallWindowUI.getWindowDom('#upload-image-mul-input').unbind("change");
        //上传多张图片
        RBChatSmallWindowUI.getWindowDom('#upload-image-mul-input').on('change', function (e) {
            const sendUsers = RBChatSmallWindowUI.getWindowAttr(e.target)
            // 文件判断
            var files = RBChatSmallWindowUI.getWindowDom('#upload-image-mul-input', null, sendUsers?.currentSelectedAlarmDataId || 'main')[0].files;
            const files_ext = ['jpg', 'png', 'jpeg', 'gif', 'webp']
            // 获取图片
            var img_files = []
            for (file of files) {
            ...
            if (img_files.length > 0) {
                ...
                that.circle_update_file(img_files.length, resultList, img_files, function () {
                    // 上传成功
                    if (resultList.length > 0) {
                        RBChatDialogHelper.showMulImageSendDialog(resultList, true, false, sendUsers)
                    } else {
                        alert('上传图片失败')
                    }
                }, sendUsers?.currentSelectedAlarmDataId)
        ...
        RBChatSmallWindowUI.getWindowDom('#upload-mul-video-input').unbind("change");
        //上传多个视频
        RBChatSmallWindowUI.getWindowDom('#upload-mul-video-input').on('change', function (e) {
            const sendUsers = RBChatSmallWindowUI.getWindowAttr(e.target)
            // 文件判断
            var files = RBChatSmallWindowUI.getWindowDom('#upload-mul-video-input', null, sendUsers?.currentSelectedAlarmDataId || 'main')[0].files;
            const files_ext = ['mp4']
            // 获取图片
            var img_files = []
            for (file of files) {
            ...
            if (img_files.length > 0) {
                ...
                that.circle_update_file(img_files.length, resultList, img_files, function () {
                    ...
                    // 上传成功
                    if (resultList.length > 0) {
                        RBChatDialogHelper.showMulImageSendDialog(resultList, false, false, sendUsers)
                    } else {
                        alert('上传视频失败')
                    }
                }, sendUsers?.currentSelectedAlarmDataId)
        ...
        //红包发送
        RBChatSmallWindowUI.getWindowDom('#send-redpag-pack').unbind("click");
        const _groupInfo = window.groupInfo
        RBChatSmallWindowUI.getWindowDom('#send-redpag-pack').click(function (e) {
                            ...
                            RBChatRestHelper.queryWalletIdFromServer(obj.data
                                , function (returnValue) {
                                    ...
                                    if (res.data) {
                                        ...
                                        if (obj.isAutoSend) {
                                            // 获取触发自动发送红包时的群ID
                                            const currentSelectedAlarmDataId = RBChatMainUI.getCurrentSelectedAlarmDataId();
                                            const params = {
                                                times: obj.times,
                                                interval: obj.interval,
                                                redParams: obj.redParams,
                                                currentSelectedAlarmDataId,
                                                id: Date.now(),
                                                time: Date.now()
                                            }
                                            // 更新定时到缓存
                                            const _oldCache = JSON.parse(sessionStorage.getItem('autoSendRed') || '[]')
                                            _oldCache.push(params)
                                            sessionStorage.setItem('autoSendRed', JSON.stringify(_oldCache))
                                            that.autoSendRed(params)
                                        }
                                        // 发送消息
                                        that.doSendMessageImpl(JSON.stringify(s_obj), 10, function () {
                                            // 清空输入框
                                            $(that.$inputMessage()[0]).val('');
                                        }, fingerPrint);
                                    }
                                }
                                , function (errorThrownStr) {
            ...
            const _info = RBChatSmallWindowUI.getWindowAttr(e.target, 'info')
            const numPeo = _info?.g_member_count  || window.groupInfo.g_member_count;
            ...
            // RBChatUtils.showIFrameURL("https://chat.imnono.net/coupon/givRedEnv.html?pfont=45px&numPeo=" + numPeo + "&userId=" + localUserUid+'&h='+headPhoto, '发送红包', callBack)
            RBChatUtils.showIFrameURL(RBChatConfig.COUPON_IFRAME_URL + "/coupon/givRedEnv.html?pfont=45px&numPeo=" + numPeo + "&userId=" + localUserUid+'&h='+headPhoto+'&chatType=1', '发送红包', callBack)
        })

        //红包发送
        RBChatSmallWindowUI.getWindowDom('#send-redpag-pack-friend').unbind("click");
        RBChatSmallWindowUI.getWindowDom('#send-redpag-pack-friend').click(function (e) {
            var callBack = function (e) {
            ...
            var headPhoto = LocalUserInfo.getObj().userAvatarFileName && LocalUserInfo.getObj().userAvatarFileName.length > 0? LocalUserInfo.getObj().userAvatarFileName:'';
            // RBChatUtils.showIFrameURL("https://chat.imnono.net/coupon/givRedEnv.html?pfont=45px&numPeo=" + numPeo + "&userId=" + localUserUid+'&h='+headPhoto, '发送红包', callBack)
            RBChatUtils.showIFrameURL(RBChatConfig.COUPON_IFRAME_URL + "/coupon/givRedEnv.html?pfont=45px&numPeo=" + 1 + "&userId=" + localUserUid+'&h='+headPhoto + '&chatType=2', '发送红包', callBack)
        })
    };
     ...
     * @param {*} uploadFiles 
     */
    UIModule5.prototype.circle_update_file = function (count, resultList, uploadFiles, callBack, dataId, retryTimes = 0) {
        const that = this;
        if (retryTimes >= 3) { // 重试超过3次，取消
            $("input[type='file'").val(null)
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, dataId).hide();
        ...
        if (retryTimes == 2) debugger
        if (uploadFiles.length > 0) {
            var file = uploadFiles[0]
            uploadFiles.splice(0, 1)
            RBChatUtils.uploadFile(file, function (data) {
                if (dataId == 'fastSend') {
                    $('.fast-send-loading').show()
                    $('.fast-send-loading').text("上传中[" + (count - uploadFiles.length) + "/" + count + "] - " + (data.percent * 100).toFixed(0) + "%")
                } else {
                    RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, dataId).show();
                    RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint', null, dataId).text("上传中[" + (count - uploadFiles.length) + "/" + count + "] - " + (data.percent * 100).toFixed(0) + "%");
                }
            }, function (errorMsg) {
                uploadFiles.unshift(file)
                that.circle_update_file(count, resultList, uploadFiles, callBack, dataId, ++retryTimes)
            }, function (data) {
                const files_ext = ['jpg', 'png', 'jpeg', 'gif', 'webp']
                // 获取图片
                let fileExt = file.name.substr(file.name.lastIndexOf(".") + 1);
                if (files_ext.indexOf(fileExt.toLowerCase()) != -1) {
                    that.calculate_image_size(file,function(rl){
                        resultList.push({...data,...rl})
                        that.circle_update_file(count, resultList, uploadFiles, callBack, dataId)
                    });
                }else{
    ...
    // 计算图片的宽和高
    UIModule5.prototype.calculate_image_size_2 = function(url,callBack){
        var img = new Image();
        img.onload = function() {
    ...
    // 计算图片的宽和高
    UIModule5.prototype.calculate_image_size = function(file,callBack){
         // 创建FileReader对象
    ...
    UIModule5.prototype.initFileUplodifive5_groupMsgSend = function () {
        ...
        var $imageMsgSendBtnObj = RBChatSmallWindowUI.getWindowDom('#upload-image-input-2');
        var $fileMsgSendBtnObj = $RBChatSmallWindowUI.getWindowDom('#upload-file-input-2');
        ...
        //实现拖文件上传
        dragEvent(RBChatSmallWindowUI.getWindowDom('#kchat-im-panel-main-chat-textarea-2'))
        // 图片上传
        ...
        $fileMsgSendBtnObj.on('change', function (e) {
            // 文件判断
        ...
        // 视频上传
        RBChatSmallWindowUI.getWindowDom('#upload-video-input-2').unbind("change");
        RBChatSmallWindowUI.getWindowDom('#upload-video-input-2').on('change', function () {
            // 文件判断
        ...
        // 输入框 post 事件
        RBChatSmallWindowUI.getWindowDom('#im-panel-inputcontent-2').on('paste', function (e) {
            console.log(e, 123124155, 2)
            const clipdata = e.originalEvent.clipboardData;
            //是文本，判断是否是消息内图片复制
            if (clipdata.types && clipdata.types.length > 0 && clipdata.types[0].indexOf('text') >= 0) {
                const text = clipdata.getData("text/plain");
    ...
    UIModule5.prototype.refreshForFileUploadHint = function (hint) {
        var fileUploadHintUIObj = RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint');
     ...
     * 初始化表情相关的事件处理。
     */
    UIModule5.prototype.initEmoji = function (dataId) {
        var that = this;
        $.get("/images/face/face_data.json", function (data) {
            ...
            that.refreshFaceBoard(data, dataId);
        });
    };
  ...
  */
    UIModule5.prototype.refreshFaceBoard2 = function (faceDatas) {
        var emojiPopObj = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_emojipopup-2'); // 表情面板 div
        var emojiOpenBtnObj = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_emojiopenbtn-2'); //表情开关按钮 i
        var faceGroupsHtml = "";
        ...
        $(".emoji_li").click(function () {
            console.log(2141241244)
            var inputObj = RBChatSmallWindowUI.getWindowDom('#im-panel-inputcontent-2');
     ...
     * @param {*} key 
     */
    UIModule5.prototype.show_plan_face_ui = function(key='', dataId){
        // 表情
        if(key == '' || key == 'emoji'){
            this.show_emoji_ui(dataId)
        // 收藏
        }else if(key == 'love'){
    ...
    UIModule5.prototype.init_face_nav_ui = function(){
       var that = this;
       RBChatSmallWindowUI.getWindowDom('#emoji-nav').empty()
       RBChatSmallWindowUI.getWindowDom('#emoji-nav').append('<div class="item item-active" key="emoji"><img src="images/face.svg"/></div>')
       var nav_face_list = window.bg_face_list
       nav_face_list.forEach(item=>{
        RBChatSmallWindowUI.getWindowDom('#emoji-nav').append('<div class="item" key="nav-'+item.id+'"><img src="'+item.key+'"/></div>')
       })
       RBChatSmallWindowUI.getWindowDom('#emoji-nav').append('<div class="item" id="nav-love" key="love"><img src="images/love-e.svg"/></div> ')


       RBChatSmallWindowUI.getWindowDom('#emoji-nav div').unbind('click')
       RBChatSmallWindowUI.getWindowDom('#emoji-nav div').click(function(){
            RBChatSmallWindowUI.getWindowDom('#emoji-nav div').removeClass('item-active')
            $(this).addClass('item-active')
            const key = $(this).attr('key')
            that.switch_collecting_ui(key == 'love' ? 'hide' : 'all_hide')
            that.switch_collection_checkbox_ui('hide')
            that.show_plan_face_ui(key)
       })
       this.init_collection_ops()
    }
     ...
     * 初始化 批量删除、删除、取消事件
     */
    UIModule5.prototype.init_collection_ops = function(){
        const that = this
        const _btn = RBChatSmallWindowUI.getWindowDom('.emoji-operation .batch-delete');
        const _btn2 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .delete');
        const _btn3 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .cancel');
        _btn.unbind('click')
        _btn.click(function(){
            that.switch_collecting_ui('show')
        })
        _btn2.unbind('click')
        _btn2.click(function(){
            // console.log
            //  that.switch_collection_checkbox_ui('show')
            const _els = that.get_select_collection_items()
            let successNums = 0
            _els.each((index, elements) => {
                const _id = $(elements).attr('id')
                //查询收藏
                RBChatRestHelper.delete_love_face(_id, function (res) {
                    successNums++
                    if (window._deleteLoves) {
                        window._deleteLoves.push(_id)
                    } else {
                        window._deleteLoves = [_id]
                    }
                    if (successNums >= _els.length) {
                        that.switch_collecting_ui('hide')
                        RBChatDialogHelper.showAlertDialog_INFO('提示', '删除成功');
                        //查询收藏
                        RBChatRestHelper.query_love_face(function (res) {
                            window.love_face_list = JSON.parse(res).filter(item => !(window._deleteLoves || []).includes(item[0]))
                            // 重新绘制界面
     ...
     * 收藏-批量删除ui-显示切换
     */
    UIModule5.prototype.switch_collecting_ui = function(type){
        const _btn = RBChatSmallWindowUI.getWindowDom('.emoji-operation .batch-delete');
        const _btn2 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .delete');
        const _btn3 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .cancel');
        const _btn4 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .select-nums');
        window._curCollectionType = type
        if (type == 'show') {
            _btn.css('display', 'none')
            _btn2.css('display', 'block')
            _btn3.css('display', 'block')
            _btn4.css('display', 'block')
            this.update_collection_select_nums(0)
            this.switch_collection_checkbox_ui('show')
        } else if (type == 'hide') {
            _btn.css('display', 'block')
            _btn2.css('display', 'none')
            _btn3.css('display', 'none')
            _btn4.css('display', 'none')
        } else {
            _btn.css('display', 'none')
            _btn2.css('display', 'none')
            _btn3.css('display', 'none')
     ...
     * 收藏-更新选中个数
     */
    UIModule5.prototype.update_collection_select_nums = function(num){
        const _btn4 = RBChatSmallWindowUI.getWindowDom('.emoji-operation .select-nums');
        _btn4.html(`选中${num || 0}个`)
    }

    /**
     * 收藏-显示批量删除勾选框
     */
    UIModule5.prototype.switch_collection_checkbox_ui = function(type){
        const that = this
        const _checkboxs = RBChatSmallWindowUI.getWindowDom('.face_love_checkbox');
        if (type == 'show') {
            _checkboxs.each(function(index, element) {
            ...
            _checkboxs.each(function(index, element) {
                ...
                $(element).css('display', 'none')
                that.update_checkbox_ui($(element), 'hide')
            });
        }
    }
     ...
     * 收藏-获取当前选中项
     */
    UIModule5.prototype.get_select_collection_items = function(){
        return RBChatSmallWindowUI.getWindowDom('.face_love_checkbox.weui-icon-success');
    }
     ...
     * 收藏-切换checkboxui
     */
    UIModule5.prototype.update_checkbox_ui = function(el, type) {
        if (type == 'hide') {
            if (el.hasClass('weui-icon-success')) {
                el.removeClass('weui-icon-success').addClass('weui-icon-circle');
            }
            return
        }
        if (el.hasClass('weui-icon-circle')) {
            el.removeClass('weui-icon-circle').addClass('weui-icon-success');
        } else if (el.hasClass('weui-icon-success')) {
            el.removeClass('weui-icon-success').addClass('weui-icon-circle');
        }
    }
    ...
    UIModule5.prototype.show_bg_face_love_ui = function(key){
        var love_emojiPopObj = RBChatSmallWindowUI.getWindowDom('.emoji-content'); //表情开关按钮 i
        love_emojiPopObj.empty();
        var that = this;
        var love_pop = RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_love_emojipopup'); //表情开关按钮 i
        // 显示其他表情列表
        var show_other_face_love = function(list){
            ...
            love_emojiPopObj.append(faceGroupsHtml); //加入各表情组内容
             ...
             // 图标绑定事件
            RBChatSmallWindowUI.getWindowDom('.love-face-img').unbind('click')
            RBChatSmallWindowUI.getWindowDom('.love-face-img').click(function () {
                //发送图片消息
    ...
    UIModule5.prototype.show_face_love_ui = function () {
        ...
        love_list.forEach(item => {
            ...
            faceGroupsHtml += "<li><i class='face_love_checkbox weui-icon-circle' id='" + item[0] +"'></i><img src='" +img_url + "' class='love-face-img' dkey='" + img_url + "'  bid='" + item[0] + "'/></li>"
        })
        faceGroupsHtml += "</ul>";
        love_emojiPopObj.append(faceGroupsHtml); //加入各表情组内容

        // 上传文件
        var upload_ = function (file, isUsedForImageMsg, isVideo = false, retryTimes = 0) {
            if (retryTimes >= 3) {
                RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_fileuphint').css('display', 'none');
                alert('上传失败')
                return
            ...
            RBChatUtils.uploadFile(file, function (data) {
                ...
                upload_(file, isUsedForImageMsg, isVideo, ++retryTimes)
                //alert(logTag+'上传出错，'+errorMsg);
                // RBChatDialogHelper.showAlertDialog_WARN('上传出错', logTag + '上传出错，' + errorMsg);
            }, function (data) {
                // 服务端在文件上传完成后返回的JSON对象，请务必与服务端的文件上传接口中返回的参数保持一致（详见服务端：FileUploader4Web.java）！
        ...
        RBChatSmallWindowUI.getWindowDom('#upload-love-face-file-input').unbind("change");
        //上传图片
        RBChatSmallWindowUI.getWindowDom('#upload-love-face-file-input').on('change', function () {
            // 文件判断
        ...
        RBChatSmallWindowUI.getWindowDom('.love-face-img').unbind('click')
        RBChatSmallWindowUI.getWindowDom('.love-face-img').click(function () {
            if (window._curCollectionType == 'show') {
                const _id = $(this).attr('bid')
                that.update_checkbox_ui($(`[id=${_id}].face_love_checkbox`))
                that.update_collection_select_nums(that.get_select_collection_items().length)
                return
            }
            //发送图片消息
            if (RBChatMainUI.getCurrentSelectedAlarmDataId()) {
                var imgUrl =$(this).attr('dkey'); 
                that.calculate_image_size_2(imgUrl,function(dr){
                    var f = imgUrl.split('/message/')[1]
                    const msgBody = {
                        text:'',
                        files:[{'fileLength':0,'fileMd5':f,'fileName':f, ...dr}],
                        type: '0',
        ...
        // 图标绑定事件
        RBChatSmallWindowUI.getWindowDom('.face_love_checkbox').unbind('click')
        RBChatSmallWindowUI.getWindowDom('.face_love_checkbox').click(function (e) {
            e.stopPropagation()
            that.update_checkbox_ui($(this))
            that.update_collection_select_nums(that.get_select_collection_items().length)
        })

        RBChatSmallWindowUI.getWindowDom('.love-face-img').unbind('contextmenu')
        RBChatSmallWindowUI.getWindowDom('.love-face-img').bind('contextmenu', function (e) {
            if (window._curCollectionType == 'show') return
            // 右键菜单主div层
     ...
     * 初始emoji图标列表
     */
    UIModule5.prototype.show_emoji_ui = function(dataId){
        const that = this;
        var love_emojiPopObj = RBChatSmallWindowUI.getWindowDom('.emoji-content', 'parent', dataId); //表情开关按钮 i
        love_emojiPopObj.empty();
        var faceGroupsHtml = "";
        faceGroupsHtml += "<ul id='im-panel-main-chat-face-emoji-love'>";
        var love_pop = RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_love_emojipopup', 'parent', dataId); //表情开关按钮 i
        ...
        love_emojiPopObj.append(faceGroupsHtml); //加入各表情组内容
         ...
         // 为每一个表情增加点击事件
         console.log(love_emojiPopObj, dataId, 21412512525)
         RBChatSmallWindowUI.getWindowDom('.emoji_li', 'parent', dataId).click(function () {
            var inputObj = RBChatSmallWindowUI.getWindowDom('.im-panel-inputcontent', 'parent', dataId);
            console.log(inputObj, 12124213412314)
            // inputObj.val(inputObj.val() + $(this).attr('desc')); // 此方式只会追加在末尾，不能插入到当前交点处
            // RBChatUtils.setTextFocus(inputObj); // 获得焦点，方便接着输入其它内容 // 该方法会将光标设置到末尾
            var pos = RBChatUtils.insertAtCursor(inputObj, $(this).attr('desc')); //插入emoji表情并返回最新输入的位置(emoji表情后)
            RBChatUtils.setCursorPos(inputObj, pos); // 设置光标到最新输入的位置(emoji表情后)
            love_pop.hide();
            if (window._chatType == 'window' && window._chatId) {
                RBChatSmallWindowUI.updateWindowAttr(window._chatId, { draft: RBChatSmallWindowUI.getWindowDom('.im-panel-inputcontent', 'parent', dataId).val() })
            }
        });
    }
     ...
     * @author Freeman
     */
    UIModule5.prototype.refreshFaceBoard = function (data, dataId) {
        var love_emojiOpenBtnObj = window.RBChatSmallWindowUI ? 
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_love_emojiopenbtn', 'parent', dataId) :
            $('.im-panel-main-chat-textarea_love_emojiopenbtn') // 表情面板 div
        console.log(love_emojiOpenBtnObj, dataId, 215126666)
            var love_emojiPopObj = window.RBChatSmallWindowUI ? 
            RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_love_emojipopup', 'parent', dataId) : 
            $('.im-panel-main-chat-textarea_love_emojipopup'); //表情开关按钮 i
        var that = this;
        love_emojiOpenBtnObj.unbind('click');
        love_emojiOpenBtnObj.click(function (e) {
            that.switch_collecting_ui('all_hide')
            RBChatSmallWindowUI.updateWindowChatType(e)
            love_emojiPopObj.toggle();
            that.init_face_nav_ui(dataId);
            that.show_plan_face_ui('', dataId);
        });
        ...
        document.body.onclick = function (e) {
            ...
            if(target.id.length > 0 && ids_list.indexOf(target.id) == -1){
                var love_emojiPopObj = RBChatSmallWindowUI.getWindowDom('.im-panel-main-chat-textarea_love_emojipopup', 'parent', dataId); //表情开关按钮 i
    ...
    /**
     * 初始化“发送合集”消息发送按钮的事件处理。
     */
    UIModule5.prototype.initBackBottom = function (e) {
        var btnObj = window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_fastSend') : $('#im-panel-main-chat-textarea_fastSend');
        var that = this;
        ...
        btnObj.click(function (e) {
            const _windowEl = $(e.target).closest('.small-window-box')
            let sendUser = {
                currentSelectedAlarmType: 'main' 
            }
            if (_windowEl.length) {
                sendUser.currentSelectedAlarmDataId = _windowEl.attr('id')?.split('_')[1]
                sendUser.currentSelectedAlarmType = _windowEl.attr('msgType')
            }
            if (!sendUser.currentSelectedAlarmDataId && !RBChatMainUI.getCurrentSelectedAlarmDataId()) {
                RBChatDialogHelper.showAlertDialog_INFO('提示', '请选择会话!');
                return
            } 
            const cb = () => {
                if (!IMSDK.isOnline()) {
                    RBChatDialogHelper.showAlertDialog_INFO('提示', '网络连接已断开，请稍后尝试!');
                    return
                }
                const _cache2 = JSON.parse(localStorage.getItem('fast-send-data') || '[]')
                const _cache = _cache2.filter(item => item.selected == 1)
                _cache.forEach(item => {
                    const fingerPrint = MBProtocalFactory.genFingerPrint()
                    let currentSelectedAlarmType = RBChatMainUI.getCurrentSelectedAlarmType()
                    let currentSelectedAlarmDataId = RBChatMainUI.getCurrentSelectedAlarmDataId()
                    if (window._chatType == 'window' && sendUser?.currentSelectedAlarmType != 'main') {
                        currentSelectedAlarmDataId = window._chatId
                        currentSelectedAlarmType = $(`#${RBChatSmallWindowUI.getWindowId(window._chatId)}`).attr('msgType')
                    }
                    ...
                    if (item.files && item.files?.length) {
                        params[1] = 13
                        const fileExt = item.files[0].fileName.substr(item.files[0].fileName.lastIndexOf(".") + 1)
                        params[0] = JSON.stringify({
                            text: params[0],
                            files: item.files,
                            type: images_ext.indexOf(fileExt.toLowerCase()) != -1 ? '0' : '1',
                        })
                    } else if (params[0]?.toLowerCase()?.substr(0, 4) == 'app:') { // 小程序
                        const minAppFlag = params[0].split(':')[1];
                        const obj = window.app_applets.find(item => item.appletId == minAppFlag || item.appletName == minAppFlag);
                        if (obj) {
                            params[0] = JSON.stringify(obj),
                            params[1] = 9
                        }
                    }
            
                    that.sendMsgRetryQueue.push({
                        fingerPrint,
                        currentSelectedAlarmType,
                        currentSelectedAlarmDataId,
                        arguments: params
                ...
                that.sendMsgRetry('init')
            }
            RBChatDialogHelper.showFastSendForm('send', cb)
        });
    ...
    /**
     * 初始化“消息撤回”消息发送按钮的事件处理。
     */
    UIModule5.prototype.initReCallEvent = function (e) {
        var btnObj = window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_reCall') : $('#im-panel-main-chat-textarea_reCall');
        var that = this;

        // 点击事件
        btnObj.click(function (e) {
            RBChatDialogHelper.showReCallDialog(window.RBChatMainUI.getCurrentSelectedAlarmDataId())
        });
     ...
     * 初始化回到底部按钮的事件处理。
     */
        UIModule5.prototype.initFastSend = function (e) {
            var btnObj = window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('.im-panel-main-toBottom-content') : $('.im-panel-main-toBottom-content');
            var that = this;
     ...
     * 初始化“名片”消息发送按钮的事件处理。
     */
    UIModule5.prototype.initContactMsgSend = function (e) {
        var btnObj = window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_contactmsgbtn') : $('#im-panel-main-chat-textarea_contactmsgbtn');
        var that = this;
        ...
        btnObj.click(function (e) {
            ...
            // 发出消息指令
            RBChatChattingContentPaneUI.doSendContactMessage4IM(localUserInfo.user_uid, localUserInfo.nickname, function () { }, sendUser);
        });
     ...
     /**
     * 初始化“反馈链接”消息发送按钮的事件处理。
     */
     UIModule5.prototype.initFeedbackLinkMsgSend = function () {
        var btnObj = window.RBChatSmallWindowUI ? RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-textarea_feedbackLinkmsgbtn') : $('#im-panel-main-chat-textarea_feedbackLinkmsgbtn');
        var that = this;
        ...
        btnObj.click(function (e) {
            ...
            var localUserInfo = LocalUserInfo.getObj();
            // 发出消息指令
            RBChatChattingContentPaneUI.doSendFeedbackLinkMessage4IM(localUserInfo.user_uid, localUserInfo.nickname, function () { }, sendUser);
        });
     ...
     */
    UIModule5.prototype.insertChatItems = async function (multiChatMsgEntity, beyongDataId, needPrepend, scrollBottom = true) {
        if (multiChatMsgEntity) {
            //标准的for循环：遍历 Array[ChatMsgEntity对象] 数组
            for (var i = 0; i < multiChatMsgEntity.length; i++) {
                this.insertChatItemWithP(multiChatMsgEntity[i], beyongDataId, needPrepend, scrollBottom);
            }
        }
        else {
     ...
     * @param needPrepend true表示插入被选元素的开头，否则插入被选元素的末尾
     */
    UIModule5.prototype.insertChatItemWithP = function (chatMsgEntity, beyongDataId, needPrepend, scrollBottom = true) {
        ...
        if (chatMsgEntity.text) {
            ...
            if (chatMsgEntity.msgType == MsgType.TYPE_SYSTEAM$INFO){ // 群聊系统指令的ui是单独的样式
                this.insertChatItemForSysteminfo(chatMsgEntity.text, timeToShow, needPrepend,chatMsgEntity, beyongDataId);
            }else if (chatMsgEntity.msgType == MsgType.TYPE_REVOKE)
                this.insertChatItemForRevoked(chatMsgEntity.text, timeToShow, needPrepend, beyongDataId);
            // 插入一条普通聊天消息显示到消息面板上
            else {
                // this.insertChatItem(
                //     chatMsgEntity.uid
                ...
                this.insertChatItem(chatMsgEntity, timeToShow, isme, needPrepend, scrollBottom, beyongDataId);
            }
        }
    };
    ...
    UIModule5.prototype.showRedPckUI = function (res, redInfo, entityData) {
        ...
        $('#redpck-row-4').click(function () {
            ...
            // RBChatUtils.showIFrameURL("https://chat.imnono.net/coupon/claimDetails-2.html??walletId=" + walletId + "&hideCash=1&userId=" + localUserUid+'&v='+Math.random(), '领取详情', callBack);
            RBChatUtils.showIFrameURL(RBChatConfig.COUPON_IFRAME_URL + "/coupon/claimDetails-2.html??walletId=" + walletId + "&hideCash=1&userId=" + localUserUid+'&v='+Math.random(), '领取详情', callBack);
        });
    ...
    UIModule5.prototype.insertChatItem = function (chatMsgEntity//uid, messageType, nickName
        ...
        , isMe, needPrepend, scrollBottom = true, beyongDataId, needHTML) {
        // 看片小程序
        if (chatMsgEntity.text?.startsWith('[视频小程序]')) {
        ...
        console.log(chatMsgEntity//uid, messageType, nickName
            , showTime
            //, messageContent, fp , parentFp
            , isMe, needPrepend, scrollBottom = true, beyongDataId, needHTML, 1212222221111)
        ...
        //else
        {
            ...
            if (messageType - 0 == 13) {
                ...
                contentHTML = "<div class='mul-img-send'><div class='row-1'>" + file_html + "</div>" +
                    (obj.text && obj.text.length > 0 ?"<div class='row-2 dsads'>" + obj.text + "</div>":"") +
                    "</div>"
            }
            // @消息
                ...
                try {
                    ...
                    select_obj.forEach(item => {
                        ...
                        origin_text = origin_text.replace(new RegExp(l, 'g'), 
                            `<font onclick='event.stopPropagation();RBChatDialogHelper.showUserInfoFromServer(false, null, ${item.user_uid}, null,true);' color='#28a0ff' style="cursor: pointer;">` + l + "</font>"
                        )   
                    })
            ...
            //小程序消息
                ...
                const { cover, fullUrl, title } =  RBChatUtils.parseVideoMiniApp(chatMsgEntity.text)
                if (fullUrl) {
                    const _id = `videoapp-${fp}`
                    RBChatUtils.decImage(cover).then(res => {
                        document.querySelector(`#${_id}`).src = res
                    })
                    contentHTML = "<div class='minapp-root' " + phoneCss + " onclick=\"javascript:minappJump('" + fullUrl + "','" + -1 + "')\">" +
                        "<div class='minapp-root-top'>" +
                        "<img id='" + _id + "' src='" + cover + "'/> <span>" + title + "</span>" + 
                        "</div>" +
                        "<div class='minapp-root-bottom'>" +
                        "[小程序]" + title +
                        "</div>" +
                ...
                if (red_obj) {
                    ...
                    let redText = (red_obj.status - 1 == 0 ? '待领取' : '已领取')
                    // 超过24小时的红包， 显示已过期
                    if (Date.now() - chatMsgEntity.date > 24 * 60 * 60 * 1000) {
                        redText = '已过期'
                    }
                    contentHTML = "<div class='redpck-root' " + phoneCss + "  id='" + red_obj.walletId + "_id_clck' time='" + chatMsgEntity.date + "'>" +
                        "<div class='redpck-top' id='" + red_obj.walletId + "_redpck_root'> <img src='/images/redpck.png'/> <div class='redpck-top-right'><span>" + this.replaceEmojiPlaceholderToHTML(red_obj.description) + "</span><p id='" + red_obj.walletId + "_status_text'>" + redText + "</p></div></div>" +
            ...
            // 回复消息
                ...
                var left_htmle = function (messageType, text) {
                    if (messageType == MsgType.TYPE_IMAGE) {
                        return "<img src='" + (text&& text.indexOf('http')!=-1 ? text: RBChatUtils.getImageDownloadURL(text, false)) + '?imageView2/2/w/150' + "'/>"
                    }
                    return ''
                }
                // 复制uiui
                ...
                window.imgSwiperListUI = function (url) {
                    ...
                    _list.sort((a, b) => Number(a.date) - Number(b.date));
                    _list.filter(item => item.msgType == MsgType.TYPE_IMAGE || item.msgType == 13).forEach(item => {
            ...
            // 大文件消息的显示逻辑处理
                ...
                contentHTML =
                    "<a href=\"" + httpDownloadURL + "\" class=\"chat-msg-file-pic\" target='_blank'>" +
                    "   <img src=\"" + imgUrl + "\" title='" + fileName + "'>" +
                    "</a>" +
                    "<div class=\"chat-msg-file-info\">" +
                    "   <h4 title='" + fileName + "'>" + fileName + "</h4>" +
                    "   <p><a href=\"" + httpDownloadURL + "\" target='_blank'><span>" + RBChatUtils.getConvenientFileSize(fileLength, 100) + "</span></a></p>" +
                    "</div>" +
                    // 自已发出的大文件消息就不需要显示这个“下载图标”了
                    (isMe ? "" :
                        "<div class=\"chat-msg-file-down\">" +
                        "   <a href=\"" + httpDownloadURL + "\" title='" + fileName + "' target='_blank'></a>" +
                        "</div>"
                    );
                ...
                if(fileMeta.isMovie){
                    ...
                    contentHTML = `
                        <a href="javascript:void(0)" onclick="javascript:videoSwiperListUI('${httpDownloadURL}');return false;">
                            ...
                            <span class="chat-msg-video-info">${durationString}</span>
                            <div 
                                onclick="event.stopPropagation(); event.preventDefault(); RBChatUtils.copyToClipboard('iOS专属链接：${httpDownloadURL}')" 
                    ...
                    contentHTML = `
                        <a href="javascript:void(0)" onclick="javascript:videoSwiperListUI('${httpDownloadURL}');return false;">
                            ...
                            <div 
                                onclick="event.stopPropagation(); event.preventDefault(); RBChatUtils.copyToClipboard('iOS专属链接：${httpDownloadURL}')" 
                                style="position: absolute; bottom: 5px; left: 8px; width:64px; height:25px; border:1px solid rgba(255,255,255,0.77); border-radius: 13px; background: rgba(28,28,28,0.50); font-size: 12px; color: #FFFFFF; font-weight: 600; display:flex; align-items:center; justify-content:center;">复制链接</div>
                        </a>
                    `;
            ...
            // 名片消息的显示逻辑处理
                ...
                contentHTML =
                    ...
                    "<div class=\"im-panel-header-user-avatar-default\"></div>" +
                    "    <img src=\"" + RBChatUtils.getUserAvatarDownloadURL(theUid, false) + "\" onerror=\"javascript:RBChatLocalUserUI.showUserLocalPic(this)\" data-uid=\"" + theUid + "\" data-name=\"" + theNickname + "\" title=\"点击查看[" + theNickname + "]的名片信息！\"/>" +
                    "</a>" +
            ...
            // 位置消息的显示逻辑处理
                ...
                // TODO: 如果是 专属反馈链接消息，单独处理
                const _url = RBChatUtils.replaceDomain(RBChatConfig.FEEDBACK_LINK, window._FEEDBACK_LINK)
                if (messageContent.includes(_url)) {
                    console.log({messageContent});
                    const linkMatch = messageContent.match(/(https?:\/\/[^\s]+)/);
                    const link = linkMatch ? linkMatch[0] : ''; // 确保 link 不为 null
                    const url = new URL(link);
                    url.host = document.domain
                    
                    contentHTML = `
                        ${messageContent}<span style="color: #28A0FE; cursor:pointer;" onclick="RBChatUtils.showIFrameURL('${url.toString()}&from=web')">查看</span>
                    `;
                } else {
            ...
            function daysBetween(date1, date2) {
                ...
                const firstTime = date1.getTime(); // 第一个时间的毫秒数
                const secondTime = date2.getTime(); // 第二个时间的毫秒数
            ...
            // 昵称颜色
            var name_color = '';
            if(chatMsgEntity.ulastlogin){
                var days = daysBetween(new Date(chatMsgEntity.ulastlogin),new Date());
                if(days > 0 && days> 3 && days<7){
                    name_color='style="color:rgb(227,207,87);"';
                }
                if(days > 0 && days>=7 && days<10){
                    name_color='style="color:blue;"';
                }
                if(days > 0 && days>= 10 && days<20){
                    name_color='style="color:#800080;"';
                }
                if(days > 0 && days>= 20){
            ...
            const heade_photo = chatMsgEntity.heade_photo && chatMsgEntity.heade_photo.length > 0 ? RBChatConfig.FILE_HTTPS_URL + '/head/'+chatMsgEntity.heade_photo:'';
            html =
                "<div  id=\'" + fp_id + "\'" + (parentFp ? " parentfp=\'" + parentFp + "\'" : "") + " class=\'chat-item\'>"
                + "    <div onclick='javascript:RBChatDialogHelper.showUserInfoFromServer(false, null, " + uid + ", null," + !isMe + ");' class=\'chat-item-avatar " + (isMe ? "chat-item-avatar-r" : "chat-item-avatar-l") + "\'>"
                + "    <div style='background:"+defaultColor+"'>" + (isMe ? '我': show_t) + " </div>"
                +   "      <img src=\'" + heade_photo + "\' onerror='javascript:$(this).remove()'>"
                + "    </div>"
                + "    <div class=\'" + (isMe ? 'chat-right' : 'chat-left') + "\'>"
                + "        <div class=\'chat-item-info\'>"
                + (isMe ? "" : level_html)
                + "          <span class=\'name\'"+name_color+"> " + nickName +"</span>" +(isMe ? '' : ' -')
                + "           <time>" + showTime + "</time>"
                + "        </div>"
        ...
        if (needHTML) return html

        // 无条件尝试清除空数据UI的显示（该UI只应在没有聊天数据时显示）
        this.clearChatPaneEmpty();
        let _contentEl
    // 添加到消息界面
        console.log(beyongDataId, RBChatSmallWindowUI.checkIsWindow(beyongDataId),_contentEl, 21312321312)
        if (RBChatSmallWindowUI.checkIsWindow(beyongDataId)) {
            _contentEl = $(`#${RBChatSmallWindowUI.getWindowId(beyongDataId)} .im-panel-main-chatcontentpane`)
        } else {
            _contentEl = this.$chatBoxContent('main')
        }
        if (needPrepend)
            _contentEl.prepend(html); // 插入被选元素的开头
        else
        _contentEl.append(html);  // 插入被选元素的末尾

        // 红包点击事件
        if (red_obj) {
            RBChatSmallWindowUI.getWindowDom("#" + red_obj.walletId + "_id_clck", null, beyongDataId).click(function () {
                const _time = Number(this.getAttribute('time'))
                const _el = document.getElementById(`${red_obj.walletId}_status_text`)
                // 超过24小时的红包， toast提示
                if (_el.innerHTML == '待领取' && Date.now() - _time > 24 * 60 * 60 * 1000) { // 
                    _el.innerHTML = '已过期'
                    RBChatToastHelper.showToast_WARN("红包超24小时已过期，已领取可在红包记录中查看", null, 'toast');
                    return
                } else if (_el.innerHTML == '已过期') { // 已过期的红包，点击无反应
                    return
                }
                // 点击领取红包
                var localUserUid = LocalUserInfo.getUid();
                //领取红包详情
                let g_id =  window.groupInfo?.g_id
                let g_name = window.groupInfo?.g_name
                if (!window.openGroupChattingType) { // 私聊红包
                    g_id = window.scrollInfo.beyongDataId
                    g_name = red_obj.nickname
                    // 自己不能领取自己发送的私聊红包, 可以打开已领取的红包
                    if (red_obj.userId == localUserUid && RBChatSmallWindowUI.getWindowDom(`#${red_obj.walletId}_status_text`, null, beyongDataId).text() != '已领取') return
                }
                RBChatRestHelper.recWalletIdFromServer(fp, g_id, red_obj.walletId, localUserUid, g_name
                    , function (returnValue) {
                        //
                        let redobJ = null;
                        try {
                            ...
                            //领取成功
                            if (redobJ.retCode - 0 == 0 || redobJ.retCode - 0 == 1001 || redobJ.retCode - 0 == 1003) {
                                var callBack = function (e) {
                                    var obj = null;
                                    try {
                                        obj = JSON.parse(e.data)
                                ...
                                // const url = "https://chat.imnono.net/coupon/claimDetails-2.html?walletId=" + walletId + "&hideCash=1&userId=" + localUserUid+'&v='+Math.random();
                                const url = RBChatConfig.COUPON_IFRAME_URL + "/coupon/claimDetails-2.html?walletId=" + walletId + "&hideCash=1&userId=" + localUserUid+'&v='+Math.random();
        ...
        // 回复点击
        const replyMsgClick = RBChatSmallWindowUI.getWindowDom('#replay-root-msg-click-' + fp, null, beyongDataId)
        if (replyMsgClick) {
            //回复点击事件
            replyMsgClick.click(function () {
                ...
                // 自动滚动到聊天的位置
                if (fp && RBChatSmallWindowUI.getWindowDom('#' + fp, null, beyongDataId)) {
                    const mainContainer = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap', null, beyongDataId);
                    const scrollToContainer = RBChatSmallWindowUI.getWindowDom('#' + fp, null, beyongDataId)
    ...
    UIModule5.prototype.insertChatItem2 = function (chatMsgEntity, content_dom, id) {
        ...
        //else
        {
            ...
            // @消息
                ...
                select_obj.forEach(item => {
                    const l = '@' + item.nickname
                    origin_text = origin_text.replace(new RegExp(l, 'g'), `<font onclick='javascript:RBChatDialogHelper.showUserInfoFromServer(false, null, ${item.user_uid}, null,true);' color='#28a0ff' style="cursor: pointer;">` + l + "</font>")
                })
                contentHTML = this.replaceEmojiPlaceholderToHTML(origin_text);
                if (contentHTML)
                    contentHTML = contentHTML.replace(new RegExp('\n', 'g'), '<br>');
            ...
            // 判断 需要显示的vip
            var level_html = '';
            if(window.managerIds?.includes(uid)){
                level_html = "<span class='m-right'><font color='black'>[管理]</font></span>"
            }else{
                if (!RBChatUtils.isMobile()) {
        ...
        // 添加右键菜单
        RBChatSmallWindowUI.getWindowDom("#msg-d-" + id).bind('contextmenu', function (e) {
            // 右键菜单主div层
            var popupId = "im-panel-msg-popupmenu";
            var oldPopupObj = $("#" + popupId);
            // 如果已经存在则先删除之（jq里选择器选回对象的Length>0表示该元素是存在的）
            ...
            // 取消置顶
            RBChatSmallWindowUI.getWindowDom('#im-panel-msg-popupmenu-cancle-msg-top').click(function () {
                newPopupObj.remove();
    ...
    // 获取消息
    UIModule5.prototype.getMsgHtml = function (content, msgType, uid) {
        if (msgType == 0) {
            return content
        } else if (msgType == 12) {
            ...
            let contentHTML
            var fileMeta = JSON.parse(content);
            var myUserId = LocalUserInfo.getUid();
            var isMe = uid - myUserId == 0
            ...
            if (fileMeta.isMovie){
                ...
                var durationString = fileMeta.durationString;
                contentHTML = `
                    <a href="javascript:void(0)" style="width:230px;display: flex;position: relative;justify-content: center;align-items: center;"  onclick="javascript:videoSwiperListUI2('${httpDownloadURL}');return false;">
                        <!-- 视频预览图片 -->
                        <img class="chat-msg-video-thumbimg" style="width:170px;height:250px;" src="${picUrl}">
                        <!-- 视频播放图标 -->
                        <img class="chat-msg-video-playicon" style="position: absolute;width: 30px;height: 30px;" src="images/common_short_video_player_continue_play_ico_nor.png">
                    </a>
                `;
                ...
                contentHTML = `
                    <a href="javascript:void(0)" style="width:230px;display: flex;position: relative;justify-content: center;align-items: center;" onclick="javascript:videoSwiperListUI2('${httpDownloadURL}');return false;">
                        <!-- 视频预览图片 -->
                        <img class="chat-msg-video-thumbimg" style="width:170px;height:250px;" src="${httpDownloadURL}?ci-process=snapshot&time=0.01">
                        <!-- 视频播放图标 -->
                        <img class="chat-msg-video-playicon" style="position: absolute;width: 30px;height: 30px;" src="images/common_short_video_player_continue_play_ico_nor.png">
                    </a>
                `;
            ...
            contentHTML =
                "<audio src=\"" + RBChatUtils.getVoiceDownloadURL(content, false) + "\" preload=\"metadata\" controls=\"controls\"></audio>";
            cssExtra = "chat-info_voicemsg";
            ...
            const obj = JSON.parse(content);
            contentHTML = "<div style='width: 300px;border: 1px solid #bcc5c7;border-radius: 15px;' class='minapp-root' " + phoneCss + ">" +
                "<div class='minapp-root-top'>" +
                "<img src='" + obj.appletImage + "'/> <span>" + obj.appletTitle + "</span>" +
                "</div>" +
                "<div class='minapp-root-bottom'>" +
            ...
            // 名片消息的messageContent中存放的就是ContactMeta对象
            // （详见创建代码：rbchat_cache.js中的createChatMsgEntity_COME_CONTACT(..)函数）
            var contactMeta = JSON.parse(content);
    ...
    // 封禁账户
    UIModule5.prototype.fengjinUser = function (uid) {
        RBChatRestHelper.fengjinToServer(uid, 0 // 0:封账号以及IP ， 1：封设备
            // 成功后的回调
            , function (returnValue) {
                if (returnValue) {
                    alert('封禁成功');
                    ...
                    RBChatUtils.logToConsole_WARN('[submitDeleteGroupMemberAsyncTask] 踢出员请求完成，但服务端返回值是空！('+returnValue+')');
                }
            }
            // 失败后的回调
            , function (errorThrownStr){
                RBChatDialogHelper.showAlertDialog_WARN('踢出失败', '踢出群员失败了，可能是网络故障，请稍后再试！');
            }
        );
    }

    // 移除群聊
    UIModule5.prototype.removeGroup = function (data) {
        const { currentChatId, g_id, name, g_name } = data
        var localUser = LocalUserInfo.getObj();
        // 调用HTTP REST接口：“【接口1016-24-23】删除群成员或退群”，具体参数和返回值，详见接口文档或服务端代码。
        RBChatRestHelper.submitDeleteOrQuitGroupToServer(localUser.user_uid, localUser.nickname, currentChatId, [['',g_id, g_name]]
            // 成功后的回调
            , function (returnValue) {
                if (returnValue) {
                    // 删除成功
                    if('1' == returnValue){
                        // 群组信息（GroupEntity对象）
                        var ge = GroupsProvider.getGroupInfoByGid(currentChatId);
                        if(ge) {
                            // 显示一个toast提示
                            // RBChatToastHelper.showToast_OK('踢出成功！', null);
                            // 先：更新本地缓存数据中的当前群成员总数
                            GroupsProvider.updateGroupMemberCount(currentChatId, -1);
                            // 再：刷新新群组基本信息的UI显示
                            RBChatRightDetailUI.refreshGroupBaseInfo(currentChatId, null);
                            // 最后：往聊天界面中显示一条被"我"(我就是群主自已了，不然哪有移除权限)删除群员成功
                            // 的系统通知给"自已"看（此通知并非服务器发出，而是本地准备好的，仅用UI显示）
                            GChatDataHelper.addSystenInfo_removeMembersSucessForLocalUser([{'nickname': name}], currentChatId, g_name);
                            // 因群员数量的变更，即时更新群头像的显示哦
                            that.forceRefreshGroupAvatarShow(currentChatId);
                        }
                    }
                    // 删除失败
                    else{
                        RBChatDialogHelper.showAlertDialog_WARN('踢出失败', '踢出群员失败，请稍后再试！');
                    }
                }
                else {
                    RBChatUtils.logToConsole_WARN('[submitDeleteGroupMemberAsyncTask] 踢出员请求完成，但服务端返回值是空！('+returnValue+')');
    ...
    UIModule5.prototype.constructPopupMenuForChatItem = function (messageType, chatMsgEntity, fp) {
        RBChatSmallWindowUI.getWindowDom("#im-panel-main-chatcontentpane").bind('contextmenu', function (e) {
            return false;
        });
        ...
        var deal_context_menu = function (e, is_forbid = false) {
            ...
            if (isGroupChat) {
                ...
                fengjin = GroupsProvider.isThisGroupOwner(currentChatId) || window.groupInfo && window.groupInfo.manage_mark - 0 == 1;
            } else {
                canMsgTop = true;
                fengjin = true;
            }
            ...
            var html =
                ...
                + '   <div class="kchat-pop ">'
                + '       <ul>'
                + (canForBid ? '    <li id="im-panel-msg-popupmenu-say-hello">' + '一键打招呼'  + '</li>' : '')
                ...
                // + (fengjin ? '    <li id="im-panel-msg-popupmenu-fenjin">' + '封禁'  + '</li>' : '')
                + (fengjin ? '    <li id="im-panel-msg-popupmenu-fenjin-account">' + '封禁账户'  + '</li>' : '')
                // + (fengjin ? '    <li id="im-panel-msg-popupmenu-fenjin-device">' + '封禁设备'  + '</li>' : '')
                + (canForBid && hasOp ? '    <li id="im-panel-msg-popupmenu-forbid">' + (is_forbid ? '取消禁言' : '禁言') + '</li>' : '')
                + (canForBid && hasOp ? '    <li id="im-panel-msg-popupmenu-tgroup">' + '踢出群'  + '</li>' : '')
                + '       </ul>'
                + '   </div>'
                + '</div>';
            ...
            // newPopupObj.css("top", y + "px");
            // newPopupObj.css("left", x + "px");
            const l = document.querySelectorAll('#im-panel-msg-popupmenu .kchat-pop li').length
            var x = e.clientX;
            var y = e.clientY;
            const windowHeight = window.innerHeight || document.documentElement.clientHeight
            y = y + l * 30 > windowHeight ? 
                Math.max(0, windowHeight - l * 30) : y
            // 在鼠标点击的位置显示菜单
            newPopupObj.css("top", y + 'px');
            newPopupObj.css("left", x + "px");
            newPopupObj.show();
            ...
            var  condit_value = 0;
            if(chatMsgEntity.payDate){
                var day = RBChatUtils.dateDiff(chatMsgEntity.nowTime, chatMsgEntity.payDate).replace('天','');
                condit_value = 15 -day<=0? 1:2;
            }

             // 发送好友信息
             var to_say_logic= function(myUserId,userId,nkname,cvalue){
                ...
                async function (returnValue) {
                    ...
                    var tposition = cvalue+3;
                    if (ree.length > tposition) {
                        if (ree[tposition]) {
                            // 发送欢迎提示语
                            const fn = (msgTypes, content) => {
                                var fingerPrint = MBProtocalFactory.genFingerPrint();
                                MessageHelper.sendMessage(msgTypes, userId, content, function (isSucess, msgBody) {
                                    if (isSucess) {
                                        var friendUid = userId;
                                        var ree = RosterProvider.getFriendInfoByUid(friendUid) || {};
                                        // 自已发出的消息，也要显示在相应的UI界面上
                                        var alarmMessageDTO = AlarmsProvider.createChatMsgAlarmForLocal(msgBody.ty, msgBody.m, ree.nickname || nkname, friendUid);
                            ...
                            if (ree[tposition].includes('|||') && !window.app_applets?.length) {
                                await RBChatMainUI.getAppList()
                            }
                            if (ree[tposition].includes('|||') && window.app_applets?.length) {
                                const apps = []
                                let content
                                let result = ree[tposition].split('|||')
                                let firstSend = 'content'
                                result.forEach((str, index) => {
                                    const t = str.substr(0, 4)
                                    if (t == 'app:') {
                                        const minAppFlag = str.split(':')[1]
                                        const obj = window.app_applets.find(item => item.appletId == minAppFlag || item.appletName == minAppFlag); 
                                        if (obj) {
                                            if (index == 0) firstSend = 'app'
                                            apps.push(JSON.stringify(obj))
                                            return
                                        }
                                    }
                                    content = str
                                ...
                                if (!apps.length) content = ree[tposition]
            ...
            // 一键打招呼
            $('#im-panel-msg-popupmenu-say-hello').click(function(){
                var myUserId = LocalUserInfo.getUid();
                var userId = chatMsgEntity.uid;
                var ree = RosterProvider.getFriendInfoByUid(userId)
                if(ree){
                    to_say_logic(myUserId,userId,chatMsgEntity.name,condit_value);
                }else{
                    RBChatRestHelper.submitAddFriendRequestToServer(userId, '新朋友你好'
                    // 数据读取成功后的回调
                    , function (returnValue) {
                        to_say_logic(myUserId,userId,chatMsgEntity.name,condit_value);
                    }
            ...
            // 踢出群
            $('#im-panel-msg-popupmenu-tgroup').click(function(){
                newPopupObj.remove();
                var localUser = LocalUserInfo.getObj();
                // 调用HTTP REST接口：“【接口1016-24-23】删除群成员或退群”，具体参数和返回值，详见接口文档或服务端代码。
                RBChatRestHelper.submitDeleteOrQuitGroupToServer(localUser.user_uid, localUser.nickname, currentChatId, [['',chatMsgEntity.uid,chatMsgEntity.name]]
                    // 成功后的回调
                    , function (returnValue) {
                        if (returnValue) {
                            // 删除成功
                            if('1' == returnValue){
                                ...
                                if(ge) {
                                    ...
                                    // 最后：往聊天界面中显示一条被"我"(我就是群主自已了，不然哪有移除权限)删除群员成功
                                    // 的系统通知给"自已"看（此通知并非服务器发出，而是本地准备好的，仅用UI显示）
                                    GChatDataHelper.addSystenInfo_removeMembersSucessForLocalUser([{'nickname':chatMsgEntity.name}], currentChatId, ge.g_name);
                                    // 因群员数量的变更，即时更新群头像的显示哦
                                    that.forceRefreshGroupAvatarShow(currentChatId);
                                }
                            }
                    ...
                    window.groupInfo.g_name
                );

            })
            // 封禁 改成两个了
            /* $('#im-panel-msg-popupmenu-fenjin').click(function(){
                newPopupObj.remove();
                RBChatRestHelper.fengjinToServer(chatMsgEntity.uid
                    // 成功后的回调
                    , function (returnValue) {
                        if (returnValue) {
                            alert('封禁成功');
            ...
            // 封禁账户
            $('#im-panel-msg-popupmenu-fenjin-account').click(function(){
                newPopupObj.remove();
                RBChatRestHelper.fengjinToServer(chatMsgEntity.uid, 0 // 0:封账号以及IP ， 1：封设备
                    // 成功后的回调
                    , function (returnValue) {
                        if (returnValue) {
                            alert('封禁成功');
            ...
            // 封禁设备
            RBChatSmallWindowUI.getWindowDom('#im-panel-msg-popupmenu-fenjin-device').click(function(){
                newPopupObj.remove();
                RBChatRestHelper.fengjinToServer(chatMsgEntity.uid, 1  // 0:封账号以及IP ， 1：封设备
                    // 成功后的回调
                    , function (returnValue) {
                        if (returnValue) {
                            alert('封禁成功');
            ...
            $('#im-panel-msg-popupmenu-replay').click(function () {
                ...
                var left_htmle = function (messageType, text) {
                    if (messageType == MsgType.TYPE_IMAGE) {
                        return "<img src='" + (chatMsgEntity.text&& chatMsgEntity.text.indexOf('http')!=-1 ? chatMsgEntity.text: RBChatUtils.getImageDownloadURL(chatMsgEntity.text, false)) + '?imageView2/2/w/150' + "'/>"
                    }
                    return ''
                }
                var c = { ...chatMsgEntity };
                c.name = c.groupName || (c.origin_name || c.name);
                //是回复消息
                if (c.msgType == 11) {
                    c.msgType = 0; //转换成文本消息
                ...
                RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-replay-ui').append(reply_html);
                RBChatSmallWindowUI.getWindowDom('#replay-close').click(function () {
                ...
                RBChatSmallWindowUI.getWindowDom('#im-panel-main-chat-replay-ui').show();
                RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane').css('padding-bottom', '66px');
                RBChatChattingContentPaneUI.scrollToBottom4IM();
            ...
            if (canDelete) {
                ...
                $("#im-panel-msg-popupmenu-delete").click(function (e) {
                    ...
                    that.doMessageDelete(currentChatType, fpForMessage, currentChatId, function () {
                        // 从ui界面上删除该消息
                        RBChatSmallWindowUI.getWindowDom("#" + fp).remove();
                    });
                    // 取消弹出菜单的显示
                    newPopupObj.remove();
                });
            ...
            if (canRevoke) {
                ...
                $("#im-panel-msg-popupmenu-revoke").click(function (e) {
                    try {
                        RBChatMsgCache.updateCache({ chatId: currentChatId, data: chatMsgEntity.fingerPrintOfProtocal, type: 'delete' })
                    } catch (err) {}
                    // console.log('点击了撤回')
        ...
        // 添加右键菜单
        RBChatSmallWindowUI.getWindowDom("#" + fp).bind('contextmenu', function (e) {
            // 群 
            if (window.openGroupChattingType) {
     ...
     * @param needPrepend true表示插入被选元素的开头，否则插入被选元素的末尾
     */
    UIModule5.prototype.insertChatItemForSysteminfo = function (sysinfo, timeToShow, needPrepend,chatMsgEntity, beyongDataId) {
        var html = this.constructChatItemForSysteminfo(sysinfo, timeToShow,chatMsgEntity);
        console.log(sysinfo, timeToShow, needPrepend,chatMsgEntity, beyongDataId, 1241256655)
        // 无条件尝试清除空数据UI的显示（该UI只应在没有聊天数据时显示）
        this.clearChatPaneEmpty();


        const _el = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane', null, beyongDataId)
        // 添加到消息界面
        if (needPrepend)
            _el.prepend(html); // 插入被选元素的开头
        else
            _el.append(html);  // 插入被选元素的末尾
    };
     ...
     * @param needPrepend true表示插入被选元素的开头，否则插入被选元素的末尾
     */
    UIModule5.prototype.insertChatItemForRevoked = function (messageContent, timeToShow, needPrepend, beyongDataId) {
    ...
    UIModule5.prototype.constructChatItemForSysteminfo = function (sysinfo, timeToShow,chatMsgEntity=null) {
        // 红包领取消息需要特殊处理
        if(chatMsgEntity && chatMsgEntity.msgType == 90){
            ...
            // 昵称颜色
            var name_color = 'style="color:#afb1b5"';
            if(chatMsgEntity.ulastlogin){
                var days = daysBetween(new Date(chatMsgEntity.ulastlogin),new Date());
                if(days > 0 && days> 3 && days<7){
                    name_color='style="text-decoration: underline;color:rgb(227,207,87);"';
                }
                if(days > 0 && days>=7 && days<10){
                    name_color='style="text-decoration: underline;color:blue;"';
                }
                if(days > 0 && days>= 10 && days<20){
                    name_color='style="text-decoration: underline;color:#800080;"';
                }
                if(days > 0 && days>= 20){
                    name_color='style="text-decoration: underline;color:red;"';
                }
            }

             // 判断 需要显示的vip
             var level_html = '';
             if(window.managerIds && window.managerIds?.includes(chatMsgEntity.beyongUid)){
                level_html = "<span class='m-right'><font color='black'>[管理]</font></span>"
             }else{
                if (!RBChatUtils.isMobile()) {
                    const cacheObj = JSON.parse(localStorage.getItem(`${chatMsgEntity.beyongUid}_info`) || '{}')
                    const _uname = cacheObj.uname || chatMsgEntity.uname
                    const _mlevel = cacheObj.mlevel || chatMsgEntity.mlevel
                    const _payDate = cacheObj.payDate || chatMsgEntity.payDate
                    const _cmoney = cacheObj.cmoney || chatMsgEntity.cmoney
             ...
             var isMe = false;
            //昵称替换
            sysinfo = sysinfo.replace(chatMsgEntity.origin_name,'<span '+name_color+'>'+chatMsgEntity.origin_name+'</span>('+level_html+')')
            var  condit_value = 0;
            if(chatMsgEntity.payDate){
                var day = RBChatUtils.dateDiff(chatMsgEntity.nowTime, chatMsgEntity.payDate).replace('天','');
                condit_value = 15 - day <=0? 1:2;
            ...
            var to_say_logic= function(myUserId,userId,nkname,cvalue){
                ...
                async function (returnValue) {
                    ...
                    if (ree.length > tposition) {
                        if (ree[tposition]) {
                            // 发送欢迎提示语
                            const fn = (msgType, content) => {
                                var fingerPrint = MBProtocalFactory.genFingerPrint();
                                MessageHelper.sendMessage(msgType, userId, content, function (isSucess, msgBody) {
                                    if (isSucess) {
                                        var friendUid = userId;
                                        var ree = RosterProvider.getFriendInfoByUid(friendUid) || {};
                                        // 自已发出的消息，也要显示在相应的UI界面上
            ...
            // 打招呼
            window.say_hello = function(userId,nick_name,cvalue){
                var nkname= decodeURIComponent(nick_name);
                var myUserId = LocalUserInfo.getUid();
                var ree = RosterProvider.getFriendInfoByUid(userId)
                if(ree){
                    to_say_logic(myUserId,userId,nkname,cvalue);
                }else{
                    RBChatRestHelper.submitAddFriendRequestToServer(userId, '新朋友你好'
                        // 数据读取成功后的回调
                        , function (returnValue) {
            ...
            var html =
                ...
                + '  <div style="background:unset;border:unset;display: flex;justify-content: center;align-items:center;" >'
                + (sysinfo ? '<div style="display:flex;cursor: pointer;" onclick="javascript:RBChatDialogHelper.showUserInfoFromServer(false, null,' + chatMsgEntity.beyongUid + ', null,'+ !isMe + ');">' + sysinfo +  (timeToShow ? '<span class="system-info-time">' + timeToShow + '</span>' : '')+ '</div>'+'<a style="margin-left:5px;" href="javascript:say_hello('+chatMsgEntity.beyongUid +',\''+encodeURIComponent(chatMsgEntity.origin_name)+'\','+condit_value+')">打招呼</a>' : '')
                + '  </div>'
                + '</div>';
            return html;
        ...
        setTimeout(() => {
           ...
           // console.log('ttt 滚动判断', t, t.scrollHeight, type)
            if (t.scrollHeight) {
                ...
                this.scrollToBottom4IM(chatId)
            }
        }, 50)

        console.log(this.$chatBox(), window._chatType, 12312312312)
       // this.$chatBox(chatId)[0].scrollTop = this.$chatBox(chatId)[0].scrollHeight; // [0]是何意？待考证
        window._unReadNum = 0
        RBChatAlarmsUI.updateUnReadUI(chatId)
    };

    // 判断是否到了底部
    UIModule5.prototype.isScrollToBottom = function () {
        var t = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap')[0]
     ...
     */
    UIModule5.prototype.send4IMCheck = function (sendUser) {
        if (this.onlineCheck()) {
            ...
            if (window._chatType == 'window' || (sendUser?.currentSelectedAlarmDataId && sendUser?.currentSelectedAlarmDataId != 'main')) return true
     ...
     * 发送图片聊天消息的实施方法。
     */
     UIModule5.prototype.doSendImageMessage4IM_mul_types = function (fileNameWithMD5, sendUser = {}) {
        //var that = this;
   ...
   */
     ...
     * 发送“名片”聊天消息的实施方法。
     */
    UIModule5.prototype.doSendContactMessage4IM = function (theUid, theNickname, fn_sucess_send, sendUser) {
        ...
        // 执行真正的消息/指令发送逻辑
        this.doSendMessageImpl(JSON.stringify(contactMeta), MsgType.TYPE_CONTACT, fn_sucess_send, fingerPrint, sendUser);
    };

    /**
     * 发送“反馈链接”聊天消息的实施方法。
     */
    UIModule5.prototype.doSendFeedbackLinkMessage4IM = async function (theUid, theNickname, fn_sucess_send, sendUser) {

        var currentSelectedAlarmDataId = RBChatMainUI.getCurrentSelectedAlarmDataId();
        // 获取最新反馈链接
        await RBChatMainUI.getFeedbackLink()
        const _url = RBChatUtils.replaceDomain(RBChatConfig.FEEDBACK_LINK, window._FEEDBACK_LINK)
        var message = `专属反馈链接：${_url}?userId=${currentSelectedAlarmDataId}&managerId=${theUid}`
        var that = this;
        if (!this.send4IMCheck(sendUser))
            return;
    ...
    UIModule5.prototype.batchSendWithNotify = (friendsList, sendMessage, batchSize = 100, delay = 100, onComplete) => {
        return new Promise((resolve) => {
        let index = 0;
    
        function sendBatch() {
            const batch = friendsList.slice(index, index + batchSize);
     ...
     * @param {*} userList 
     */
    UIModule5.prototype.circleSendGroupMsg = function (count, msgContent, msgType, userList, callBack, noTips) {
        ...
        if (userList.length > 0) {
            ...
            this.doSendMessageImpl(msgContent, msgType, function () {
                ...
                setTimeout(()=>{
                    // 重更新开始调用
                    that.circleSendGroupMsg(count, msgContent, msgType, userList, callBack, noTips);
                },10);
            }, fingerPrint, {
            ...
            if (!noTips) {
                console.log('ttt 群发 完成', Date.now() - window._testTime)
                alert('消息发送完成')
            }
        }
    }
    ...
    */
     ...
     *  创建红包、获取红包详情
     * @param {*} params 红包参数
     * @param {*} fn_callback_after_send_sucess 成功后回调
     * @param {*} fn_callback_after_send_error 失败后回调
     */
    UIModule5.prototype.getRedPack = function (params, fn_callback_after_send_sucess, fn_callback_after_send_error) {

        infoData = $.ajax({
            type: "GET",
            url: window.RBChatConfig.HTTP_REST_URL + '?processorId=1018&jobDispatchId=29&actionId=41&t=' + Date.now() + '&newData=' + params,
            crossDomain: true,
            success: function(infoData) {
                let resultInfo = JSON.parse(JSON.parse(infoData).returnValue)
                if (resultInfo.retCode == 0) {
                    //查询红包详情
                    RBChatRestHelper.queryWalletIdFromServer(resultInfo.data
                        , function (returnValue) {
                            const res = JSON.parse(returnValue);
                            if (res.data) {
                                const s_obj = res.data;
                                s_obj['title'] = '元友红包';
                                fn_callback_after_send_sucess(JSON.stringify(s_obj))
                            } else {
                                fn_callback_after_send_error(res)
                            }
                        }
                        , function (errorThrownStr) { // 获取红包详情报错，终止定时发红包
                            fn_callback_after_send_error(errorThrownStr)
                            //alert('首页历史"消息"列表数据读取出错，原因是：'+errorThrownStr);
                            // RBChatDialogHelper.showAlertDialog_WARN('加载失败', '首页历史"消息"列表数据加载出错，可能是网络故障，请稍后再试！');
                        }
                    );
                } else { // 创建红包报错，终止定时发红包
                    fn_callback_after_send_error(resultInfo)
                }
            }
        });
    }
     ...
     * @param {*} params 自动发送参数
     */
    UIModule5.prototype.autoSendRed = function (params) {
        const { times, interval, redParams, id, time, currentSelectedAlarmDataId } = params
        let curTimes = times
        let autoSendRedInterval
        const fn = () => {
            // 获取红包成功后，发送消息
            const successCb = function (data) {
                const fingerPrint = MBProtocalFactory.genFingerPrint()
                GMessageHelper.sendGroupMessage(10, currentSelectedAlarmDataId, data, function (isSucess, msgBody, currentChattingGe) {
                    if (isSucess) {
                        ...
                        // 消息数据对象
                        var chatMsgEntity = ChatMsgEntity.prepareSendedMessage(msgBody.m, 0, fingerPrint, 10);
                        ...
                        // 将此条消息存入缓存并在UI上显示
                        // if (currentSelectedAlarmDataId == RBChatMainUI.getCurrentSelectedAlarmDataId()) {
                        RBChatMainUI.processRecivedMessage(true, true, alarmMessageDTO, chatMsgEntity, true);
                        // }
                    }
                ...
                curTimes--
                const _oldCache = JSON.parse(sessionStorage.getItem('autoSendRed') || '[]')
                const _index = _oldCache.findIndex(item => item.id == id) 
                if (_index != -1) {
                    _oldCache[_index].times--
                    _oldCache[_index].time = Date.now()
                }
                if (curTimes <= 0) {
                    clearInterval(autoSendRedInterval)
                    if (_index != -1) _oldCache.splice(_index, 1)
                }
                sessionStorage.setItem('autoSendRed', JSON.stringify(_oldCache))
            }
            ...
            const errorCb = (data) => {
                ...
                if (code == 1002) {
                    RBChatDialogHelper.showAlertDialog_ERROR('提示', '当前余额不足，自动红包策略已停止，如需继续可重新设置');
                } else {
                    RBChatDialogHelper.showAlertDialog_ERROR('提示', `${errorMsg}，自动红包策略已停止，如需继续可重新设置`);
                }
                clearInterval(autoSendRedInterval)
                const _oldCache = JSON.parse(sessionStorage.getItem('autoSendRed') || '[]')
                const _index = _oldCache.findIndex(item => item.id == id) 
            ...
            // 创建红包
            this.getRedPack(redParams, successCb, errorCb)
        }

        // 刷新后，根据上次发送时间设定定时
        const delay = Date.now() - time - interval * 60 * 1000
        setTimeout(() => {
            fn()
            autoSendRedInterval = setInterval(() => {
                fn()
            }, interval * 60 * 1000)
        }, delay > 0 ? 0 : Math.abs(delay))
    }

    UIModule5.prototype.deleteSendMsgRetryQueue = function (fingerPrint) {
        const _index = this.sendMsgRetryQueue.findIndex(item => item.fingerPrint == fingerPrint) 
        if (_index != -1) this.sendMsgRetryQueue.splice(_index, 1)
    }

    UIModule5.prototype.sendMsgRetry = async function (type) {
        console.log('ttt 消息 重试 Start', type, this.sendMsgRetryQueue.length)
        //2025-10-30  Allen 屏蔽
        // 发送消息逻辑
        const sendMsg = (item) => {
            // const { currentSelectedAlarmType, currentSelectedAlarmDataId, arguments } = item
            // const [msgContent, msgType, fn_callback_after_send_sucess, fingerPrint, sendUser, ...otherParams] = arguments
            // const _sendUser = sendUser?.currentSelectedAlarmDataId ? sendUser : { currentSelectedAlarmType, currentSelectedAlarmDataId }
            // this.doSendMessageImpl.apply(this, [msgContent, msgType, null, fingerPrint, _sendUser, ...otherParams])
        }

        // 初始化对队列所有消息进行发送
        if (type == 'init' && this.sendMsgRetryQueue.length) {

            // 延迟，等待上一条消息完成
            const delay = (data) =>  {
                return new Promise(resolve => {
                    const timer = setInterval(() => {
                        const _index = this.sendMsgRetryQueue.findIndex(item => item.fingerPrint == data.fingerPrint) 
                        if (_index == -1) {
                            clearInterval(timer)
                            resolve()
                        }
            ...
            clearInterval(this.sendMsgRetryTimer)
            this.sendMsgRetryTimer = null

            // 根据会话拆成二维数组
            const result = this.sendMsgRetryQueue.reduce((acc, curr) => {
                const group = acc.find(item => item[0].currentSelectedAlarmDataId === curr.currentSelectedAlarmDataId && item[0].currentSelectedAlarmType === curr.currentSelectedAlarmType);
                if (group) {
                  group.push(curr);
                } else {
                  acc.push([curr]);
            ...
            const sendMsgQueueFn = async (item) => {
                for (let i = 0; i < item.length; i++) {
                    if (i != 0) {
                        await delay(item[i - 1])
                    }
                    const _index = this.sendMsgRetryQueue.findIndex(item2 => item2.fingerPrint == item[i].fingerPrint) 
                    if (_index != -1) { 
                        sendMsg(item[i])
                    }
                }
            ...
            // 同一个会话多条消息，第二条需等待前面一条完成才能开始发送。防止消息顺序问题
            result.forEach(item => {
                sendMsgQueueFn(item)
            })
        ...
        // 发消息重试定时器
        if (type != 'init' && !this.sendMsgRetryTimer) {
            this.sendMsgRetryTimer = setInterval(() => {
                console.log('ttt 消息 Timer Start', this.sendMsgRetryQueue.length)
                if (!this.sendMsgRetryQueue.length) {
                    clearInterval(this.sendMsgRetryTimer)
                    this.sendMsgRetryTimer = null
                    return
                }

                sendMsg(this.sendMsgRetryQueue[0])
            }, 5000)
        }
    }
    ...
    UIModule5.prototype.doSendMessageImpl = function (msgContent, msgType, fn_callback_after_send_sucess, fingerPrint, sendUser = {}, isIMCheck = true, isGroupSend = false) {
        console.log('ttt 消息 消息发送', msgContent, fingerPrint, arguments)
        var currentSelectedAlarmType = RBChatMainUI.getCurrentSelectedAlarmType();
        var currentSelectedAlarmDataId = RBChatMainUI.getCurrentSelectedAlarmDataId();
        if (window._chatType == 'window' && sendUser?.currentSelectedAlarmType != 'main') {
            currentSelectedAlarmDataId = window._chatId
        ...
        console.log("currentSelectedAlarmDataId: ", currentSelectedAlarmDataId);
        console.log("currentSelectedAlarmType: ", currentSelectedAlarmType);
        // 文件上传默认选择了当前一个用户
        if (sendUser?.currentSelectedAlarmDataId && sendUser?.currentSelectedAlarmDataId != 'main') {
            currentSelectedAlarmType = sendUser.currentSelectedAlarmType
            currentSelectedAlarmDataId = sendUser.currentSelectedAlarmDataId
        }

        if ((!currentSelectedAlarmDataId && currentSelectedAlarmDataId != 'main') || !currentSelectedAlarmType) {
            return
        }
        ...
        console.log($('#alarms_li_'+currentSelectedAlarmType+'_'+currentSelectedAlarmDataId).length,'#alarms_li_'+currentSelectedAlarmType+'_'+currentSelectedAlarmDataId)
        // 发送的是好友聊天和临时聊天
        if (currentSelectedAlarmType == AlarmMessageType.tempChatMessage || currentSelectedAlarmType == AlarmMessageType.reviceMessage) {
                ...
                MessageHelper.sendMessage(msgType, currentSelectedAlarmDataId, msgContent, function (isSucess, msgBody) {
                    if (isSucess) {
                        RBChatSmallWindowUI.updateWindowAttr(currentSelectedAlarmDataId, { draft: '' })
                        //// 清空输入框
                        //that.$inputMessage.val('');
                        if (fn_callback_after_send_sucess)
                            fn_callback_after_send_sucess();
    
                        if(currentSelectedAlarmType == AlarmMessageType.tempChatMessage){
                            ...
                            var alarmMessageDTO = AlarmsProvider.createATempChatMsgAlarmForLocal(msgBody.ty, msgBody.m
                                ...
                                , guestUid);
                            var chatMsgEntity = ChatMsgEntity.prepareSendedMessage(msgBody.m, 0, fingerPrint, msgType);
                            if(!isGroupSend || isGroupSend&&RBChatSmallWindowUI.getWindowDom('#alarms_li_'+currentSelectedAlarmType+'_'+currentSelectedAlarmDataId).length){
                                 // 将此条消息存入缓存并在UI上显示
                            ...
                            if(!isGroupSend || isGroupSend&&$('#alarms_li_'+currentSelectedAlarmType+'_'+currentSelectedAlarmDataId).length){
                                    // 将此条消息存入缓存并在UI上显示
                                    RBChatMainUI.processRecivedMessage(true, false, alarmMessageDTO, chatMsgEntity, isGroupSend);
                ...
                }, fingerPrint, isIMCheck,isGroupSend, sendUser);
        }
        // 发送的是陌生人/临时聊天类型的消息
        else if (currentSelectedAlarmType == AlarmMessageType.tempChatMessage) {
        ...
        // 发送的是群聊消息
            ...
            }, fingerPrint,isGroupSend, false, sendUser);
        }
        else {
    ...
    UIModule5.prototype.dealMsgFromServer = function (chattingHistoryList, params) {
        const { beyongDataId, isGroupChatting } = params
        const TAG = (isGroupChatting ? "群聊" : "一对一");
        ...
        if (chattingHistoryList && chattingHistoryList.length > 0) {// 正常处理完成的情况下接口约定为本字段存放Vector<Vector>结果
            ...
            // 存储消息记录
            local_time_obj={'id':beyongDataId,unread:0}
            localStorage.setItem('time-'+beyongDataId,JSON.stringify(local_time_obj))
            ...
            for (var i = 0; i < chattingHistoryList.length; i++) {
                ...
                var srcUid = row[0]
                    ...
                    , uname = row.length > 17 ? row[17] : ''
                    , ulastlogin = row.length > 18 ? row[18] : ''
                    , groupName = row[8]
                ...
                chatMsgEntity.origin_name = origin_name;
                ...
                chatMsgEntity.uname = uname
                chatMsgEntity.ulastlogin = ulastlogin
                chatMsgEntity.beyongUid = beyongUid
                if (msg_type - 0 == 9) {
    ...
    UIModule5.prototype.loadCacheHistoryFromCache2 = function (alarmMessageType, beyongDataId) {
        // 先清空当前的聊天消息显示
        if (window._chatType == 'window' && beyongDataId == window.scrollInfo?.beyongDataId) {
            $('#kchat-im-panel #im-panel-main-chatcontentpane').empty();
        }
        ...
        RBChatSmallWindowUI.getWindowDom('#tip_my_' + beyongDataId).hide();

        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg').hide();

        RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane').removeClass('have-msg-top')
        ...
        // 查看消息撤回记录入口
        isGroupChatting ? $('#im-panel-main-chat-textarea_reCall').show() : $('#im-panel-main-chat-textarea_reCall').hide();

        !isGroupChatting ? $('#send-feedbackLink').show() : $('#send-feedbackLink').hide();
        !isGroupChatting ? $('#send-redpag-friend').show() : $('#send-redpag-friend').hide();


        console.log(window._chatType, window._chatId, window.scrollInfo, 12312312321)
        ...
        const getHistoryCb = (list) => {
            var needSetChatPaneEmpty = false;
            if (!list.length) {
                needSetChatPaneEmpty = true;
            } else {
                const local_time_obj={ 'id':beyongDataId, unread: 0 }
                localStorage.setItem('time-'+beyongDataId,JSON.stringify(local_time_obj))
                if (isGroupChatting) {
                    GroupChattingCache.putChatCaches(beyongDataId, list, true);
                }
                else {
                    // 数据放入与该用户聊天的缓存里存起来（以备后续使用）
                    SingleChattingCache.putChatCaches(beyongDataId, list, true);
                }
            }
            if (needSetChatPaneEmpty) {
                ...
                if (window._chatId == beyongDataId
                    || RBChatSmallWindowUI.checkIsWindow(beyongDataId)
                    || RBChatMainUI.isCurrentSelectedAlarm(AlarmMessageType.tempChatMessage, beyongDataId)
                    ...
                    // if (window._chatType == 'chat') window._chatId = null
                    if (list.length <= RBChatConfig.LOAD_CHAT_RECORDER_COUNTS) {
                        if (window._chatType != 'window' && !RBChatSmallWindowUI.checkIsWindow(beyongDataId)) {
                            window.scrollInfo = { beyongDataId, isGroupChatting, top: 0 };
                        }
                        console.log(JSON.parse(JSON.stringify(list)), 125126162)
                        RBChatChattingContentPaneUI.insertChatItems(list, beyongDataId);
                        // 过滤消息
                        that.showMyTip(list, beyongDataId);
                    } else {
                        const showList = list.slice(list.length - RBChatConfig.LOAD_CHAT_RECORDER_COUNTS, list.length);
                        console.log(JSON.parse(JSON.stringify(showList)), 125126162, 2)
                        RBChatChattingContentPaneUI.insertChatItems(showList, beyongDataId);
                        if (window._chatType != 'window' && !RBChatSmallWindowUI.checkIsWindow(beyongDataId)) {
                    ...
                    // 放到当前聊天内容的前面显示
                    RBChatChattingContentPaneUI.scrollToBottom4IM(beyongDataId, 'clickChat');
                }
        ...
        const getHistory = (isRetry) => {
            const _cache = RBChatMsgCache.checkHasCache(beyongDataId)
            if (!isGroupChatting && _cache) {
                if (!isGroupChatting) {
                    const _newTitle = $(`#alarms_li_msgtitle_${alarmMessageType}_${beyongDataId}`)
                    // 兼容预加载数据没有更新最新的nick、uname没有更新
                    const uname = _cache.findLast(item => item.uid == beyongDataId && item.uname)
                    if (_newTitle.length) {
                        _cache.forEach(item => {
                            if (item.uid == beyongDataId) {
                                item.name = _newTitle.text()
                                if (uname && !item.uname) {
                                    item.uanme = uname
                                }
                            }
            ...
            RBChatRestHelper.queryChattingHistoryFromServer(
                ...
                , function (returnValue) {
                    ...
                    // 服务端返回的是java 2维Vector<Vector>对象数组（相当于JS里的2维嵌套数组）
                    if (!isGroupChatting) {
                        RBChatAlarmsUI.getLastSendMsg(chattingHistoryList, beyongDataId)
                    }

                    console.log(chattingHistoryList, 12321444444)
        
                    const curChatDatId = RBChatMainUI.getCurrentSelectedAlarmDataId()
                    const curChatType = RBChatMainUI.getCurrentSelectedAlarmType()
                    // 不是当前会话，不做处理, 防止串消息
                    if (chattingHistoryList.length && 
                        curChatDatId != chattingHistoryList[0][0] && curChatDatId != chattingHistoryList[0][1] &&
                        !RBChatSmallWindowUI.checkIsWindow(beyongDataId)
                    ) {
                        return
                    }
                    // 兼容预览跟消息列表不同步问题
                    // if (!isRetry && chattingHistoryList.length && curChatDatId == beyongDataId) {
                    //     const _lastChatPreview = $(`#alarms_li_msgcontent_${curChatType}_${curChatDatId}`).text()
                    //     const _newChatPreview = MessageHelper.parseMessageForShow(chattingHistoryList[0][4], chattingHistoryList[0][3])
                    //     if (!_lastChatPreview.includes(_newChatPreview) && !_newChatPreview.includes(_lastChatPreview)) {
                    //         debugger
                    //         getHistory(true)
                    //         return
                    //     }
                    ...
                    if (chattingHistoryList && chattingHistoryList.length > 0) {// 正常处理完成的情况下接口约定为本字段存放Vector<Vector>结果
                        ...
                        for (var i = 0; i < chattingHistoryList.length; i++) {
                            ...
                            localStorage.setItem(`${srcUid}_info`, JSON.stringify({
                                time: msgTime2Timestamp,
                                uname,
                                payDate,
                                cmoney,
                                mlevel
                            ...
                            chatMsgEntity.ulastlogin = ulastlogin
                            chatMsgEntity.beyongUid = beyongUid
                            chatMsgEntity.heade_photo = heade_photo
                            if (msg_type - 0 == 9) {
                                chatMsgEntity.msgType = 9;
                            }
                            // 放入数组
                        ...
                        RBChatMsgCache.updateCache({ chatId: beyongDataId, data: [] })
                    }
                    ...
                    console.log(124124124214, 2, needSetChatPaneEmpty)
                    if (needSetChatPaneEmpty) {
                        // 显示空数据ui
                        that.setChatPaneEmpty();
                    } else {
                        ...
                        if (window._chatId == beyongDataId
                            ...
                            if (list.length <= RBChatConfig.LOAD_CHAT_RECORDER_COUNTS) {
                                ...
                                RBChatMsgCache.updateCache({ chatId: beyongDataId, data: list })
                                RBChatChattingContentPaneUI.insertChatItems(list, beyongDataId);
                                // 过滤消息
                                that.showMyTip(list, beyongDataId);
                            } else {
                                ...
                                RBChatMsgCache.updateCache({ chatId: beyongDataId, data: showList })
                                RBChatChattingContentPaneUI.insertChatItems(showList, beyongDataId);
                                if (window._chatType != 'window' && !RBChatSmallWindowUI.checkIsWindow(beyongDataId)) {
                                    window.scrollInfo = { beyongDataId, isGroupChatting, top: list.length - RBChatConfig.LOAD_CHAT_RECORDER_COUNTS }
                                }
                    ...
                    // getHistoryCb(chattingHistoryList)
                }
                // 数据加载失败后的回调
                , function (errorThrownStr) {
                    //alert('聊天历史记录数据读取出错，原因是：'+errorThrownStr);
    ...
    UIModule5.prototype.init_msg_top_ui = function () {
        ...
        RBChatRestHelper.query_msg_top(groupId, function (returnValue) {
            ...
            if (top_msg_list.length > 0) {
                if (window._chatType == 'window' && window.scrollInfo?.beyongDataId == window._chatId) {
                    RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg', 'parent').hide();
                }
                RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg').show();
                RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane').addClass('have-msg-top')
    ...
    UIModule5.prototype.show_top_msg_ui = function (isClick = false) {
        ...
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-title').text('置顶消息  #' + (index + 1))
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-content').html(this.replaceEmojiPlaceholderToHTML(html_content).replaceAll('<br>', ' '))

        try {
            const _el = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap')[0]
            const { scrollHeight } = _el
            _el.scrollTo(0, scrollHeight)
        } catch (err) {}

        // 遍历线条
        RBChatSmallWindowUI.getWindowDom('.top-line-item').css('background', 'rgba(0,186,255,0.2)')
        RBChatSmallWindowUI.getWindowDom('.line-' + (window.to_msg_show_index % 3)).css('background', 'rgba(0,186,255,1)')

        //绑定点击事件
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-left').unbind('click')
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-left').click(function () {
            // 处理点击事件
            var index_ = window.to_msg_show_index % window.top_msg_list.length
            const item_ = window.top_msg_list[index_]
            const mainContainer = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap');
            const scrollToContainer = RBChatSmallWindowUI.getWindowDom('#' + item_[1]);
            if (scrollToContainer) {
                mainContainer.scrollTop(
                    scrollToContainer.offset()?.top - mainContainer.offset()?.top + mainContainer.scrollTop() - 50
                );
                RBChatSmallWindowUI.getWindowDom('#' + item_[1]).addClass('show-my-tip-select')
                setTimeout(() => {
                    RBChatSmallWindowUI.getWindowDom('#' + item_[1]).removeClass('show-my-tip-select')
                }, 1500);
            }
        ...
        // 右边点击
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-right').unbind('click')
        RBChatSmallWindowUI.getWindowDom('#im-chat-top-msg-right').click(function () {
    ...
    UIModule5.prototype.loadChatHistoryFromLocalCache = function (alarmMessageType, dataId) {
        ...
        if (cachedHistory) {
            ...
            //标准的for循环：遍历 Array[ChatMsgEntity对象] 数组
            this.insertChatItems(cachedHistory, dataId);
                ...
                if (chattingHistoryList && chattingHistoryList.length > 0) {// 正常处理完成的情况下接口约定为本字段存放Vector<Vector>结果
                    ...
                    if (chatHistoryDatas.length > 0) {
                        ...
                        if (RBChatMainUI.isCurrentSelectedAlarm(AlarmMessageType.tempChatMessage, beyongDataId)
                            ...
                            // 放到当前聊天内容的前面显示
                            RBChatChattingContentPaneUI.insertChatItems(chatHistoryDatas, beyongDataId,true);
                            RBChatChattingContentPaneUI.scrollToBottom4IM(beyongDataId);
                        }
                    }
                    else {
            ...
            RBChatRestHelper.submitDeleteChattingSingleMsgToServer(fpForMessage
                ...
                , function (returnValue) {
                    ...
                    if (returnValue) {
                        ...
                        if ('1' == returnValue) {
                            try {
                                RBChatMsgCache.updateCache({ chatId: forId, data: fpForMessage, type: 'delete' })
                            } catch (err) {}
    ...
    UIModule5.prototype.doMessageRevoking = function (chatType, message, toId) {
        ...
        var  msg_name = message.origin_name ?  message.origin_name:message.name;
        // 准备好将要发出的"撤回"指令的指令内容
        var contentForRevokeCMD = RBMessageRevokingManager.constructRevokedMetaForOperator(fpForMessage
            ...
            , isGroupChat && !message.isOutgoing ? msg_name : null);// RevokedMeta
    ...
    const _oldCache = JSON.parse(sessionStorage.getItem('autoSendRed') || '[]')
    if (_oldCache?.length) {
        _oldCache.forEach(item => {
            thisModule.autoSendRed(item)
        })
    }
    ...
    thisModule.initFastSend();   // “发送合集”消息功能的初始化！
    thisModule.initBackBottom();   // 回到底部功能的初始化！
    thisModule.initReCallEvent();
    thisModule.initFeedbackLinkMsgSend()
