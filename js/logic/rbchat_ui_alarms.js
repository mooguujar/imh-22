...
var RBChatAlarmsUI = (function () {
        ...
        $('#im-panel-userlist-wrap-alarms-search-input').bind('input porpertychange', function () {
            ...
            if (val.length > 0) {
                ...
                $('#kchat-im-panel-userlist-alarms li').each(function (i, item) {
                    const _el = $(item)
                    const name = _el.find(".msg_title ").text()
                    const id = _el.attr('im-dataid')
                    $(item).css('display', name.indexOf(val) >= 0 || id.indexOf(val) >= 0 ? '' : 'none');
                })
    ...
    UIModule4.prototype.reload = function (fn_callback_complete) {
        ...
        SingleChattingCache.loadOfflineMessagesAsync(null, function (offlineMessagesList) {
            console.log('ttt 未读 离线消息', offlineMessagesList)
            // 【【1】AJAX异步接口调用】从服务端异步加载聊天历史（就是显示在首页“消息”里的item）
            AlarmsProvider.refreshHistoryChattingAlarmsAsync(function (alarmsHistoryArrays, needClear) {
                // 将加载完成后的聊天历史在UI上显示出来
                that.refreshOfflineMessagesToUI(alarmsHistoryArrays,offlineMessagesList, 'updateCache');
                that.refreshHistoryChattingAlarmsUI(alarmsHistoryArrays, needClear);
                that.refreshOfflineMessagesToUI(alarmsHistoryArrays,offlineMessagesList);
    ...
    UIModule4.prototype.refreshHistoryChattingAlarmsUI = function (arrayArrays, needClear) {
        ...
        if (arrayArrays && arrayArrays.length > 0) {
            ...
            for (var i = 0; i < arrayArrays.length; i++) {
                ...
                var lastSendUid = row[6];       // 最后发送消息uid
    ...
    //改完未读消息ui位置
    UIModule4.prototype.changeUnReadUI=function(key){
        return
        const l = $("#kchat-im-panel-userlist-alarms li[beTop='true'");
        //有置顶的元素
        if(l && l.length > 0){
           l.eq(l.length-1).after($('.alarms_li_'+key));
        //没有置顶的元素
        }else{
            this.$notEmptyUIRoot.prepend($('.alarms_li_'+key));
        }
    }
     ...
     */
    UIModule4.prototype.refreshOfflineMessagesToUI = function (alarmsHistoryArrays,offlineMessagesList, type) {

         // 读取本地时间
         var get_localTime = function(key){
            var local_time_str =  localStorage.getItem('time-'+key)
            var local_time_obj ={}
            if(local_time_str){
        ...
        // 保存当前时间戳
        if(offlineMessagesList['0']){
            localStorage.setItem('offmsg_time',offlineMessagesList['0'])
        }
        // 离线消息数量
        sessions_count={};
        if(offlineMessagesList){
            for(key in offlineMessagesList){
                // 离线未读+本地未读
                var count = offlineMessagesList[key];
                var time_obj = get_localTime(key);
                if(time_obj && time_obj.unread > 0){
                    count = count+ time_obj.unread;
                }
                if(count > 0){
                    console.log('ttt 未读 离线消息 set', key, count)
                    sessions_count[key] = count;
                    if (type == 'updateCache') {
                        window[`_unread_${key}`] = count
                    } else {
                        // localStorage.setItem('time-'+key,JSON.stringify({'id':key,'unread':count}));
                        this.setUnread(0, key, count);
                        this.changeUnReadUI(key);
                    }
                }
        ...
        // 会话列表中，没有的
        if(alarmsHistoryArrays && alarmsHistoryArrays.length > 0){
            const session_unread_list =  alarmsHistoryArrays.filter(item=> !sessions_count[item[0]])
            session_unread_list.forEach(item=>{
                var time_obj = get_localTime(item[0]);
                if(time_obj && time_obj.unread > 0){
                    console.log('ttt 未读 离线消息 set 2', item[0], time_obj.unread)
                    if (type == 'updateCache') {
                        window[`_unread_${item[0]}`] = time_obj.unread
                    } else {
                        this.setUnread(0, item[0], time_obj.unread);
                        this.changeUnReadUI(item[0]);
                    }
                }
            })
        }
    ...
    UIModule4.prototype.existsItem = function (alarmMessageType, dataId) {
        ...
        // 兼容type，临时消息跟好友消息type类型错乱
        if (alarmMessageType == AlarmMessageType.tempChatMessage) {
            $itemObj = $('#alarms_li_' + AlarmMessageType.reviceMessage + '_' + dataId) 
            return $itemObj.length ? true : false
        } else if (alarmMessageType == AlarmMessageType.reviceMessage) {
            $itemObj = $('#alarms_li_' + AlarmMessageType.tempChatMessage + '_' + dataId) 
            return $itemObj.length ? true : false
        }
        return false;
    };
    ...
    UIModule4.prototype.insertOrUpdate = function (amd, toFirst, isGroupMsgSend=false) {
        if (this.existsItem(amd.alarmMessageType, amd.dataId)) {
            ...
            // 已存在则更新之
            this.updateItemContent(amd.alarmMessageType, amd.dataId, amd.date, amd.msgContent, amd.title, isGroupMsgSend);
    ...
    UIModule4.prototype.setCancleTopUI = function(id, cancle){
        const _el = $(id)
        const alarmMessageType = _el.attr('im-alarmtype')
        const dataId = _el.attr('im-dataid')
        const dateTimestamp = _el.attr('im-timeStamp')
        const { index: newChatIndex }  = AlarmsProvider.updateChatTime({ alarmMessageType, dataId, time: dateTimestamp, is2Top: !cancle }, true)
       
            // 取消
        if(cancle){
            _el.removeAttr('beTop');
        ...
        //设置
            ...
            _el.attr('beTop', 'true');
            _el.addClass('active')
            _el.css('background', 'rgba(0,0,0,0.1)')
            // 放到首位
            // $(id).prependTo(this.$notEmptyUIRoot);
        }
        if (newChatIndex == 99999 && cancle) {
            this.$notEmptyUIRoot.append(_el)
        } else if (newChatIndex == 99999 || newChatIndex == 0) {
            _el.prependTo(this.$notEmptyUIRoot)
        } else {
            const el = $('#kchat-im-panel-userlist-alarms li').eq(newChatIndex)
            if (el.attr('im-dataid') == dataId) return
            if (cancle) {
                el.after(_el); 
            } else {
                el.before(_el); 
    ...
    //  * @returns 
    //  */
    UIModule4.prototype.getLastSendMsg = function(list, chatId){
        const _obj = {}
        list.forEach(item => {
            if (!_obj[item[0]]) {
                _obj[item[0]] = item[5]
        ...
        this.updateChatCache(_obj, chatId)
    }

    UIModule4.prototype.updateChatCache = function(obj, chatId, type = 'update'){
        const _curUid = IMSDK.getLoginInfo().loginUserId
        const _cache = JSON.parse(localStorage.getItem(`${_curUid}_chatCache`) || '{}')

        if (!_cache[chatId]) {
            _cache[chatId] = {}
        }
        const _lastCache = _cache[chatId]
        if (type == 'update') {
            if ((obj[_curUid] || 0) > (_lastCache[_curUid] || 0)) {
                _lastCache[_curUid] = obj[_curUid]
            }
            if ((obj[chatId] || 0) > (_lastCache[chatId] || 0)) {
                _lastCache[chatId] = obj[chatId]
            }
        } else if (type == 'delete') {
            _cache[chatId] = {}
        }

        localStorage.setItem(`${_curUid}_chatCache`, JSON.stringify(_cache))
    }


      
    // 获取是否滚动条在底部
    UIModule4.prototype.isScrollBottom = function (chatId) {
        const t = RBChatSmallWindowUI.getWindowDom('#im-panel-main-chatcontentpane-wrap', null, chatId)[0]
        const isScrollToBottom = t.scrollTop + t.clientHeight >= t.scrollHeight - 50

        return isScrollToBottom
    }

    // 更新会话未读数、ui
    UIModule4.prototype.updateUnReadUI = function (chatId) {
        if (window.requestIdleCallback) {
            window.requestIdleCallback((deadline) => {
                const _el = RBChatSmallWindowUI.getWindowDom('.im-panel-main-toBottom-nums', null, chatId)
                if (window._unReadNum) {
                    _el.html(`${window._unReadNum}条新消息`)
    ...
    // 会话点击
    UIModule4.prototype.clickChatItem = function (alarmMessageType, dataId) {
        ...
        if (alarmMessageType == AlarmMessageType.reviceMessage
            ...
            // 设置选中
            window._unReadNum = 0
            RBChatAlarmsUI.updateUnReadUI(dataId)
            this.selectedItem(alarmMessageType, dataId);
            RBChatChattingContentPaneUI.showRightChatContent();
            RBChatChattingContentPaneUI.scrollToBottom4IM(dataId);
    ...
    UIModule4.prototype.insertItem = function (amd, toFirst, isGroupMsgSend = false) {
        ...
        if (this.existsItem(alarmMessageType, dataId)) {
            // 已存在则更新之
            this.updateItemContent(alarmMessageType, dataId, dateTimestamp, alarmContent, title, isGroupMsgSend);
            return
        }
        ...
        let isUnreadChatForFriend = false
        if (window[`_unread_${dataId}`] && amd.lastSendUid == dataId) {
            isUnreadChatForFriend = true
        }
        let newChatIndex
        console.log('ttt 未读 渲染会话', dataId, isUnreadChatForFriend)
        if (!isGroupMsgSend) {
            newChatIndex = AlarmsProvider.updateChatTime({ alarmMessageType, dataId, time: dateTimestamp, is2Top, isUnreadChatForFriend }).index
        }
        // 添加置顶效果
        let tempStr = ''
        ...
        if(isLevel){
          ...
          //  level_html = "<p class='other-tip-"+dataId+"' style='font-size:8px !important;' cmoney='"+(amd.cmoney || '')+"' noPayDate='"+(amd.noPayDate || '')+"' noTime='"+(amd.noTime || '')+"' level='"+(amd.level == undefined ? '': amd.level) +"' lastloginTime='"+(amd.lastLoginTime || '')+"' >"
            ...
            if(!haveV && (amd.isOnline == 1 || amd.onlineWeb == 1)){
                level_html = level_html+ " <span><font color='#57dc2d'>在线</font></span>"
                haveV = true;
            }
        ...
        if(level_css.length == 0 && is2Top){
            level_css = " style = 'background:rgba(0,0,0,0.1);'"
        }

        let str = ''
        try {
            str = JSON.stringify(amd)
        } catch (err) {}
        ...
        if (_tab == 1) {
            if (alarmMessageType != 4) {
                ...
                const _curUid = IMSDK.getLoginInfo().loginUserId
                const _cache = JSON.parse(localStorage.getItem(`${_curUid}_chatCache`) || '{}')
                console.log((RBChatMainWindowUI.checkAtThreeDaysChat(_cache, dataId), 125125125215))
                if  (!RBChatMainWindowUI.checkAtThreeDaysChat(_cache, dataId)) {
                    isShow = false
                }
            }
        }
        ...
        var html=
            "<li id=\'alarms_li_" + alarmMessageType + "_" + dataId + "\'  "+" class=\'alarms_li_item alarms_li_" + dataId + isActive + "\'   "+ tempStr + level_css+"  + im-amd=\'" + str + "\' +   im-alarmtype=\'" + alarmMessageType + "\' im-dataid=\'" + dataId + "\' im-alarmMessageType=\'" + alarmMessageType + "\' im-timeStamp=\'" + dateTimestamp + "\'>"
            + "       <div>"
            +"                 <div class='tip_my' id='tip_my_"+dataId+"'+><img src='/images/group-tip.png'/></div>"
            + "            <a class=\'dasd top-tag\' title=\'Current Tag\'></a>"
            ...
            + "                 <span id=\'alarms_li_unreadflag_"+ dataId + "\' class=\'"+noTipclass+" \' style=\'display:none;\'>0</span>"
            ...
            + (!RBChatUtils.isMobile() && alarmMessageType - 0 == 4 ? "   <div  class='online_status_"+dataId+"' style='height:10px;width:10px;background:"+((amd.isOnline==1 || amd.onlineWeb==1)?"#57dc2d":"#f26c4f")+" ; border-radius: 50%;'></div>":"")
            + "            </div>"
            + "            <div class=\'info\'>"
        ...
        if(isGroupMsgSend){
            ...
            //    hadAdd = true
            // }else{
            //     // this.$notEmptyUIRoot.eq(newChatIndex).before(html);

            //     // this.$notEmptyUIRoot.insertBefore(html, this.$notEmptyUIRoot.children()[newChatIndex]);
            //     // if (toFirst || is2Top)
        ...
        if (!hadAdd) {
            if (newChatIndex == 99999) {
                this.$notEmptyUIRoot.append(html);
            } else if (document.querySelectorAll('#kchat-im-panel-userlist-alarms li').length) {
                const el = $('#kchat-im-panel-userlist-alarms li').eq(newChatIndex)
                el.before(html);
        ...
        if (!isShow) {
            $("#alarms_li_" + alarmMessageType + "_" + dataId).css('display', 'none')
        }
            ...
            $('#im-panel-msg-popupmenu-no-tip').click(function(){
                ...
                var  localNoTipSet = function(){
                    ...
                    if(isNoTip){
                        ...
                        that.setNoTipUI("#alarms_li_unreadflag_"+ dataId ,!isNoTip);
                        newPopupObj.remove();
                }
                // 发起接口
        ...
        const chatId = "alarms_li_" + alarmMessageType + "_" + dataId
        $(`#${chatId} .smallWindow-swtich`).click(function(e) {
            e.stopPropagation();
            RBChatSmallWindowUI.showWindow(dataId, alarmMessageType)
        })

        //** 为“消息”列表的item添加点击事件处理
        $(`#${chatId}`).click(function () {
            console.log('ttt _chatType 更新 chat', 2)
            window._chatType = 'chat'
            window._chatId = null
            const data = $(`#alarms_li_${alarmMessageType}_${dataId}`).attr('im-amd')
            if (RBChatSmallWindowUI.checkIsWindow(dataId)) {
                window._chatId = dataId
                RBChatSmallWindowUI.resetWindow(dataId)
            } else {
                sessionStorage.setItem('lastActiveChat', data)
            }
            //// 取出vid值
     ...
     * @param msgContent
     */
    UIModule4.prototype.updateItemContent = function (alarmMessageType, dataId, time, msgContent, title, isGroupSend) {
        ...
        const _notip =  RBChatUtils.isKeyVal('noTipStr',dataId);

        // 兼容type异常问题
        if (!magContentObj?.length && ['4', '8'].includes(alarmMessageType + '')) {
            let newAlarmMessageType = alarmMessageType == 4 ? 8 : 4
            msgTimeObj = $("#alarms_li_msgtime_" + newAlarmMessageType + "_" + dataId);
            magContentObj = $("#alarms_li_msgcontent_" + newAlarmMessageType + "_" + dataId);
            msgTitleObj = $("#alarms_li_msgtitle_" + newAlarmMessageType + "_" + dataId);
        }
        ...
        if (msgContent) {
            ...
            if (_notip || isGroupSend) return
            const { index: newChatIndex } = AlarmsProvider.updateChatTime({ alarmMessageType, dataId, time, is2Top }, true)
            const el = $('#kchat-im-panel-userlist-alarms li').eq(newChatIndex)
            const _el2 = $(`#alarms_li_${alarmMessageType}_${dataId}`)
            _el2.attr('im-timeStamp', time)
            if (el.attr('id') == `alarms_li_${alarmMessageType}_${dataId}`) return
            el.before(_el2);
        }
        else {
    ...
    UIModule4.prototype.deleteWithConfirm = function (alarmMessageType, dataId) {
        ...
        var fn_submitCallback = function () {
            AlarmsProvider.deleteChatCache(dataId)
            RBChatMsgCache.updateCache({ chatId: dataId, type: 'deleteChat' })
            RBChatDialogHelper.closeDialog(dialogId);
            that.delete(alarmMessageType, dataId);
            that.updateChatCache({}, dataId, 'delete')
            RBChatSmallWindowUI.handleCloseWindow(dataId)
        };
    ...
    UIModule4.prototype.delete = function (alarmMessageType, dataId) {
        ...
        const { tempChatMessage, reviceMessage, groupChatMessage } = AlarmMessageType
        if (alarmMessageType == tempChatMessage
            || alarmMessageType == reviceMessage
            || alarmMessageType == groupChatMessage) {

            var localUserUid = LocalUserInfo.getObj().user_uid;

            var isDeleteGroupChatting = (alarmMessageType == groupChatMessage);
                ...
                , function (returnValue) {
                    ...
                    if (returnValue) {
                        ...
                        if ('1' == returnValue) {
                            ...
                            // 如果是群聊
                            else if (alarmMessageType == groupChatMessage) {
                                // 从缓存中删除此群聊天记录
                                GroupChattingCache.removeChatCache(dataId);
                            }
    ...
    UIModule4.prototype.selectedItem = function (alarmMessageType, dataId) {
        ...
        if ((_selectedAlarmType != -1 && alarmMessageType != -1 && _selectedAlarmType === alarmMessageType)
            ...
            && !RBChatSmallWindowUI.checkIsWindow(dataId)
            && (window._chatId != dataId)
        ) {
            ...
            RBChatRestHelper.abortPendingRequestQueue()
            ...
            if (_selectedAlarmType != -1 && (_selectecAlarmDataId && _selectecAlarmDataId !== 'N/A')) {
                // if (window._chatType == 'chat') {
                //     window._chatId = null
                // }
                if(window._chatType != 'window' && !$("#alarms_li_" + _selectedAlarmType + "_" + _selectecAlarmDataId).attr('beTop')){
            ...
            if (window._chatType != 'window') {
                // 先清空UI显示
                $('#im-panel-main-rightdetail-content').empty()
            } 
            if(isGroupChatting){
                ...
                RBChatRestHelper.submitGetMsgRedTime(dataId,function(returnValue){
                    if (returnValue && returnValue.length && dataId == RBChatMainUI.getCurrentSelectedAlarmDataId()) {
                        window.lastReadTime = returnValue;
                        const _list = document.querySelectorAll('.chat-item')
                        for (let i = 0; i < _list.length; i++) {
                            const _el = $(_list[i]).find('.chat-info-unread')
                            if (Number(_el.attr('time')) < Number(window.lastReadTime || 0)) {
                                _el.addClass('chat-info-read')
                            }
                        }
                    }

                },function(){}, window._chatType != 'window')
                // 加载右边的详情查看功能
                RBChatRightDetailUI.showTabsForSelectedAlarm(alarmMessageType, dataId);
    ...
    UIModule4.prototype.resetUnread = function (alarmType, dataId) {
        ...
        var unReadObj = $("#alarms_li_unreadflag_" + dataId);
        unReadObj.text(0);
        unReadObj.hide();
      ...
      const _tab = $('.dom_item').attr('tabident');
        if (_tab == 1) {
            ...
            for (let i = 0; i < _els.length; i++) {
                console.log(_els[i], _els[i].style.display , 1241241241255)
    ...
    UIModule4.prototype.getTotalUnreadCount = function () {
        ...
        for (var i = 0; i < objArray.length; i++) {
            ...
            if (obj) {
                const _parentEl = obj.closest('.alarms_li_item');
                if (_parentEl.css('display') == 'none') continue
