...
(function () {
    ...
    function processRecivedMessage(isme, isGroupChatting, alarmMessageDTO, chatMsgEntity, isGroupSend=false) {
        ...
        const isScrollBottom = RBChatAlarmsUI.isScrollBottom(alarmMessageDTO.dataId)
        ...
        if (chatMsgEntity) {
            ...
            if (isGroupChatting) {
                if(!isGroupSome){
                    isSame = GroupChattingCache.putChatCache(alarmMessageDTO.dataId, chatMsgEntity, false);
                }
            }else{
                RBChatAlarmsUI.updateChatCache({ [chatMsgEntity.uid]: chatMsgEntity.date }, chatMsgEntity.uid)
                if(!isGroupSome){
                    isSame = SingleChattingCache.putChatCache(alarmMessageDTO.dataId, chatMsgEntity, false);
                }
            }
            //消息重复不处理
            if(isSame == 1){
             ...
             if(chatMsgEntity.msgType - 0 ==11){
                ...
                const { uid, beyongUid } = msg_obj.from
                isReplyMe = uid == myuid && beyongUid != uid
                if(isReplyMe){
        ...
        if (alarmMessageDTO) {
            // 插入或更新首页“消息”item
            RBChatAlarmsUI.insertOrUpdate(alarmMessageDTO, !isGroupSend,isGroupSend);
        }
        // if (!isme) {
        ...
        // }
         ...
         // 如果现在收到的消息正是属于当前正在聊天着的用户，则不需要在左另用户列表上显示未读标识
         if (isCurrentSelectedAlarm(alarmMessageDTO.alarmMessageType, alarmMessageDTO.dataId) || RBChatSmallWindowUI.checkIsWindow(alarmMessageDTO.dataId)) {
            needInsertToContentPane = true;
        }
        // 否则显示未读标识
        else {
        ...
        if (needInsertToContentPane) {//} && !firstOnlineUser) {
            if (chatMsgEntity && !isGroupSome) {
                // 插入一条消息显示到消息面板上
                RBChatChattingContentPaneUI.insertChatItemWithP(chatMsgEntity, alarmMessageDTO.dataId);
            }
        }
        RBChatMsgCache.updateCache({ chatId: alarmMessageDTO.dataId, isGroupChatting, data: chatMsgEntity, type: 'add' })
        // 自动滚动底部（以便即时显示最新消息）
        if(needInsertToContentPane) {
            if (isScrollBottom || isme) {
                RBChatChattingContentPaneUI.scrollToBottom4IM(alarmMessageDTO.dataId);
            } else {
                window._unReadNum++
                RBChatAlarmsUI.updateUnReadUI(alarmMessageDTO.dataId)
            }
        }

        // 是否为消息免打扰群
        let isNoTip = false
        if (alarmMessageDTO?.dataId) {
            isNoTip = RBChatUtils.isKeyVal('noTipStr', alarmMessageDTO.dataId)
        }
        // 如果当前首页的"消息"tab处于不可见状态，则设置“在线队列”的ui上显示一个新消息提示红点点
        if (!isme && !RBChatMainWindowUI.isAlarmsTabSelected() && !isNoTip) {
    ...
    function processRecivedRevokeCMD(chatType, fpForRevokeCMD, fromId, messageContent) {
        ...
        if (messageContentObj && messageContentObj.fpForMessage) {
            ...
            if (originalMessage) {
                ...
                RBChatMsgCache.updateCache({ chatId: originalMessage.uid, data: originalMessage, type: 'add' })
                if (sucess) {
    ...
    window.imgLoadError = function (e) {
        const _imgEl = $(e)
        const _url = _imgEl.attr('src')
        const urlObj = new URL(_url);

        let retryLoad = urlObj.searchParams.get('retryLoad');
        if (!retryLoad || parseInt(retryLoad) < 2) {
            retryLoad = parseInt(retryLoad || 0) + 1;

            urlObj.searchParams.set('retryLoad', retryLoad);
            $(e).attr('src', urlObj.toString())
        }
    }
    ...
    // 切换ui
    window.videoSwiperListUI2 = function (url) {
        RBChatUtils.fvideo([url], 0)
    }
     ...
     * @param {*} obj 
     */
    function showNewFriendUI(obj){
        // 关闭弹框
        window.closeNewFriendUII = function (uedUsername) {
            $('#mermber-new-friend-add-tip-' + uedUsername).remove();
        }
        // 点击会员聊天
        window.neww_friend_login_2_chat = function(srcUid,huiId,nickname){
            $('#mermber-new-friend-add-tip-' +srcUid).remove();
            var alarmMessageDTO = AlarmsProvider.createChatMessageAlarm(
                MsgType.TYPE_TEXT, "现在开始发起聊天吧.", decodeURIComponent(nickname), srcUid, 0);
            ...
            // 发送弹框回执
            RBChatRestHelper.new_friend_huizhi(huiId);
        }

        // 过滤重复显示
        const dom = $('#mermber-new-friend-add-tip-'+obj.userId)
        if(dom){
            dom.remove();
        }
        var html = "<div class='mebers-level-root-add-new-friend' id='mermber-new-friend-add-tip-" + obj.userId + "'>" +
            "<div class='mebers-level-root-title'><span>新好友 <a href='javascript:void(0)' onclick=\"javascript:neww_friend_login_2_chat(" + obj.userId + ",'"+obj.id+"','"+encodeURIComponent(obj.nickname)+"')\">去聊天</a></span><div onclick=\"javascript:closeNewFriendUII(\'" + obj.userId + "\')\">关闭</div></div>" +
    ...
    function showMemberLoginTIpsUI(obj) {
        ...
        // 3分钟后自动关闭弹窗
        setTimeout(() => {
            window.closeMemreLoginTipsUI(obj.userId)
        }, 3 * 60 * 1000)
                ...
                function (returnValue) {
                    ...
                    if (ree.length > 0) {
                        ...
                        if(!obj.type || obj.type =='1' || obj.type =='2'){
                            ...
                            reeIndex = parseInt(obj.type, 10);
                        }
    
                        if (ree[reeIndex]) {
                            // 发送欢迎提示语
                            sendWelcome(ree[reeIndex], obj.userId);
                        }
                    }
                }
                // 数据读取失败后的回调
        ...
        // 发送告警统计倒计时
        RBChatRestHelper.send_notice_count_num(myUserId,obj.userId,obj.type, sendWelcomeFn, sendWelcomeFn)

        // TODO: 更新在线人数和在线状态，还要更新 window.friends_group_list, 分组在线人数，群组在线人数？还有其他吗？
    ...
    function showMemberLoginTIpsUI201(obj) {
        var dialog_id = uuid.v1();
        window.closeMemreLoginTipsUI = function (id) {
            $('#mermber-login-tip-201' + id).remove();
        }

        //关闭弹框
        const dom = $('#mermber-login-tip-201'+dialog_id)
        if(dom){
            dom.remove();
        }

        var html = "<div class='mebers-level-root' id='mermber-login-tip-201" + dialog_id + "'>" +
        "<div class='mebers-level-root-title'><span>小程序报警</span><div onclick=\"javascript:closeMemreLoginTipsUI(\'" + dialog_id + "\')\">关闭</div></div>" +
        "<div class='mebers-level-root-item'>域名:" + obj.url + "</div>" +
        "<div class='mebers-level-root-item'>总连通率:" + obj.zlt + "</div>" +
        "<div class='mebers-level-root-item'>IP正确率:" + obj.ipzql + "</div>" +
        "<div class='mebers-level-root-item'>移动连通率:" + obj.ydlt + "</div>" +
        "<div class='mebers-level-root-item'>联通连通率:" + obj.ltlt + "</div>" +
        "<div class='mebers-level-root-item'>电信连通率:" + obj.dxlt + "</div>" +
        "<div class='mebers-level-root-item'>备注:" + obj.bz + "</div>" +
        "</div>";
        $('body').append(html)
    }
    ...
    async function getFeedbackLink () {
        return new Promise(resolve => {
            // 一小时内不重复拉取
            if ((window._FEEDBACK_LINK_TIME || 0) - 60 * 60 * 1000 > Date.now()) {
                return resolve()
            }
            // 反馈链接
            RBChatRestHelper.getAdminConfig('FEEDBACK_LINK', (res) => {
                window._FEEDBACK_LINK = res
                window._FEEDBACK_LINK_TIME = Date.now()
                resolve()
            })
        })
    }
     ...
     * @param {*} userId 
     */
    async function sendWelcome(welcome, userId) {
        if (welcome.includes('|||') && !window.app_applets?.length) {
            await getAppList()
        }
        if (welcome.includes('|||') && window.app_applets?.length) {
            const apps = []
            let content
            let result = welcome.split('|||')
            ...
            if (!apps.length) content = welcome

            setTimeout(() => {
                apps.forEach(item => {
                    send_im_text(item, userId, null, 9);
                })
            }, firstSend == 'app' ? 0 : 500)

            setTimeout(() => {
                if (content) {
                    send_im_text(content, userId);
                }
     ...
     * @param {*} userId 
     */
    function send_im_text(welcome, userId, callBack, msgType = 0){
    ...
    function onIMData(pFromServer) {
        ...
        // console.log('【onIMData】收到一条消息/指令，内容为：'+JSON.stringify(pFromServer), true);
        ...
        // 判断消息是否为图片消息，添加到自动下载队列
        RBChatChattingContentPaneUI.addPreloadImgQueue(pFromServer) 
        ...
        if (dataContent) {
            ...
            // 保存消息时间
            if(obj.t || obj.f){
                var local_key = typeu == UserProtocalsType.MT43_OF_TEMP$CHAT$MSG_SERVER$TO$B || typeu == UserProtocalsType.MT03_OF_CHATTING_MESSAGE ? obj.f:obj.t;
                var local_time_str =  localStorage.getItem('time-'+local_key)
                var local_time_obj ={'id':local_key}
                if(local_time_str){
                    local_time_obj = JSON.parse(local_time_str)
                }
                //存储消息会话
                localStorage.setItem('offmsg_time',obj.sm?obj.sm:timestamp);
                if(getCurrentSelectedAlarmDataId() ==local_key){
                    localStorage.setItem('time-'+local_key,JSON.stringify({'id':local_key,'unread':0}));
                }else{
                    if(local_time_obj.unread){
                        local_time_obj.unread = local_time_obj.unread + 1;
                    }else{
                        local_time_obj.unread =1;
                    }
                    localStorage.setItem('time-'+local_key,JSON.stringify(local_time_obj));
                }
            }
            // 选择了当前的群
            ...
            //已阅状态更新
            if (typeu == 100) {
                const { expireTime, id } = JSON.parse(JSON.parse(obj.dataContent).ext_msg)
                if (expireTime > Date.now()) {
                    RBChatDialogHelper.showAdminOnlineNoticeAlert(id)
                }
            }
            ...
            //群管下的会员上线通知
            if (typeu == 83) {
                const _key = `${obj.userId}_83`
                // 同个用户的上线通知, 10s内最多触发一次
                if (!ws_cache[_key]) {
                    ws_cache[_key] = setTimeout(() => {
                        ws_cache[_key] = null
                    }, 10 * 1000)
            ...
            const isFriendRed = typeu == 47 && obj.wt == 1
            //更新红包状态ui
            if (typeu == 87 || isFriendRed) {
                const t = $('#' + (obj.walletId || obj.wid) + '_redpck_root');
                const l = $('#' + (obj.walletId || obj.wid) + '_status_text');
                if (t && l) {
            ...
            if (typeu == 84) {
                ...
                try {
                    RBChatMsgCache.updateCache({ chatId: obj.userId, type: 'updateKey', key: 'redStatus', val: 1, data: obj.fingerprint })
                } catch (err) {}
                // 判断是否是当前会话
                    ...
                    window.friends_group_list.forEach(item=>{
                        ...
                        item.list.forEach(item_=>{
                            if(item_.user_uid == userId){
                                ...
                                var online = isOnline == 1 || onlineWeb == 1
                                if(!online){
                                    item_.latestOfflineTime = obj.latestOfflineTime;
                                }
                        ...
                        if(groupId.length > 0){
                             ...
                             if(dom){
                                ...
                                const  online_count = item.list.filter(item=> item.isOnline == 1 || item.onlineWeb == 1).length;
            ...
            if(typeu == 300){
                // 载入好友列表（载入数据并UI显示）
                // 5分钟内最多更新一次
                if (window._lastUpdateFor300 && (window._lastUpdateFor300 + 10 * 60 * 1000) > Date.now()) return
                RosterProvider.refreshRosterAsync(function () {
                    window._lastUpdateFor300 = Date.now()
                    RBChatRosterUI.reloadFromCache(() => {
                    })
                });
            }
        ...
        // 【IM指令：临时/陌生人聊天消息->由服务端转发给接收人B的【步骤2/2】
            ...
            if (msgType == MsgType.TYPE_REVOKE) {
                try {
                    RBChatMsgCache.updateCache({ chatId: tcmd.f, data: JSON.parse(tcmd.m).fpForMessage, type: 'delete' })
                } catch (err) {}
                // 进行消息撤回指令的处理逻辑
            ...
            // 收到的是正常的聊天消息等
                ...
                if(obj.ued){
                    ...
                    chatMsgEntity.uname = _obj.uedUsername
                    chatMsgEntity.ulastlogin = _obj.ullt 
                    chatMsgEntity.heade_photo = obj.h
                    alarmMessageDTO.heade_photo = obj.h
                }
        ...
        // 【IM指令：群聊/世界频道聊天消息->由服务端转发给接收人B的【步骤2/2】
            ...
            if (ge) {
                ...
                if (msgType == MsgType.TYPE_REVOKE) {
                    try {
                        RBChatMsgCache.updateCache({ chatId: toGid, data: JSON.parse(msgBody.m).fpForMessage, type: 'delete' })
                    } catch (err) {}
                    // 进行消息撤回指令的处理逻辑
                ...
                // 收到的是正常的聊天消息等
                    ...
                    if(obj.ued){
                        ...
                        chatMsgEntity.cmoney =_obj.uedMoney
                        chatMsgEntity.uname = _obj.uedUsername
                        alarmMessageDTO.uname = _obj.uedUsername
                        chatMsgEntity.ulastlogin = _obj.ullt 
                        alarmMessageDTO.ulastlogin = _obj.ullt
                        chatMsgEntity.heade_photo = obj.h
                        alarmMessageDTO.heade_photo = obj.h
                    }
        ...
        // 【IM指令：群聊系统指令->通用的系统信息给指定群员（由Server发出，指定群员接收）】
            ...
            // 私聊
            const isFriend = cmdBody.wt == 1
            if (isFriend) {
                toGid = cmdBody.userId
                ge = {
                    g_name: cmdBody.nickName
                }
            }
            if (ge) {
                var alarmMessageDTO = AlarmsProvider.createAGroupChatMsgAlarm(cmdBody.ty, cmdBody.m, ge.g_name, toGid, cmdBody.nickName, 0, isFriend);
                ...
                // 领取红包消息
                if(obj.ty == 90){
                    alarmMessageDTO.origin_name = obj.nickName
                    chatMsgEntity.origin_name  = obj.nickName
                    chatMsgEntity.beyongUid  = obj.userId
                    alarmMessageDTO.beyongUid  = obj.userId
                    if(obj.ued){
                        const _obj = JSON.parse(obj.ued)
                ...
                // 将数据放入并在UI上显示
                processRecivedMessage(false, !isFriend, alarmMessageDTO, chatMsgEntity);
            }
            else {
        ...
        // 【IM指令：普通一对一聊天消息->（聊天消息可能是：文本、图片、语音留言、礼物等）】
            ...
            if (ree != null) {
                ...
                if (msgType == MsgType.TYPE_REVOKE) {
                    ...
                    try {
                        RBChatMsgCache.updateCache({ chatId: msgBody.f, data: JSON.parse(msgBody.m).fpForMessage, type: 'delete' })
                    } catch (err) {}
                    // 进行消息撤回指令的处理逻辑
                    ...
                    if(ree.nickname){
                        alarmMessageDTO.origin_name = ree.nickname;
                    }
                    RBChatAlarmsUI.insertOrUpdate(alarmMessageDTO, true);
                }
                // 收到的是正常的聊天消息等
                    ...
                    if(ree.nickname){
                        alarmMessageDTO.origin_name = ree.nickname;
                        chatMsgEntity.origin_name = ree.nickname;
                    }
                    if(obj.ued){
                        const _obj = JSON.parse(obj.ued)
                        // console.log('ued消息测试-单聊',_obj)
    ...
    function initFileCDN(){
        RBChatConfig.FILE_HTTPS_URL = 'https://ossoversea.nongzhiw.cn'
        RBChatRestHelper.getFileCDN(function(data){
            console.log("======initFileCDN=======", {data});
            // RBChatConfig.FILE_HTTPS_URL = data
            window._VIDEO_HTTPS_URL = data
        }, function(err){
            console.log("=======initFileCDN===",{err});
        })
    }

    function getLastNotice(retryTimes = 0) {
        if (retryTimes > 3) return
        RBChatRestHelper.getLastNotice((res) => { // success
            if (res) {
                RBChatDialogHelper.showAdminOnlineNoticeAlert(res)
            }
        }, () => { // error
            getLastNotice(++retryTimes)
        })
    }
    ...
    function onIMDisconnected() {
        ...
        // RBChatMsgCache.updateCache({ type: 'clear' })
    ...
    function onIMReconnectSucess() {
        ...
        clearTimeout(reconTImer2)
        reconTImer2 = setTimeout(() => {
            RBChatChattingContentPaneUI.sendMsgRetry('init')
            // 群管通知 获取最新通知信息
            if (!window.isLiXian) {
                getLastNotice()
            }
        ...
        // 等待0-5分钟执行拉取数据，防止同一时间拉爆服务器
        // const _waitTime = Math.random() * 5 * 60 * 1000
    ...
    function onIMMessagesLost(lostMessages) {
        ...
        if (lostMessages != null) {
            console.log("[消息发送失败]:"+JSON.stringify(lostMessages))
            logger.info("[消息发送失败]:"+JSON.stringify(lostMessages));
     ...
     * @since 4.0
     */
    function onIMMessagesBeReceived(p) {
        const { dataContent: theFingerPrint, typeu } = p
        console.log('ttt 消息 ack', theFingerPrint, p, new Date(), JSON.parse(JSON.stringify(window._groupSendMap || {})))
        if (theFingerPrint != null) {
            log("[IM] [收到对方消息应答] fp=" + theFingerPrint, true);
            if (window._groupSendMap && p.from?.startsWith('groupSend')) { // 群发消息回执，需渲染ui
                RBChatGroupSend.groupSendTaskOp(p, 'update')
                const _msgs = JSON.parse(theFingerPrint)
                _msgs.forEach(({ fp }) => {
                    const { uid, content, msgType, nk } = window._groupSendMap[fp]
                    delete window._groupSendMap[fp]
                    if (window.requestIdleCallback) {
                        window.requestIdleCallback((deadline) => {
                            while (deadline.timeRemaining() > 0) {
                                if (!document.querySelector(`#alarms_li_4_${uid}`) && document.querySelectorAll('.alarms_li_item').length >= 400) return
                                // 執行非緊急任務
                                RBChatAlarmsUI.updateChatCache({ [LocalUserInfo.getUid()]: Date.now() }, uid) 
                                const alarmMessageDTO = AlarmsProvider.createChatMsgAlarmForLocal(msgType, content, nk, uid);
                                var chatMsgEntity = ChatMsgEntity.prepareSendedMessage(content, 0, fp, msgType);
                                RBChatMainUI.processRecivedMessage(true, false, alarmMessageDTO, chatMsgEntity, true);
                            }
                        });
                    } else {
                        if (!document.querySelector(`#alarms_li_4_${uid}`) && document.querySelectorAll('.alarms_li_item').length >= 400) return
            ...
            // 发消息ack, 删除消息发送重试队列
            if (typeu == -1) {
                RBChatChattingContentPaneUI.deleteSendMsgRetryQueue(theFingerPrint)
            }
    ...
    function loadAllDatas() {
        ...
        AlarmsProvider.optimizeCache()

        // 刷新本地用户信息的UI显示
        RBChatLocalUserUI.refresh();

        // 获取管理ids
        RBChatRestHelper.submitGetGoupMannagersToServer(function(returnValue){
            try {
                window.managerIds = JSON.parse(returnValue) || []
            } catch (err) {}

            // 读取上次浏览器刷新起最后激活的会话，并激活
            const _lastWindows = localStorage.getItem('windowList')
            if (_lastWindows) {
                const list = JSON.parse(localStorage.getItem('windowList') || '[]')
                if (list.length && RBChatSmallWindowUI) {
                    list.forEach((item, index) => {
                        const { _windowId, _type, ...args } = item
                        RBChatSmallWindowUI.showWindow(_windowId, _type, args)
                    })
                }
            }

            // 读取上次浏览器刷新起最后激活的会话，并激活
            const _lastActiveChat = sessionStorage.getItem('lastActiveChat')
            if (_lastActiveChat) {
                const { alarmMessageType, dataId } = JSON.parse(sessionStorage.getItem('lastActiveChat') || '{}')
                if (alarmMessageType && dataId) {
                    window._chatType = 'chat'
                    window._chatId = null
                    if (RBChatSmallWindowUI.checkIsWindow(dataId)) {
                        window._chatId = dataId
                        RBChatSmallWindowUI.resetWindow(dataId)
                        $('#kchat-im-panel .im-panel-inputcontent').attr('placeholder', '禁言中 …')
                        $('#kchat-im-panel .im-panel-inputcontent').attr('disabled', true)
                    }
        ...
        RBChatRestHelper.getAdminConfig('GROUP_BAN_ADMIN_UIDS', (res) => {
            window._GROUP_BAN_ADMIN_UIDS = res ? res.split(',') : []
        })

        RBChatRestHelper.getAdminConfig('GROUP_BAN_ADMIN_OPEN', (res) => {
            window._GROUP_BAN_ADMIN_OPEN = res
        })
    ...
    function isCurrentSelectedAlarm(alarmType, dataId) {
        return ((mCurrentSelectedAlarmType == alarmType)
            && (mCurrentSelectedAlarmDataId == dataId)) || ((alarmType == 8 || alarmType == 4) && mCurrentSelectedAlarmDataId == dataId)
    }
