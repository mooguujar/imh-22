...
(function () {
     ...
     */
    function _uploadFile(file, onProgress, errorFun, successFunc) {
        //获取视频时长
        ...
        getVideoTime(file).then(duration => {
            var coss_d = function(data){
                ...
                cos.uploadFile({
                    Bucket: 'im-1306281965', /* 填写自己的bucket，必须字段 */
    ...
    */
    ...
    var _fimgList = function (imgList, index) {
        ...
        items += `<img src="${imgList[window.cueIndex]}" />`
        return $(".container").html(items);
    }
    // 下一张图片
    $(".next").click(function () {
        ...
        if (window.cueIndex < window.imgListTotol.length - 1) {
            ...
            items = ''
            items += `<img src="${window.window.imgListTotol[cueIndex + 1]}" />`
    ...
    var _showIFrameURL = function (url, title, callBack) {
        ...
        if (_isMobile()) {
            ...
            var html_ =
                ...
                + '       <iframe src=\'' + url + '&pfont=45px&v=338' + '\'/ >'
                + '   </div>'
                + '</div>';
        } else {
            var html_ =
    ...
    var _draw_mul_pic = function(imgs_list,isVideo=false){
            ...
            if(imgs_list.length == 1 || imgs_list.length == 2){
                content_html =
                    "<div class='item' style='justify-content: center;'>"+ imgs_list.map((item,index) =>{return ""+get_img_url(item.fileMd5, 'width:100%;height:250px;'+(index!=0 ? 'margin-left:5px':''),  imgs_list.length == 2 ? 'width:50%' : '')+""}).join('')
                    +"</div>"
            // 3张
            ...
            // 5张
                ...
                var img_top = "<div style='display:flex;width: 100%'>"+get_img_url(imgs_list[0].fileMd5, 'width:100%;height:200px;', 'width:50%;margin-right:5px')+get_img_url(imgs_list[1].fileMd5, 'width:100%;height:200px;', 'width:50%;margin-right:5px')+"</div>"
                ...
                var rows_html = result_list.map((item,i)=>{
                    ...
                    if(len == 2){
                        ...
                        imgs_html = item.map((item_,index)=>{
                            return  get_img_url(item_.fileMd5, 'width:100%;height:200px;' , 'width:33%;' +(index != 0 ? '':'margin-right:5px'))
                        }).join('')
                    }else{
                        imgs_html = item.map((item_,index)=>{
                            return  get_img_url(item_.fileMd5, 'width:100%;height:'+(len == 4? 120:200)+'px;', 'width:33%;'+(index != 0 ? 'margin-left:5px':''))
                        }).join('')
                    }
...
(function () {
    ...
    function _newMessagePromt() {
        if (RBChatUtils.isMsgToneOpenFromCookie())
            RBChatUtils.playAudio(AUDIO_BASE_URL + "audio_msg.mp3")
    }
