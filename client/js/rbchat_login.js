...
(function(){
    ...
    if(_isMobile()){
        loadCss('css/login_phone.css');
        $('#is_pc').css({'display': 'none'});
        $('#rbchat-wrapper-footer').css({'display': 'none'});

    } else {
        loadCss('css/login.css');
        $('.tips_dom').css({'display': 'none'});
        $('.row_1').css({'display': 'none'});
        $('.row_1_forgot').css({'display': 'none'});
        $('.sdzw').css({'display': 'none'});
        $('.phone_register_title').css({'display': 'none'});
    }
    setTimeout(()=>{
    ...
    $('.tick').click(function(){
        if(status === 0){
            $('#tips_val').css({'display': 'none'})
            let items = `<img src="images/bg01.png" alt="">`
            $('#tips_val').css({'display': 'none'})
            $('.tick').html(items)
            status = 1
        } else if(status === 1) {
            $('.tick').html('')
            let items = `<div class="redMarker "></div>`
            $('.tick').html(items)
            $('#tips_val').css({'display': 'block'})
            status = 0
        }
    })
    ...
    function doLogin()
    {
        ...
        var loginName = $.trim($('#rbchat-form-loginname').val());
        var loginPsw = $('#rbchat-form-loginpsw').val();

        //登录类型
        var  loginType = $('#longinType').val();
        var  countryCode =  $('#contry').val();
        ...
        if (loginName.length === 0) {
            ...
            errorMsg = '密码长度至少是6位哦';
        } else if (!RBChatUtils.stringIsInt(loginName) && !RBChatUtils.stringIsEmail(loginName)) {
            errorMsg = '登陆账号不符号要求（必须是邮箱或UID）';
        } else if(_isMobile() && status === 0){
            errorMsg = '请阅读并勾选页面协议';
        }
        ...
        else
        {
            ...
            // 手机号要增加区号
            if(loginType - 0 == 1){
                loginName = countryCode+loginName;
            }
            // 提交登陆认证
            RBChatRestHelper.submitLoginToServer(loginName, loginPsw
                , function (returnValue){
                    ...
                    if(!RBChatUtils.isStringEmpty(returnValue))
                    {
                        ...
                        // 如果需要保存密码，则保存到cookie中
                        if(isNeedRememberPassword())
                            saveAccountAndPswToCookie(loginName, loginPsw);
                            localStorage.setItem('_userInfo',returnValue)
