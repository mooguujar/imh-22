...
(function () {
    ...
    w.IMSDK = {
        ...
        loginImpl : function (varloginInfo, wsUrl, supportSSL){
            ...
            this._socket.on(MBProtocalType.C_IMEVT_COMMON$DATA, function (p) {
                if(w.IMSDK._debugEnable){
            ...
            this._socket.on(MBProtocalType.S_IMEVT_RECIVED, function (p) {
                ...
                // 将收到的应答事件通知事件处理者
                w.IMSDK.emitLocalLogicEvent(MBLocalLogicEvent.LOCAL_LOGIC_EVENT_MESSAGE_BE_RECIEVED, theFingerPrint);
     ...
     */
    w.IMSDK.onLocalLogicEvent(MBLocalLogicEvent.LOCAL_LOGIC_EVENT_MESSAGE_BE_RECIEVED, function (theFingerPrint) {
        if (theFingerPrint) {
            w.IMSDK.callback_onIMLog('[E]  收到对方已收到消息事件的通知，fp=' + theFingerPrint, true);

            if (w.IMSDK.callback_onIMMessagesBeReceived)
                w.IMSDK.callback_onIMMessagesBeReceived(theFingerPrint);
        }
    });
