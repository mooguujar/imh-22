...
function MBQoS4ReciveDaemon(){
    ...
    this.recievedMessages = new HasMapClass();
 ...
 */
MBQoS4ReciveDaemon.prototype.excute = function () {
    // 极端情况下本次循环内可能执行时间超过了时间间隔，此处是防止在前一
    // 次还没有运行完的情况下又重复过劲行，从而出现无法预知的错误
    if (!this._excuting) {
        this._excuting = true;

        // logger.info("[IMSDK-本机QoS]【QoS收】+++++ START 暂存处理线程正在运行中，当前长度" + this.recievedMessages.size() + ".");
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            var delta = common.getCurrentUTCTimestamp() - (recievedTime ? recievedTime : 0);

            // TODO: 最终代码时要删除掉！！！！
            console.debug('___________________________cccccccc，key=' + key + ', recievedTime=' + recievedTime
                + ',  getCurrentUTCTimestamp()=' + common.getCurrentUTCTimestamp() + ', delta=' + delta);

            // 该消息包超过了生命时长，去掉之
            if (delta >= this.MESSAGES_VALID_TIME) {
                logger.warn("[IMSDK-本机QoS]【QoS收】指纹为" + key + "的包已生存" + delta + "ms(最大允许" + this.MESSAGES_VALID_TIME + "ms), 马上将删除之.");
                this.recievedMessages.remove(key);
            }
        }
    }

    // logger.debug("[IMSDK-本机QoS]【QoS收】+++++ END 暂存处理线程正在运行中，当前长度" + this.recievedMessages.size() + ".");
 ...
 */
MBQoS4ReciveDaemon.prototype.startup = function (immediately) {
    this.stop();
 ...
 * <b>本线程的启停，目前属于MobileIMSDK算法的一部分，暂时无需也不建议由应用层自行调用。</b>
 */
MBQoS4ReciveDaemon.prototype.stop = function () {
 ...
 */
MBQoS4ReciveDaemon.prototype.addRecievedProtocal = function (p) {
    if (p && p.QoS)
 ...
 */
MBQoS4ReciveDaemon.prototype.addRecieved = function (fingerPrintOfProtocal) {
    if (fingerPrintOfProtocal) {
        if (this.recievedMessages.contains(fingerPrintOfProtocal))
            logger.warn("[IMSDK-本机QoS]【QoS收】指纹为" + fingerPrintOfProtocal
        ...
        logger.warn("[IMSDK-本机QoS]【QoS收】无效的 fingerPrintOfProtocal==null!");
    }
};

MBQoS4ReciveDaemon.prototype.putImpl = function (fingerPrintOfProtocal) {
    if (fingerPrintOfProtocal)
        this.recievedMessages.put(fingerPrintOfProtocal, common.getCurrentUTCTimestamp());
};
 ...
 */
MBQoS4ReciveDaemon.prototype.hasRecieved = function (fingerPrintOfProtocal) {
 ...
 * 据交叉。
 */
MBQoS4ReciveDaemon.prototype.clear = function () {
 ...
 * @return {int} 列表大小
 */
MBQoS4ReciveDaemon.prototype.size = function () {
 ...
 */
MBQoS4ReciveDaemon.prototype.setCheckInterval = function (interval) {
 ...
 */
MBQoS4ReciveDaemon.prototype.setMessageValidTime = function (time) {
    this.MESSAGES_VALID_TIME = time;
};


module.exports = MBQoS4ReciveDaemon;