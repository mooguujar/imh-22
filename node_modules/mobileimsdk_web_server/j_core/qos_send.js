...
// 引用sdk主类对外了暴露的用户管理对象
// var userProcessor = require('./im-server-sdk.js').userProcessor;
// 引用SDK主类的公开方法
// var publicSendDataLocalWithProtocal = require('./im-server-sdk').publicSendDataLocalWithProtocal;

// // 引用SDK主类的公开方法
// var callback_onMessagesLost = require('./im-server-sdk').callback_onMessagesLost;
...
function MBQoS4SendDaemon() {
    ...
    this.sentMessages = new HasMapClass();
    /* 本Hash表目前仅用于QoS重传判断是否是“刚刚”发出的消息之用（别无它用），<key=String, value=long> */
    this.sendMessagesTimestamp = new HasMapClass();
 ...
 * @see #doRetryCheck()
 */
MBQoS4SendDaemon.prototype.excute = function () {
    // 极端情况下本次循环内可能执行时间超过了时间间隔，此处是防止在前一次还没有运行完的情况下又重复执行，从而出现无法预知的错误
    if (!this._excuting) {

        logger.info("[IMSDK-本机QoS]【QoS发】+++++ START 重试管理线程正在运行中，当前长度" + this.sentMessages.size() + ".");
 ...
 */
MBQoS4SendDaemon.prototype.doRetryCheck = function (lostMessages) {
    this._excuting = true;
    try {
        if (this.sentMessages.size() > 0)
            logger.info("[IMSDK-本机QoS]【QoS发】====== 消息发送质量保证线程运行中, 当前需要处理的列表长度为" + this.sentMessages.size() + "...");
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (p && p.QoS) {
                ...
                if (p.retryCount && p.retryCount >= this.QOS_TRY_COUNT) {
                    logger.debug("[IMSDK-本机QoS]【QoS发】指纹为" + p.fp + "的消息包重传次数已达" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次)上限，将判定为丢包！");
                ...
                // 没有达到重传上限则开始进行重传
                    ...
                    var delta = common.getCurrentUTCTimestamp() - (sendMessageTimestamp == null ? 0 : sendMessageTimestamp);
                    // 该消息包是“刚刚”发出的，本次不需要重传它哦
                    if (delta <= this.MESSAGES_JUST$NOW_TIME) {
                        logger.debug("[IMSDK-本机QoS]【QoS发】指纹为" + key + "的包距\"刚刚\"发出才" + delta
                    ...
                    //### 2015103 Bug Fix END
                        ...
                        // IMSDK.sendData(p);
                        var result = imSdk.publicSendDataLocalWithProtocal(p, null);
                        logger.debug("[IMSDK-本机QoS]【QoS发】指纹为" + p.fp + "的消息包已重传完成（result="+result+"）。");
                        ...
                        logger.debug("[IMSDK-本机QoS]【QoS发】指纹为" + p.fp + "的消息包已进行重传，此次之后重传次数已达" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次).");
                    }
                }
            }
            // value值为null，从列表中去掉吧
        ...
        logger.warn("[IMSDK-本机QoS]【QoS发】消息发送质量保证线程运行时发生异常：", eee);
    }
 ...
 */
MBQoS4SendDaemon.prototype.onRetryCheck = function (al) {
    if (al && al.length > 0) {
 ...
 */
MBQoS4SendDaemon.prototype.notifyMessageLost = function (lostMessages) {
    // IMSDK.emitLocalLogicEvent(MBLocalLogicEvent.LOCAL_LOGIC_EVENT_MESSAGE_LOST, lostMessages);
    imSdk.callback_onMessagesLost(lostMessages);
};
 ...
 */
MBQoS4SendDaemon.prototype.startup = function (immediately) {
    this.stop();
 ...
 * 注意：本线程的启停，目前属于MobileIMSDK算法的一部分，暂时无需也不建议由应用层自行调用。
 */
MBQoS4SendDaemon.prototype.stop = function () {
 ...
 */
MBQoS4SendDaemon.prototype.exist = function (fingerPrint) {
 ...
 */
MBQoS4SendDaemon.prototype.put = function (p) {
    if (p) {
        if (p.fp) {
            if (p.QoS) {
                // 如果列表中已经存则仅提示（用于debug）
                if (this.sentMessages.get(p.fp) != null)
                    logger.warn("[IMSDK-本机QoS]【QoS发】指纹为" + p.fp + "的消息已经放入了发送质量保证队列，该消息为何会重复？（生成的指纹码重复？还是重复put？）");

                // save it
                this.sentMessages.put(p.fp, p);
                // 同时保存时间戳
                this.sendMessagesTimestamp.put(p.fp, common.getCurrentUTCTimestamp());
            } else {
                logger.warn("[IMSDK-本机QoS]This protocal is not QoS pkg, ignore it!");
            }
        } else {
            logger.warn("[IMSDK-本机QoS]Invalid arg p.getFp() == null.");
        }
    } else {
        logger.warn("[IMSDK-本机QoS]Invalid arg p==null.");
    }
};
 ...
 * @param fingerPrint {String} 消息包的特纹特征码（理论上是唯一的）
 */
MBQoS4SendDaemon.prototype.remove = function (fingerPrint) {
    ...
    logger.warn("[IMSDK-本机QoS]【QoS发】指纹为" + fingerPrint + "的消息已成功从发送质量保证队列中移除(可能是收到接收方的应答也可能是达到了重传的次数上限)，重试次数="
 ...
 * @since 3.2
 */
MBQoS4SendDaemon.prototype.clear = function () {
 ...
 * @return {int}
 */
MBQoS4SendDaemon.prototype.size = function () {
 ...
 */
MBQoS4SendDaemon.prototype.setCheckInterval = function (interval) {
 ...
 */
MBQoS4SendDaemon.prototype.setQoSTryCount = function (cnt) {
    this.QOS_TRY_COUNT = cnt;
};


module.exports = MBQoS4SendDaemon;